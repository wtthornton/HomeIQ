...
services:
  ...
  # ❌ DEPRECATED: enrichment-pipeline service removed in Epic 31 (October 2025)
  # Events now flow directly: websocket-ingestion → InfluxDB
  # enrichment-pipeline:
  ...
  admin-api:
    ...
    depends_on:
      - influxdb
      - websocket-ingestion
      # enrichment-pipeline dependency removed (Epic 31)
    networks:
      - homeiq-network
    environment:
...
services:
  ...
  # enrichment-pipeline resource limits removed (Epic 31 - service deprecated)
...
│   ├── websocket-ingestion/
│   │   └── Dockerfile
│   # enrichment-pipeline/ removed (Epic 31 - deprecated)
│   └── admin-api/
│       └── Dockerfile
├── scripts/
