...
**Test override.** The state directory is `/var/lib/homeiq` on an appliance and
is overridable with `HOMEIQ_STATE_DIR`, so
`scripts/appliance/firstboot-secrets.sh` can be exercised unprivileged against a
scratch directory (`libs/homeiq-ha/tests/test_firstboot_secrets.py`). The override
changes the location and nothing else: the file is still written through a temp
file created under `umask 077` and then explicitly `chmod 600`, and when the script
runs as an ordinary user the owner is that user rather than `root`. No decision
above depends on the path being the default one.
