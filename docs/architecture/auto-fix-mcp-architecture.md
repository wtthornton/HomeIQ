...
**Status:** Current  
**Purpose:** Architecture for the config-driven auto-fix pipeline and its integration with TappsMCP (quality/experts) and docs-mcp (documentation) via direct stdio `tapps-mcp`. This document is the single reference for pipeline design, MCP tooling, and generalization across projects/repos.

---

## 1. Overview

The **auto-fix pipeline** finds bugs, applies fixes, validates with quality tools, and opens pull requests. It is designed to run in a single repo (e.g. HomeIQ) or, in a generalized form, across multiple repos with a shared config and optional meta-repository. All MCP-based quality and documentation capabilities in this project are provided through **tapps-mcp** (direct stdio): **TappsMCP** (code quality, security, experts, doc lookup) and **docs-mcp** (documentation retrieval and search) run as direct stdio MCP servers configured in `.mcp.json` / `.cursor/mcp.json`.
...
- **MCP via direct stdio:** TappsMCP and docs-mcp are configured as direct stdio servers in `.mcp.json`; clients connect to each server directly without a gateway.
...
### 2.1 Direct stdio servers

```
            ┌─────────────────────────┐         ┌─────────────────────────┐
            │    tapps-mcp (stdio)     │         │    docs-mcp (stdio)      │
...
- **tapps-mcp:** Direct stdio MCP server. Clients configure it in `.mcp.json` / `.cursor/mcp.json` as a stdio command (e.g. `uv run tapps-mcp serve`).
- **TappsMCP:** Quality scoring, security scanning, quality gates, dead-code detection, dependency scanning, **domain experts** (RAG-backed), and **documentation lookup** (Context7-backed when configured). Tools are exposed with the prefix `mcp__tapps-mcp__tapps_*` (e.g. `mcp__tapps-mcp__tapps_quick_check`).
- **docs-mcp:** Separate direct stdio MCP server. Provides documentation retrieval and search so agents can pull in project docs (e.g. architecture, API reference, deployment), pipeline docs (auto-bugfix workflow, scan format), or other curated content during pipeline phases. Use docs-mcp when the agent needs structured access to project or pipeline documentation; use TappsMCP’s `tapps_lookup_docs` for external library documentation.

### 2.2 Client configuration

| Context | Config file | Purpose |
|--------|-------------|---------|
| **Cursor / IDE** | `.cursor/mcp.json` | Entries for `tapps-mcp` and `docs-mcp` as stdio commands. Enables TappsMCP and docs-mcp in the IDE. |
| **Headless / auto-bugfix** | `.mcp.json` (project root) | Same `tapps-mcp` and `docs-mcp` entries so Claude CLI can call TappsMCP and docs-mcp during scan, fix, refactor, test, and feedback. |

Both configs expose the same servers via direct stdio; tool names are prefixed by the server name (e.g. `mcp__tapps-mcp__...`). See [.cursor/MCP_SETUP_INSTRUCTIONS.md](../../.cursor/MCP_SETUP_INSTRUCTIONS.md).

### 2.3 When to use which tooling

| Need | Use |
|------|-----|
| Session start, project profile, quality gates, security scan, experts, validate changed files | **TappsMCP** (`mcp__tapps-mcp__tapps_*`) |
| Up-to-date **library** docs (FastAPI, pytest, InfluxDB, etc.) before writing code | **TappsMCP** `tapps_lookup_docs` (Context7-backed) |
| **Project** docs: architecture, API reference, deployment, auto-bugfix workflow, scan format | **docs-mcp** (`mcp__docs-mcp__*`) |
...
- **MCP:** config path, **tapps_mcp_server** (e.g. `tapps-mcp`), scan_tools, fix_tools, expert_domains, docs_lookup. Tool names use the `mcp__tapps-mcp__*` prefix; docs-mcp is configured as a separate stdio server.
...
Current HomeIQ behavior is equivalent to this model with defaults and a fixed `TappsMcpServer = "tapps-mcp"` in the script. See [implementation/AUTO_BUGFIX_GENERALIZATION_PLAN.md](../../implementation/AUTO_BUGFIX_GENERALIZATION_PLAN.md).

### 3.4 Streaming and dashboard

The runner uses **streaming** (e.g. `claude --print --output-format stream-json`) so that:

- Tool calls (including `mcp__tapps-mcp__tapps_*` and docs-mcp) are visible in real time.
...
**docs-mcp** (via direct stdio) is used to give the agent structured access to project and pipeline documentation:
...
Prompts for scan, fix, and refactor can instruct the agent to “use docs-mcp to load relevant project docs (architecture, pipeline, API) when needed.” No change to the MCP config is required; docs-mcp is already configured as a direct stdio server.
...
In all modes, `tapps-mcp` and `docs-mcp` remain the direct stdio servers for quality and documentation; only the project root and config paths change.
...
|----------|-------------|
| [.cursor/MCP_SETUP_INSTRUCTIONS.md](../../.cursor/MCP_SETUP_INSTRUCTIONS.md) | TappsMCP and docs-mcp via direct stdio; IDE and headless config. |
| [AGENTS.md](../../AGENTS.md) | TappsMCP tool list and tapps-mcp setup. |
...
| [scripts/auto-bugfix.ps1](../../scripts/auto-bugfix.ps1) | Entrypoint; `-TappsMcpServer` default `tapps-mcp`. |

---

*This architecture document is maintained as the single reference for the auto-fix pipeline and MCP (tapps-mcp, docs-mcp) design. Use **docs-mcp** to retrieve it during pipeline runs when agents need the full design context.*
