...
class StatsEndpoints:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.exception(f"Failed to initialize InfluxDB: {e}")
            self.use_influxdb = False
    ...
    async def close(self):
        ...
        try:
            ...
            logger.exception(f"Error closing InfluxDB client: {e}")
    ...
    def _add_routes(self):
        ...
        async def get_services_stats():
            ...
            try:
                ...
                logger.exception(f"Error getting services statistics: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        async def get_metrics(
            ...
            try:
                return await self._get_metrics(metric_name, service, limit)
        ...
        async def get_performance_stats():
            ...
            try:
                ...
                logger.exception(f"Error getting performance statistics: {e}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def _get_stats_from_influxdb(self, period: str, service: str | None = None) -> dict[str, Any]:
        """
    ...
    async def _transform_websocket_health_to_stats(self, health_data: dict[str, Any], period: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error transforming websocket health to stats: {e}")
            return {
    ...
    async def _get_metrics(self, metric_name: str | None, service: str | None, limit: int) -> list[MetricData]:
        """Get specific metrics"""
        metrics = []
    ...
    async def _get_api_metrics(self, service_name: str, service_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error getting health from {service_name}: {e}")
            return {
                "service": service_name,
                "events_per_hour": 0.0,
                "uptime_seconds": 0.0,
    ...
    async def _get_active_data_sources(self) -> list[str]:
        ...
        try:
            ...
            logger.exception(f"Error querying active data sources from InfluxDB: {e}")
    ...
    async def _get_api_metrics_with_timeout(self, service_name: str, service_url: str, timeout: int) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error getting metrics from {service_name}: {e}")
    ...
    def _transform_health_to_stats(self, health_data: dict[str, Any], service_name: str, period: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error transforming health data for {service_name}: {e}")
            return {
                "metrics": {"error": str(e)},
                "trends": {},
                "alerts": [{
                    "service": service_name,
                    "level": "error",
                    "message": f"Failed to transform health data: {e!s}",
                }],
            }

    def _create_fallback_metric(self, service_name: str, status: str, error_message: str | None = None) -> dict[str, Any]:
