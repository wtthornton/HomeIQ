...
def upgrade() -> None:
    ...
    result = conn.execute(
        sa.text(
            "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('suggestions', 'automation_versions')"
        )
    )
    ...
    if "suggestions" in existing_tables:
        ...
        if "json_schema_version" not in suggestions_columns:
            op.add_column(
                "suggestions", sa.Column("json_schema_version", sa.String(), nullable=True)
            )
    ...
    if "automation_versions" in existing_tables:
        ...
        if "automation_json" not in automation_versions_columns:
            op.add_column(
                "automation_versions", sa.Column("automation_json", sa.JSON(), nullable=True)
            )
        if "ha_version" not in automation_versions_columns:
            op.add_column(
                "automation_versions", sa.Column("ha_version", sa.String(), nullable=True)
            )
        if "json_schema_version" not in automation_versions_columns:
            op.add_column(
                "automation_versions", sa.Column("json_schema_version", sa.String(), nullable=True)
            )
