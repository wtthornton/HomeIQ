...
async def get_plan(plan_id: str, db: DatabaseSession) -> dict[str, Any]:
    """
...
async def get_compiled(compiled_id: str, db: DatabaseSession) -> dict[str, Any]:
    """
...
async def get_deployment(deployment_id: str, db: DatabaseSession) -> dict[str, Any]:
    """
...
async def get_lifecycle(conversation_id: str, db: DatabaseSession) -> LifecycleResponse:
    """
    ...
    # Get plan
    plan_query = (
        select(Plan).where(Plan.conversation_id == conversation_id).order_by(Plan.created_at.desc())
    )
    ...
    if plan:
        ...
        if compiled:
            ...
            deployment_query = select(Deployment).where(
                Deployment.compiled_id == compiled.compiled_id
            )
