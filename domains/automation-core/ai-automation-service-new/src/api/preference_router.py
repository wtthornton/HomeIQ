...
class PreferenceUpdateRequest(BaseModel):
    ...
    blueprint_preference: str | None = Field(
        None, description="Blueprint preference (low/medium/high)"
    )
...
async def get_preferences(
    ...
    return PreferenceResponse(
        max_suggestions=10, creativity_level="balanced", blueprint_preference="medium"
    )


@router.put("")
async def update_preferences(
    _request: PreferenceUpdateRequest, user_id: str = Query("default", description="User ID")
):
    """
