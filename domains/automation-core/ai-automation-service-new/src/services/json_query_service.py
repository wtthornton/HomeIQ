...
class JSONQueryService:
    ...
    def query(
        self, automations: list[HomeIQAutomation | dict[str, Any]], filters: dict[str, Any]
        ...
        logger.debug(
            f"Query returned {len(results)} automations from {len(automation_objects)} total"
        )
        return results
