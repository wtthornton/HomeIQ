...
from src.agent.services.conversation_service import Conversation, ConversationService
...
async def test_chat_endpoint_logic_new_conversation(
    ...
    with (
        patch("src.agent.database.get_session", return_value=session_generator()),
        patch(
            "src.agent.services.conversation_service.get_session", return_value=session_generator()
        ),
        patch(
            "src.agent.services.conversation_service.db_create_conversation",
            new_callable=AsyncMock,
            return_value=mock_conversation,
        ),
        patch(
            "src.agent.services.conversation_service.db_get_conversation",
            new_callable=AsyncMock,
            return_value=mock_conversation,
        ),
        patch.object(
            conversation_service, "add_message", new_callable=AsyncMock
        ) as mock_add_message,
        patch.object(
...
async def test_safe_parse_tool_arguments():
    ...
    from src.agent.api.chat_endpoints import _safe_parse_tool_arguments
...
def test_tool_call_arguments_preserved_in_response_model():
    ...
    tool_call = ToolCall(
        id="call_abc123", name="preview_automation_from_prompt", arguments=parsed_args
    )
