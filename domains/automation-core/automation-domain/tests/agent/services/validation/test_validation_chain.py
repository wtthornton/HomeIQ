...
from src.agent.models.automation_models import ValidationResult
from src.agent.services.validation.validation_chain import ValidationChain
from src.agent.services.validation.validation_strategy import ValidationStrategy
...
class MockValidationStrategy(ValidationStrategy):
    ...
    async def validate(self, _automation_yaml: str) -> ValidationResult:
        ...
        return ValidationResult(
            valid=False, errors=[f"Error from {self._name}"], warnings=[], score=50.0
        )
...
class TestValidationChain:
    ...
    async def test_validate_empty_strategies(self):
        ...
        assert (
            "All validation strategies failed" in result.errors[0]
            or "unavailable" in result.errors[0]
        )
