...
from src.agent.services.validation.basic_validation_strategy import BasicValidationStrategy
...
class TestBasicValidationStrategy:
    ...
    def test__detect_dimming_pattern_issues_single_trigger_both_states(self, mock_tool_handler):
        ...
        assert len(warnings) > 0
        assert any(
            "separate triggers" in w.lower() or "separate trigger" in w.lower() for w in warnings
        )
