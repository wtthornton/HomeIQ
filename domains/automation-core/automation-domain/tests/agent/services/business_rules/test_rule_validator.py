...
from src.agent.services.business_rules.rule_validator import BusinessRuleValidator
from src.agent.services.entity_resolution.entity_resolution_service import EntityResolutionService
...
def mock_entity_resolution_service():
    ...
    service.resolve_entities = AsyncMock(
        return_value=MagicMock(
            success=True, matched_entities=["light.office"], matched_areas=["office"]
        )
    )
    return service
...
class TestBusinessRuleValidator:
    ...
    def test_check_safety_requirements_security_domain(self):
        ...
        is_safe, warnings = validator.check_safety_requirements(
            entities=entities, services=services, automation_dict=automation_dict
        )
    ...
    async def test_validate_entity_resolution(self, mock_entity_resolution_service):
        ...
        result = await validator.validate_entity_resolution(
            "turn on office lights", target_domain="light"
        )
