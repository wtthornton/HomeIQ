...
async def test_get_state_context_cached(device_state_context_service, mock_context_builder):
    ...
    with patch.object(
        device_state_context_service.ha_client, "get_states", new_callable=AsyncMock
    ) as mock_get_states:
...
@pytest.mark.asyncio
async def test_get_state_context_missing_entities(
    device_state_context_service, mock_context_builder
):
