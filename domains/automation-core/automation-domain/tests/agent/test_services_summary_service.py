...
from src.agent.services.services_summary_service import ServicesSummaryService
...
async def test_get_summary_empty(services_summary_service, mock_context_builder):
    ...
    with patch.object(
        services_summary_service.ha_client, "get_services", new_callable=AsyncMock, return_value={}
    ):
        summary = await services_summary_service.get_summary()
