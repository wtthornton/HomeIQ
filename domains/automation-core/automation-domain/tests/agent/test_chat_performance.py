...
def mock_chat_client():
    """Create a mock AgentForge-backed chat client with fast responses."""
...
@pytest.mark.asyncio
async def test_chat_response_time_performance(test_client, mock_chat_client):
    ...
    mock_chat_client.chat_turn = AsyncMock(side_effect=fast_completion)
...
@pytest.mark.asyncio
async def test_chat_concurrent_users(test_client, mock_chat_client):
    ...
    assert mock_chat_client.chat_turn.call_count >= num_concurrent


@pytest.mark.asyncio
async def test_chat_sustained_load(test_client, mock_chat_client):
    ...
    assert total_elapsed < duration_seconds * 2, (
        f"Sustained load took too long: {total_elapsed:.2f}s"
    )


@pytest.mark.asyncio
async def test_chat_error_recovery_performance(test_client, mock_chat_client):
    """Test that error scenarios don't significantly impact performance"""
    # Mock an AgentForge failure (then success)
    ...
    async def completion_with_error(*_args, **_kwargs):
        ...
        call_count += 1
        if call_count == 1:
            raise AgentChatError("Temporary error")
        await asyncio.sleep(0.1)
        return create_fast_mock_completion("Recovery response")

    mock_chat_client.chat_turn = AsyncMock(side_effect=completion_with_error)
...
@pytest.mark.asyncio
async def test_chat_memory_usage_stability(test_client, mock_chat_client):
...
@pytest.mark.asyncio
async def test_chat_context_refresh_performance(
    test_client, mock_chat_client, mock_context_builder
):
