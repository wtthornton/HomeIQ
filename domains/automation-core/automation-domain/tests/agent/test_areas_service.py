...
from src.agent.services.areas_service import AreasService
...
async def test_get_areas_list_with_areas(areas_service, mock_context_builder):
    ...
    with patch.object(
        areas_service.data_api_client, "get_areas", new_callable=AsyncMock, return_value=mock_areas
    ):
...
async def test_get_areas_list_empty(areas_service, mock_context_builder):
    ...
    with (
        patch.object(
            areas_service.data_api_client, "get_areas", new_callable=AsyncMock, return_value=[]
        ),
        patch.object(
            areas_service.data_api_client, "fetch_entities", new_callable=AsyncMock, return_value=[]
        ),
    ):
        areas_list = await areas_service.get_areas_list()
