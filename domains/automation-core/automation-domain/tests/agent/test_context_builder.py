...
async def test_build_context_success(context_builder):
    ...
    context_builder._devices_summary_service.get_summary = AsyncMock(
        return_value="Philips Hue Go (Signify) - Office"
    )
...
async def test_get_cached_value(context_builder):
    ...
    with patch("src.agent.services.context_builder.get_session") as mock_get_session:
