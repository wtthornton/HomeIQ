...
class TestRegistriesUseTheSharedClient:
    ...
    async def test_area_registry(self):
        ws = _ws()
        with patch("src.agent.clients.ha_client.HAWebSocketClient", return_value=ws):
    ...
    async def test_ssl_context_is_forwarded(self):
        ...
        ws = _ws()
        with patch("src.agent.clients.ha_client.HAWebSocketClient", return_value=ws) as ctor:
...
class TestFailuresSurface:
    ...
    def test_rest_registry_fallbacks_are_gone(self):
        ...
        import src.agent.clients.ha_client as module
