...
from src.agent.services.automation_rag_service import SPORTS_KEYWORDS, AutomationRAGService
...
class TestSportsIntentMatching:
    ...
    def test_sports_prompts_match(self, automation_rag_service: AutomationRAGService, prompt: str):
        ...
        assert automation_rag_service._matches_sports_intent(prompt), (
            f"Expected match for: {prompt}"
        )
    ...
    def test_non_sports_prompts_no_match(
        self, automation_rag_service: AutomationRAGService, prompt: str
    ):
        """Non-sports prompts should not match sports intent."""
        assert not automation_rag_service._matches_sports_intent(prompt), (
            f"Expected no match for: {prompt}"
        )
...
class TestGetAutomationContext:
    ...
    @pytest.mark.asyncio
    async def test_returns_context_for_sports_prompt(
        self, automation_rag_service: AutomationRAGService
    ):
    ...
    @pytest.mark.asyncio
    async def test_returns_empty_for_non_sports_prompt(
        self, automation_rag_service: AutomationRAGService
    ):
