...
from src.agent.services.helpers_scenes_service import HelpersScenesService
...
async def test_get_summary_empty(helpers_scenes_service, mock_context_builder):
    ...
    with (
        patch.object(
            helpers_scenes_service.ha_client, "get_helpers", new_callable=AsyncMock, return_value=[]
        ),
        patch.object(
            helpers_scenes_service.ha_client, "get_scenes", new_callable=AsyncMock, return_value=[]
        ),
    ):
        summary = await helpers_scenes_service.get_summary()
