...
from src.agent.services.automation_patterns_service import AutomationPatternsService
from src.agent.services.context_builder import ContextBuilder
from src.agent.services.context_filtering_service import ContextFilteringService
from src.agent.services.context_prioritization_service import ContextPrioritizationService
from src.agent.services.device_state_context_service import DeviceStateContextService
from src.agent.services.devices_summary_service import DevicesSummaryService
...
class TestPhase1Features:
    ...
    async def test_health_scores_in_device_summary(self):
        ...
        with (
            patch.object(
                service.data_api_client, "get_devices", new_callable=AsyncMock
            ) as mock_get_devices,
            patch.object(
                service.data_api_client, "fetch_entities", new_callable=AsyncMock
            ) as mock_fetch_entities,
            patch.object(
                service.data_api_client, "get_areas", new_callable=AsyncMock
            ) as mock_get_areas,
        ):
    ...
    def test_entity_availability_highlighting(self):
        ...
        mock_state = {"entity_id": "light.office_go", "state": "unavailable", "attributes": {}}
...
class TestPhase2Features:
    ...
    async def test_device_capabilities_in_summary(self):
        ...
        with (
            ...
            assert (
                "effects" in summary.lower()
                or "colors" in summary.lower()
                or "presets" in summary.lower()
            )
        ...
        mock_entities = [
            {
                ...
                "device_id": "test_light_1",
                "domain": "light",
                "attributes": {"max_brightness": 255, "min_color_temp": 153, "max_color_temp": 500},
            }
        ]
    ...
    async def test_automation_patterns_service(self):
        ...
        with (
            ...
            patch.object(
                service, "_get_automation_config", new_callable=AsyncMock
            ) as mock_get_config,
        ):
            ...
            patterns = await service.get_recent_patterns(
                user_prompt="turn on office lights", limit=3
            )
