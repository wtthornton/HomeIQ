...
async def test_fetch_entities_success(data_api_client):
    ...
    with patch.object(
        ...
        return_value=_mock_response(mock_entities),
    ):
...
async def test_fetch_entities_dict_response(data_api_client):
    ...
    with patch.object(
        ...
        "call",
        new_callable=AsyncMock,
        return_value=_mock_response(mock_response_data),
    ):
        entities = await data_api_client.fetch_entities()
...
async def test_fetch_entities_http_error(data_api_client):
    ...
    with (
        patch.object(
            data_api_client._cross_client, "call", new_callable=AsyncMock, return_value=mock_resp
        ),
