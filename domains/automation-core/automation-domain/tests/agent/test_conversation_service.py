...
from src.agent.services.conversation_service import (
    ConversationService,
    ConversationState,
...
class TestConversationTitleAndSource:
    ...
    async def test_create_conversation_with_source_proactive(self, conversation_service):
        ...
        conversation = await conversation_service.create_conversation(
            title="💡 Energy suggestion", source="proactive"
        )
        assert conversation is not None
    ...
    async def test_create_conversation_with_title_and_source(self, conversation_service):
        ...
        conversation = await conversation_service.create_conversation(
            title="Test Title", source="proactive"
        )
    ...
    async def test_update_title(self, conversation_service):
        ...
        result = await conversation_service.update_conversation_title(
            conversation.conversation_id, "Updated Title"
        )
        assert result is True
