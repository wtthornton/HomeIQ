...
from src.agent.clients.data_api_client import DataAPIClient
from src.agent.clients.ha_client import HomeAssistantClient
from src.agent.clients.yaml_validation_client import YAMLValidationClient
from src.agent.models.automation_models import AutomationPreviewRequest
from src.agent.tools.ha_tools import HAToolHandler
...
def mock_yaml_validation_client():
    ...
    client.validate_yaml = AsyncMock(
        return_value={"valid": True, "errors": [], "warnings": [], "score": 100.0}
    )
    return client
...
def tool_handler(mock_ha_client, mock_data_api_client, mock_yaml_validation_client):
    ...
    return HAToolHandler(
        mock_ha_client, mock_data_api_client, yaml_validation_client=mock_yaml_validation_client
    )
...
class TestHAToolHandler:
    ...
    def test__parse_automation_yaml_invalid(self, tool_handler):
        ...
        request = AutomationPreviewRequest(
            user_prompt="test", automation_yaml="not a dict", alias="test"
        )
        yaml_str = "not a dict"
    ...
    @pytest.mark.asyncio
    async def test_preview_automation_from_prompt_success(
        self, tool_handler, mock_yaml_validation_client
    ):
