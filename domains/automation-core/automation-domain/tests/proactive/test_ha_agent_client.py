...
from src.proactive.clients.ha_agent_client import HAAgentClient
...
async def test_send_message_with_refresh_context():
    ...
    with patch.object(client._cross_client, "call", return_value=mock_response) as mock_call:
...
async def test_send_message_timeout():
    ...
    with patch.object(client._cross_client, "call", side_effect=httpx.TimeoutException("Timeout")):
        result = await client.send_message("Test message")
        assert result is None  # Graceful degradation
