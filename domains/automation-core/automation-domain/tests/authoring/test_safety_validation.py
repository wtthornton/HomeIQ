...
from src.authoring.clients.ha_client import HomeAssistantClient
from src.authoring.database.models import Suggestion
from src.authoring.services.deployment_service import (
    DeploymentError,
    DeploymentService,
    SafetyValidationError,
)
from src.authoring.services.yaml_generation_service import YAMLGenerationService
...
class TestSafetyValidation:
    ...
    def mock_ha_client(self):
        ...
        client.deploy_automation = AsyncMock(
            return_value={"status": "deployed", "automation_id": "automation.test_123"}
        )
        return client
...
"""
    ...
    async def test_force_deploy_skips_safety_validation(
        ...
        result = await deployment_service.deploy_suggestion(
            approved_suggestion.id, force_deploy=True
        )
    ...
    async def test_safety_validation_with_mixed_entities(
        ...
        mock_yaml_service.validate_entities = AsyncMock(
            return_value=(False, ["light.invalid", "switch.missing"])
        )
    ...
    async def test_deploy_response_includes_state_last_triggered_verification_warning(
        ...
        mock_yaml_service.validate_entities = AsyncMock(return_value=(True, []))
        mock_ha_client.deploy_automation = AsyncMock(
            return_value={
                "status": "deployed",
...
"""
    ...
    async def test_status_validation_requires_approved_status(
        self, deployment_service: DeploymentService, draft_suggestion: Suggestion
    ):
    ...
    async def test_skip_validation_bypasses_status_check(
        ...
        result = await deployment_service.deploy_suggestion(
            draft_suggestion.id, skip_validation=True
        )
...
"""
    ...
    async def test_deployment_error_on_invalid_yaml(
        self, deployment_service: DeploymentService, test_db: AsyncSession, mock_yaml_service
    ):
        ...
        mock_yaml_service.validate_yaml = AsyncMock(
            return_value=(False, "YAML syntax error: line 1")
        )
