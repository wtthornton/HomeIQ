...
class TestRunMigrations:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    @patch("src.authoring.database._run_alembic_upgrade")
    ...
    async def test_run_migrations_success(self, mock_exists, mock_upgrade):
        ...
        from src.authoring.database import run_migrations
...
class TestInitDb:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    @patch("src.authoring.database.db.initialize", new_callable=AsyncMock, return_value=True)
    @patch("src.authoring.database.run_migrations", new_callable=AsyncMock)
    async def test_init_db_success(self, mock_run_migrations, mock_initialize):
        ...
        from src.authoring.database import init_db
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    @patch("src.authoring.database.db.initialize", new_callable=AsyncMock, return_value=False)
    @patch("src.authoring.database.run_migrations", new_callable=AsyncMock)
...
class TestDatabaseConnection:
    ...
    async def test_get_db_yields_session(self):
        ...
        from src.authoring.database import db, get_db, init_db
