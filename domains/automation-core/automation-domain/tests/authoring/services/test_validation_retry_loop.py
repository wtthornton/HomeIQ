...
from src.authoring.clients.linter_client import LintFinding, LintResult
from src.authoring.services.validation_retry_loop import (
    ValidationRetryLoop,
)
...
async def test_pass_on_second_try():
    ...
    loop = ValidationRetryLoop(
        llm_client=MockOpenAIClient(responses=[corrected_yaml]),
        linter_client=MockLinterClient(
            results=[
...
def test_error_details_formatting():
    ...
    from src.authoring.services.validation_retry_loop import ValidationFinding, ValidationRetryLoop
