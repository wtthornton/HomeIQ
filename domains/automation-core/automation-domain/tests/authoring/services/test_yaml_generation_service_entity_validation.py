...
import pytest
from src.authoring.clients.llm_client import AutomationLLMClient

from src.authoring.clients.data_api_client import DataAPIClient
from src.authoring.clients.yaml_validation_client import YAMLValidationClient
from src.authoring.services.yaml_generation_service import (
    YAMLGenerationError,
...
class TestFetchEntityContext:
    ...
    @pytest.mark.asyncio
    async def test_fetch_entity_context_success(
        self, yaml_service, mock_data_api_client, sample_entities
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_entity_context_grouping(
        self, yaml_service, mock_data_api_client, sample_entities
    ):
...
class TestFormatEntityContextForPrompt:
    ...
    def test_format_entity_context_domain_limit(self, yaml_service):
        ...
        entity_context = {"entities": {"light": entities}, "total_count": 60}
...
class TestExtractEntityIds:
    ...
    def test_extract_direct_entity_id(self, yaml_service):
        ...
        data = {"entity_id": "light.office_main", "action": "light.turn_on"}
    ...
    def test_extract_nested_structure(self, yaml_service):
        ...
        data = {
            "trigger": [
                ...
                {"platform": "state", "entity_id": "binary_sensor.office_motion_2"},
            ],
            "action": {"service": "light.turn_on", "target": {"entity_id": "light.office_main"}},
        }
    ...
    def test_extract_area_id_not_added(self, yaml_service):
        ...
        data = {"target": {"area_id": "office"}, "entity_id": "light.office_main"}
...
class TestValidateEntities:
    ...
    @pytest.mark.asyncio
    async def test_validate_entities_valid(
        self, yaml_service, mock_data_api_client, sample_entities
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_entities_invalid(
        self, yaml_service, mock_data_api_client, sample_entities
    ):
...
class TestEntityValidationIntegration:
    ...
    async def test_validation_blocks_invalid_yaml(
        ...
        assert "binary_sensor.fictional_motion" in str(
            exc_info.value
