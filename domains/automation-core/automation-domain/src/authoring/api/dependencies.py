...
_llm_client: AutomationLLMClient | None = None
...
def init_clients() -> None:
    ...
    global _data_api_client, _ha_client, _llm_client
    ...
    _llm_client = AutomationLLMClient()
    ...
    logger.info(
        "Singleton HTTP clients initialized (llm=agentforge %s, linter=%s)",
        settings.agentforge_url,
...
def get_llm_client() -> AutomationLLMClient:
    """Get the AgentForge-backed LLM client singleton.

    There used to be two of these -- a chat model for plans and a reasoning
    model for YAML. Model choice is now a property of each AgentForge gene, so
    one client serves every generation call and ``get_llm_yaml_client`` is an
    alias kept for the call sites that name the YAML path explicitly.
    """
    if _llm_client is None:
        return AutomationLLMClient()
    return _llm_client


def get_llm_yaml_client() -> AutomationLLMClient:
    """Alias of :func:`get_llm_client` -- see its docstring."""
...
def get_yaml_generation_service(
    _db: DatabaseSession,
    llm_client: Annotated[AutomationLLMClient, Depends(get_llm_yaml_client)],
    ...
    """Get YAML generation service (YAML and HomeIQ JSON generation run as AgentForge workflows).
...
def get_suggestion_service(
    db: DatabaseSession,
    data_api_client: Annotated[DataAPIClient, Depends(get_data_api_client)],
    llm_client: Annotated[AutomationLLMClient, Depends(get_llm_client)],
...
def get_json_rebuilder(
    ...
    """Get JSON rebuilder (HomeIQ JSON generation runs as an AgentForge workflow)."""
    return JSONRebuilder(llm_client=llm_client)
