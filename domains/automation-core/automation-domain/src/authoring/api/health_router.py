...
#: TAP-7275: automation-domain answers one process-wide /health
#: (src/api/health.py), so this module's own /health is not mounted. The
#: validation-metrics route lives on its own router so it can be mounted
#: without also registering a second /health handler that would never be
#: reached and would collide on its OpenAPI operation id.
...
@metrics_router.get("/health/validation-metrics")
