...
from ..clients.llm_client import AutomationLLMClient
...
class YAMLGenerationService:
    ...
    def __init__(
        self,
        llm_client: AutomationLLMClient,
        ...
        Args:
            llm_client: AgentForge-backed client for generating YAML
    ...
    async def _fetch_entity_context(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(entities)} entities, grouped into {len(entity_context)} domains"
            )
            return {"entities": entity_context, "total_count": len(entities)}

        except Exception as e:
            logger.warning(
                f"Failed to fetch entity context from Data API: {e}. Continuing with empty context."
            )
            return {}
    ...
    async def generate_homeiq_json(
        self, suggestion: dict[str, Any] | Suggestion, homeiq_context: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """
...
"""
            ...
            # Generate HomeIQ JSON via AgentForge with entity context
            automation_json = await self.llm_client.generate_homeiq_automation_json(
                prompt=prompt,
            ...
            validation_result = await self.json_validator.validate(
                automation_json, validate_entities=True, validate_devices=True, validate_safety=True
            )
    ...
    async def _generate_yaml_from_homeiq_json(self, suggestion: dict[str, Any] | Suggestion) -> str:
        ...
        try:
            ...
            automation_json = await self.generate_homeiq_json(
                suggestion=suggestion, homeiq_context=homeiq_context
            )
            ...
            if not is_valid:
                error_msg = (
                    f"YAML generation failed: Invalid entities found: {', '.join(invalid_entities)}"
                )
...
"""
        ...
        try:
            ...
            plan_dict = await self.llm_client.generate_structured_plan(
                prompt=prompt,
                entity_context=entity_context,  # R2: Pass entity context
...
"""
        ...
        # Generate YAML via AgentForge with entity context (R2)
        yaml_content = await self.llm_client.generate_yaml(
            prompt=prompt,
            entity_context=entity_context,  # R2: Pass entity context
    ...
    async def validate_yaml(self, yaml_content: str) -> tuple[bool, str | None]:
        ...
        if self.yaml_validation_client:
            try:
                ...
                logger.warning(
                    f"YAML Validation Service failed, falling back to basic validation: {e}"
                )
    ...
    async def validate_entities(self, yaml_content: str) -> tuple[bool, list[str]]:
        """
        ...
        if isinstance(data, dict):
            ...
            if "." in data and len(data.split(".")) == 2:
                ...
                if len(parts[0]) > 0 and len(parts[1]) > 0:
                    ...
                    if (
                        parts[0] in valid_domains
                        or parts[0].startswith("input_")
                        or parts[0].startswith("binary_")
                    ):
