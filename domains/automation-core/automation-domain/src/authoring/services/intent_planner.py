...
class IntentPlanner:
    ...
    def __init__(
        ...
        Args:
            llm_client: AgentForge-backed client for LLM calls
...
- temperature_control requires a temperature sensor and climate device in the area."""
        ...
        # Plan on AgentForge (TAP-7275). The template catalogue and the entity
        # context are built here and passed as workflow inputs, so the gene can
        # only name a template this library actually has.
        try:
            plan_data = await self.llm_client.plan_intent(
                user_text=user_text,
                template_catalog=system_prompt,
                context=context or {},
            )
