...
class SuggestionService:
    ...
    def __init__(
        ...
        Args:
            ...
            llm_client: AgentForge-backed client for generating suggestions
    ...
    def _validate_llm_client(self) -> None:
        """Validate the AgentForge-backed LLM client is configured."""
        if not self.llm_client or not self.llm_client.configured:
            error_msg = "AGENTFORGE_API_KEY not configured. Cannot generate suggestions."
    ...
    async def _generate_from_patterns(
        ...
        for i in range(actual_limit):
            try:
                pattern = patterns[i]
                description = await self.llm_client.generate_suggestion_description(
                    pattern_data={
                        "pattern": pattern,
