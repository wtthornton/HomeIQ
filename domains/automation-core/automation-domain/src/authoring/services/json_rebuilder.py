...
class JSONRebuilder:
    ...
    def __init__(self, llm_client: AutomationLLMClient):
        """
        Initialize JSON rebuilder.

        Args:
            llm_client: AgentForge-backed client for LLM operations
        """
...
"""
        ...
        try:
            fixed_json = await self.llm_client.generate_homeiq_automation_json(
                prompt=prompt, homeiq_context=None, temperature=0.1, max_tokens=3000
            )
