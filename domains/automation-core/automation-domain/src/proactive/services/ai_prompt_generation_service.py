...
from ..clients.breakers import agentforge_breaker
...
WORKFLOW_PROACTIVE_SUGGEST = "proactive-suggest"
...
class AIPromptGenerationService:
    ...
    def __init__(
        self,
        agentforge: AgentForgeClient | None = None,
        ha_agent_url: str = "http://automation-domain:8030",
        ...
        Args:
            agentforge: AgentForge client (built from settings when omitted)
            ha_agent_url: URL to the agent slice for context retrieval
        ...
        if not self.agentforge.configured:
            logger.warning(
                "AGENTFORGE_API_KEY not set - AI prompt generation unavailable. "
        ...
        logger.info(
            "AI Prompt Generation Service initialized "
            f"(engine=agentforge:{WORKFLOW_PROACTIVE_SUGGEST}, device_validation=enabled{rag_status})"
        )
    ...
    async def generate_prompts(
        ...
        """
        if not self.agentforge.configured:
            logger.warning("No AgentForge key - using fallback generation")
            return self._fallback_generation(context_analysis, max_prompts)

        # Circuit breaker: fast-fail to rule-based fallback when AgentForge is down
        if not await agentforge_breaker.allow_request():
            logger.warning(
                "AI FALLBACK: AgentForge circuit open -- using rule-based suggestion generation"
            )
            return self._fallback_generation(context_analysis, max_prompts)
        ...
        try:
            ...
            # Step 4: Call the AgentForge workflow to generate suggestions
            suggestions = await self._call_llm(llm_context, max_prompts, device_inventory)
...
""")
    ...
    async def _call_llm(
        ...
        """Ask AgentForge's ``proactive-suggest`` workflow for suggestions.

        The home context and the device inventory are built here and passed as
        workflow inputs; the system prompt, the model and the credential belong
        to the gene. A failure records against the circuit breaker and returns
        an empty list, which ``generate_prompts`` turns into the rule-based
        fallback -- the same degradation shape the previous provider path had.
        """
        try:
            answer = await self.agentforge.run_workflow_json(
                WORKFLOW_PROACTIVE_SUGGEST,
                {
                    "home_context": context,
                    "device_inventory": json.dumps(device_inventory or {}, default=str),
                    "max_suggestions": max_prompts,
                },
            )
        except AgentForgeError as e:
            await agentforge_breaker.record_failure()
            logger.error("AgentForge suggestion workflow failed: %s", e)
            return []

        await agentforge_breaker.record_success()
        suggestions = answer.get("suggestions")
        if not isinstance(suggestions, list):
            logger.warning(
                "proactive-suggest returned no 'suggestions' list (keys=%s)",
                sorted(answer),
            )
            return []
