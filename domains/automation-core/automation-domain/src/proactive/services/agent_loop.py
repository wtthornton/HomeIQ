...
class ProactiveAgentLoop:
    ...
    async def _reason(self, observations: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            async with agentforge_breaker:
                # The reasoning step shares the `proactive-suggest` gene with
                # AIPromptGenerationService: one gene proposes both the
                # user-facing suggestions and the observe/reason/act actions
                # for the same home state, so they cannot disagree about what
                # the house is doing. This caller reads `actions`.
                answer = await self._agentforge.run_workflow_json(
                    "proactive-suggest",
                    {
                        "home_context": prompt,
                        "device_inventory": "{}",
    ...
    def _init_agentforge_client(self) -> None:
        """Wire the reasoning step to AgentForge (TAP-7275)."""
        if not self.settings.agentforge_api_key:
            logger.warning("No AgentForge API key — agent loop reasoning disabled")
            return
        from ...agentforge_client import AgentForgeClient

        self._agentforge = AgentForgeClient(
            base_url=self.settings.agentforge_url,
            api_key=self.settings.agentforge_api_key.get_secret_value(),
            project_slug=self.settings.agentforge_project_slug,
            timeout=self.settings.agentforge_timeout,
        )
