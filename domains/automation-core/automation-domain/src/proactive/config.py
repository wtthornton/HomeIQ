...
class Settings(BaseServiceSettings):
    ...
    ha_device_control_notify_url: str = Field(
        default="http://automation-domain:8030/api/notify",
        description="Notification endpoint for task results (Story 27.5)",
    )

    # AgentForge (TAP-7275): suggestion generation and the agent loop's
    # reasoning step run as the `proactive-suggest` workflow. The model name
    # and provider credential are properties of the gene, not of this service.
