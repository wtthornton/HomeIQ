...
class Settings(BaseServiceSettings):
    ...
    service_name: str = "automation-domain-agent"
    service_port: int = 8030
    database_schema: str = Field(
        default="agent",
        validation_alias="AGENT_DB_SCHEMA",
        description=(
            "Postgres schema for the agent slice. Read from AGENT_DB_SCHEMA, not "
            "DATABASE_SCHEMA: automation-domain runs three slices in one process "
            "and a single DATABASE_SCHEMA would bind all three to one schema."
        ),
    )
    ...
    ai_automation_service_url: str = Field(
        default="http://automation-domain:8030",
    ...
    # AgentForge (TAP-7275). Every model call this slice makes is a workflow
    # run; the model, sampling settings and provider credential are properties
    # of the gene definitions under agentforge/projects/homeiq/, not of this
    # process. There is no provider choice left to configure.
    ...
    agentforge_api_key: SecretStr | None = Field(
        default=None,
        description="AgentForge project API key",
    )
