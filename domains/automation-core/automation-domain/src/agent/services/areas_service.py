...
class AreasService:
    ...
    def __init__(self, settings: Settings, context_builder: ContextBuilder):
        ...
        self._cache_ttl = (
            1800  # 30 minutes (P1: Increased TTL for static data - areas rarely change)
        )
    ...
    async def get_areas_list(self) -> str:
        ...
        try:
            ...
            logger.info(
                f"✅ Generated optimized areas list: {len(areas)} areas ({len(areas_str)} chars)"
            )
            return areas_str
            ...
            try:
                ...
                logger.error(
                    f"❌ Fallback area extraction also failed: {fallback_error}", exc_info=True
                )
