...
_ml_engine_breaker = CircuitBreaker(
    name="ml-engine-suggestions", failure_threshold=3, recovery_timeout=60.0
)
...
class DeviceSuggestionService:
        ...
        try:
            ...
            for suggestion in suggestions:
                ...
                for device in matched_devices:
                    if device.get("device_id") == device_id or device.get(
                        "entity_id", ""
    ...
    async def _generate_suggestions_from_data(
        ...
        device_name = (
            device_data.get("name") or device_data.get("friendly_name") or f"Device {device_id[:8]}"
        )
        ...
        if device_context.capabilities:
            ...
            capability_names = [
                ...
                for cap in device_context.capabilities
                if cap.get("capability_name")
            ]
            capability_text = (
                ", ".join(capability_names[:3]) if capability_names else "device capabilities"
            )
        ...
        if device_context.related_synergies:
            ...
            suggestion = DeviceSuggestion(
                ...
                data_sources=DataSources(
                    synergies=[
                        s.get("id", "") for s in device_context.related_synergies if s.get("id")
                    ],
                    device_capabilities=True,
                ),
        ...
        if device_context.compatible_blueprints:
            ...
            suggestion = DeviceSuggestion(
                ...
                data_sources=DataSources(
                    blueprints=[
                        b.get("id", "") for b in device_context.compatible_blueprints if b.get("id")
                    ],
                ),
        ...
        if device_context.home_assistant_entities:
            ...
            entity_name = (
                primary_entity.get("friendly_name")
                or primary_entity.get("name")
                or primary_entity.get("entity_id", "")
            )
            ...
            suggestion = DeviceSuggestion(
                ...
                home_assistant_entities=HomeAssistantEntities(
                    action_entities=[primary_entity.get("entity_id")]
                    if primary_entity.get("entity_id")
                    else [],
                ),
                home_assistant_services=HomeAssistantServices(
                    actions=[
        ...
        for suggestion in suggestions:
            ...
            suggestion.automation_preview.trigger = replace_entity_ids(
                suggestion.automation_preview.trigger
            )
            suggestion.automation_preview.action = replace_entity_ids(
                suggestion.automation_preview.action
            )
