...
class EnhancedContextBuilder:
    ...
    async def build_binary_sensor_context(self) -> str:
        ...
        try:
            ...
            parts.append(
                "(MANDATORY: Use ALL sensors listed for each area in trigger entity_id list)"
            )
            parts.append("")
            ...
            for area_id in sorted(sensor_by_area.keys()):
                ...
                for sensor in sensors:
                    parts.append(
                        f"  - {sensor['entity_id']} [{sensor['device_class']}] (state: {sensor['state']})"
                    )
                parts.append("")
...
"""
    ...
    async def build_enhanced_context(
        self, user_prompt: str | None = None, area_ids: list[str] | None = None
    ) -> str:
        """
