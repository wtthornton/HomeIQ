...
def _build_patterns() -> list[ThreatPattern]:
    ...
    def add(
        cat: ThreatCategory, sev: str, desc: str, regex: str, flags: int = re.IGNORECASE
    ) -> None:
        patterns.append(
            ThreatPattern(
                category=cat,
    ...
    add(
        ...
        "high",
        "Jailbreak attempt",
        r"(DAN|do anything now|no restrictions|no limitations|unrestricted mode)",
    )
    add(
        pi,
        "high",
        "Context manipulation",
        r"(begin new|start over|reset context|clear instructions)",
    )
    add(
        pi,
        "medium",
    ...
    add(
        ...
        "high",
        "Indirect injection",
        r"(when you see this|if you read this|execute the following)",
    )
    add(
        pi,
        "medium",
        "Token manipulation",
        r"(split across|character by character|spell out|one letter at a time)",
    )
    add(
        pi,
        "high",
        "Persona injection",
        r"(from now on|henceforth|going forward).{0,30}(you (will|must|should|are))",
    )
    add(
        pi,
        "high",
        "Developer/debug mode escalation",
        r"(developer mode|debug mode|god mode|admin mode|root mode)",
    )
    add(
        pi,
        "critical",
    ...
    add(
        pi,
        "high",
        "Instruction smuggled in HTML comment",
        r"<!--.{0,60}(ignore|instruction|system).{0,60}-->",
    )
    add(
        pi,
        "medium",
        "Escape-sequence obfuscation",
        r"((\\x[0-9a-fA-F]{2}){4,}|(\\u[0-9a-fA-F]{4}){4,})",
    )
    add(
        pi,
        "high",
    ...
    add(
        ...
        r"(shell_command\.|command_line\.|subprocess|os\.system)",
    )
    ...
    add(
        ha,
        "critical",
        "Hassio/addon management",
        r"(hassio\.|supervisor\.|addon\.|ha (core|os|supervisor))",
    )
    ...
    add(
        ha,
        "critical",
        "Eval/exec in templates",
        r"\{\{.*(__import__|eval|exec|compile|getattr).*\}\}",
    )
    ...
    add(
        ...
        "critical",
        "Credential theft",
        r"(password|secret|token|api.?key|credential|private.?key)",
        re.IGNORECASE,
    )
    ...
    add(
        ex,
        "critical",
        "File reading (.env, secrets)",
        r"(\.env|secrets\.yaml|configuration\.yaml|auth\.json)",
    )
    ...
    add(
        ex,
        "high",
        "Clipboard/keystroke capture",
        r"(pyperclip|xclip|pynput|keyboard\.(record|on_press))",
    )
    add(ex, "high", "Screen capture", r"(ImageGrab|screencapture|scrot\b|pyautogui\.screenshot)")
    add(
        ex,
        "high",
        "Email/SMTP data sending",
        r"(smtplib|sendmail\b|smtp\.[a-z0-9-]+\.[a-z]{2,}|mailto:)",
    )
    ...
    add(
        de,
        "high",
        "Bootloader/kernel tampering",
        r"(grub-install|update-grub|/boot/|modprobe\s+-r)",
    )
    ...
    add(
        ...
        r"(/etc/systemd/system|systemctl\s+(enable|daemon-reload))",
    )
    ...
    add(
        ...
        "critical",
        "SSH key injection",
        r"(ssh-rsa\s+AAAA|ssh-ed25519\s+AAAA|>>\s*\S*authorized_keys)",
    )
    add(
        pe,
        "high",
    ...
    add(
        pe,
        "critical",
        "Python import hook persistence",
        r"(sitecustomize|usercustomize|site-packages/\S*\.pth)",
    )
    add(
        pe,
        "critical",
    ...
    add(
        ...
        r"(nohup\b|setsid\b|disown\b|screen\s+-dm|tmux\s+new-session\s+-d)",
    )
    ...
    add(
        ...
        "critical",
        "Reverse shell",
        r"(nc\s+-e|ncat\s+--exec|bash\s+-i\s*>&|/dev/tcp/|socat\s+\S*exec)",
    )
    add(
        nw,
        "critical",
        "Bind shell / inbound listener",
        r"(nc\s+-l|socket\.bind|http\.server|SimpleHTTPServer)",
    )
    ...
    add(
        ...
        "high",
        "Tunneling service",
        r"(localtunnel|serveo\.net|cloudflared\s+tunnel|frp[cs]\s+-c)",
    )
    ...
    add(
        ...
        r"(chmod\s+[ug]?\+s|chmod\s+[24]\d{3}|find\s+\S*-perm\s+-[24]000)",
    )
    ...
    add(
        ...
        "critical",
        "Container escape",
        r"(--privileged\b|/var/run/docker\.sock|nsenter\b|docker\s+run\S*\s-v\s*/:)",
    )
    ...
    add(
        ...
        r"(/etc/shadow|/etc/passwd|lsass|security\s+dump-keychain)",
    )
    add(
        pv,
        "high",
        "Access-token harvesting",
        r"(HA_TOKEN|long.?lived.?access.?token|Authorization:\s*Bearer)",
    )
    add(
        pv,
        "critical",
        "Admin account creation/promotion",
        r"(is_admin\s*[:=]\s*true|auth_provider|user\.create)",
    )
    ...
    add(
        ...
        r"(pip3?\s+install|npm\s+install|yarn\s+add|apt-get\s+install)",
    )
    add(
        sc,
        "critical",
        "Install from URL/VCS",
        r"(pip3?\s+install\s+(git\+|https?://)|npm\s+install\s+https?://)",
    )
    add(
        sc,
        "critical",
        "Untrusted package index",
        r"(--index-url|--extra-index-url|--trusted-host|--registry\s)",
    )
    add(
        sc,
        "high",
    ...
    add(
        sc,
        "medium",
        "Dependency integrity override",
        r"(--force-reinstall|--ignore-installed|--no-deps)",
    )
    add(
        sc,
        "critical",
    ...
    add(
        sc,
        "critical",
        "Dynamic remote code import",
        r"(__import__\s*\(|importlib\.import_module|exec\s*\(\s*requests)",
    )
    add(
        sc,
        "critical",
        "Unsafe deserialization",
        r"(pickle\.loads?|marshal\.loads|yaml\.load\s*\((?!.*Loader))",
    )
    add(
        sc,
        "high",
