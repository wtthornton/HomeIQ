...
class ServicesSummaryService:
    ...
    async def get_summary(self, skip_truncation: bool = False) -> str:
        ...
        try:
            ...
            total_services = sum(
                len(svcs) for svcs in services_data.values() if isinstance(svcs, dict)
            )
            logger.info(
                f"📊 Processing {total_domains} domains with {total_services} total services"
            )
            ...
            for domain in sorted(services_data.keys()):
                ...
                if not isinstance(domain_services, dict):
                    logger.warning(
                        f"Domain {domain} services is not a dict: {type(domain_services)}"
                    )
                    continue
                ...
                for service_name in sorted(domain_services.keys()):
                    ...
                    if isinstance(service_info, dict):
                        ...
                        logger.debug(
                            f"Service {domain}.{service_name} info is not a dict: {type(service_info)}"
                        )
            ...
            max_length = (
                8000 if skip_truncation else 6000
            )  # Increased from 1500 to 6000 for all services
    ...
    def _format_full_parameter_schema(
        self, service_name: str, fields: dict[str, Any], description: str = ""
    ) -> str:
        """
        ...
        if required_params or optional_params:
            ...
            if required_params:
                param_str_parts.append(
                    f"required: {', '.join(required_params[:3])}"
                )  # Limit to 3 required
            if optional_params:
                param_str_parts.append(
                    f"optional: {', '.join(optional_params[:3])}"
                )  # Limit to 3 optional
    ...
    def _format_service_2025(self, domain: str, service: str, info: dict) -> str:
        ...
        target_selector = (
            target_fields.get("selector", {}) if isinstance(target_fields, dict) else {}
        )
        target_options = []
