...
class DevicesSummaryService:
    ...
    def __init__(self, settings: Settings, context_builder: ContextBuilder):
        ...
        self.ha_client = HomeAssistantClient(
            ha_url=settings.ha_url, access_token=settings.ha_token.get_secret_value()
        )
        ...
        self._cache_ttl = (
            1800  # 30 minutes (P1: Increased TTL for static data - devices rarely change)
        )
    ...
    async def get_summary(self, skip_truncation: bool = False) -> str:
        ...
        try:
            ...
            try:
                ...
                device_capabilities = defaultdict(
                    lambda: {"effects": set(), "color_modes": set(), "presets": set()}
                )
                ...
                for entity in entities:
                    ...
                    if device_id:
                        ...
                        if "max_brightness" in attributes:
                            device_capabilities[device_id]["max_brightness"] = attributes[
                                "max_brightness"
                            ]
            ...
            try:
                ...
                if zigbee_metadata_map:
                    logger.info(
                        "Fetched Zigbee2MQTT metadata for %d devices", len(zigbee_metadata_map)
                    )
            except Exception as e:
                logger.warning(
                    "Could not fetch Zigbee2MQTT metadata (ml-engine may be unavailable): %s", e
                )
                    ...
                    try:
                        ...
                        if context_result and context_result.get("context"):
                            ...
                            if isinstance(context, dict):
                                device_description = context.get(
                                    "device_description"
            ...
            # Sort areas by name
            sorted_areas = sorted(
                devices_by_area.keys(), key=lambda a: area_name_map.get(a, a).lower()
            )
            ...
            for area_id in sorted_areas:
                try:
                    ...
                    logger.debug(
                        f"📱 Formatting area {area_id} ({area_name}): {len(area_devices)} devices"
                    )
                    ...
                    device_limit = (
                        len(area_devices) if skip_truncation else min(20, len(area_devices))
                    )
                    ...
                    for device in area_devices[:device_limit]:
                        ...
                        if device["manufacturer"] != "Unknown" or device["model"] != "Unknown":
                            manufacturer_model = (
                                f"{device['manufacturer']} {device['model']}".strip()
                            )
                        ...
                        if device_id_for_caps in device_capabilities:
                            ...
                            if caps.get("color_modes"):
                                color_modes_list = list(caps["color_modes"])[
                                    :2
                                ]  # Show top 2 color modes
            ...
            if unassigned_devices:
                ...
                unassigned_limit = (
                    len(unassigned_devices) if skip_truncation else min(20, len(unassigned_devices))
                )
                ...
                if not skip_truncation and len(unassigned_devices) > 20:
                    unassigned_parts.append(
                        f"  ... and {len(unassigned_devices) - 20} more devices"
                    )
