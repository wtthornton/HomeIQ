...
class ContextPrioritizationService:
    ...
    def score_service_relevance(self, service: str, user_prompt: str) -> float:
        ...
        if domain in domain_keywords and any(
            keyword in prompt_lower for keyword in domain_keywords[domain]
        ):
            score += 0.3
        ...
        if action in action_keywords and any(
            keyword in prompt_lower for keyword in action_keywords[action]
        ):
            score += 0.2
    ...
    def prioritize_services(
        self, services: list[str], user_prompt: str, top_n: int = 15
    ) -> list[str]:
        """
