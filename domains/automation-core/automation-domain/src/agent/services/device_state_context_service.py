...
class DeviceStateContextService:
    ...
    async def get_state_context(
        ...
        try:
            ...
            state_map = {
                state.get("entity_id"): state for state in all_states if state.get("entity_id")
            }
            ...
            if len(context) > max_length:
                ...
                logger.info(
                    f"✅ Truncated state context from {len(state_lines)} to {len(truncated_lines)} entities"
                )
            ...
            logger.info(
                f"✅ Generated state context for {len(relevant_states)} entities ({len(context)} chars)"
            )
            return context
            ...
            logger.warning(
                f"⚠️ Failed to fetch device states: {e}. State context will be unavailable."
            )
            return ""
