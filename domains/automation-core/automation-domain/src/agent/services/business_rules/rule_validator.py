...
class BusinessRuleValidator:
    ...
    def check_safety_requirements(
        ...
        blocked_services_used = [
            s for s in services if any(s.startswith(bs) for bs in self.BLOCKED_SERVICES)
        ]
        ...
        if isinstance(trigger, list):
            has_time_trigger = any(
                t.get("platform") in ["time", "time_pattern", "sun", "calendar"] for t in trigger
            )
    ...
    def calculate_safety_score(
        ...
        if isinstance(trigger, list):
            ...
            has_time_trigger = trigger.get("platform") in [
                "time",
                "time_pattern",
                "sun",
                "calendar",
