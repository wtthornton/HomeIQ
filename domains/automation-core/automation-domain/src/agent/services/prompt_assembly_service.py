...
class PromptAssemblyService:
    ...
    async def assemble_messages(
        ...
        cached_prompt = (
            None
            if refresh_context
            else await self.conversation_service.get_cached_context(conversation_id)
        )
        if not cached_prompt:
            ...
            try:
                ...
                if "not initialized" in str(e):
                    ...
                    raise
                logger.error(
                    f"[Context Building] Conversation {conversation_id}: ❌ Error building context: {e}"
                )
                raise
            ...
            if entity_result.success and entity_result.matched_entities:
                ...
                if entity_result.success and entity_result.matched_entities:
                    try:
                        ...
                        if (
                            hasattr(self.context_builder, "_rag_registry")
                            and self.context_builder._rag_registry
                        ):
                            try:
                                rag_context = (
                                    await self.context_builder._rag_registry.get_merged_context(
                                        user_message
                                    )
                                )
        ...
        try:
            ...
            logger.warning(
                f"[Memory Context] Conversation {conversation_id}: Failed to inject memory context: {e}"
            )
...
Instructions: Process this request now. Use tools if needed. Do not respond with generic welcome messages."""
        ...
        if len(messages) < len(
            [{"role": "system", "content": system_prompt}, *emphasized_messages]
        ):
            logger.info(
    ...
    def _truncate_message_list(
        ...
        if first_user_idx >= 0 and first_user_idx < len(messages) - 10:
            ...
            truncated = (
                [first_user_msg, *recent_messages]
    ...
    def _build_hidden_context(self, hidden_context: dict[str, Any]) -> str:
        ...
        if hidden_context.get("constraint_mode") == "blueprint":
            ...
            lines.append(
                "- Suggest valid entity IDs that match the blueprint's domain/device_class requirements"
            )
