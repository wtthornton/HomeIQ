...
class ContextBuilder:
    ...
    async def initialize(self) -> None:
        ...
        self._entity_inventory_service = EntityInventoryService(
            settings=self.settings, context_builder=self
        )
    ...
    async def build_context(self, skip_truncation: bool = False) -> str:
        ...
        if not self._initialized:
            logger.error(
                "❌ CRITICAL: Context builder not initialized! Call context_builder.initialize() first."
            )
            raise RuntimeError("Context builder not initialized")

        logger.debug(
            f"Building Tier 1 context (devices, areas, services, etc.) - skip_truncation={skip_truncation}"
        )
        ...
        try:
            logger.debug(
                f"🔍 Checking devices summary service: {self._devices_summary_service is not None}"
            )
            if self._devices_summary_service is None:
                logger.error(
                    "❌ CRITICAL: _devices_summary_service is None! Service not initialized."
                )
                ...
                logger.debug(
                    f"📱 DevicesSummaryService type: {type(self._devices_summary_service).__name__}"
                )
                devices_summary = await self._devices_summary_service.get_summary(
                    skip_truncation=skip_truncation
                )
                logger.debug(
                    f"📱 Devices summary received: {len(devices_summary) if devices_summary else 0} chars"
                )
                if devices_summary and len(devices_summary.strip()) > 0:
                    context_parts.append(f"DEVICES:\n{devices_summary}\n")
                    logger.info(
                        f"✅ Devices summary added to context ({len(devices_summary)} chars)"
                    )
                else:
                    logger.warning(
                        "⚠️ Devices summary is empty - will show 'unavailable' in context"
                    )
        ...
        logger.debug(
            "⏭️ Skipping capability patterns (consolidated into entity inventory for token efficiency)"
        )
        ...
        logger.debug(
            "⏭️ Skipping entity attributes (merged into entity inventory for token efficiency)"
        )
        ...
        logger.debug(
            "⏭️ Skipping automation patterns in static context (added dynamically per-message)"
        )
        ...
        try:
            if hasattr(self, "_enhanced_context_builder") and self._enhanced_context_builder:
                ...
                if entity_inventory and "Unavailable" not in entity_inventory:
                    ...
                    logger.info(
                        f"✅ Added entity inventory context ({len(entity_inventory)} chars)"
                    )
                else:
        ...
        try:
            if hasattr(self, "_enhanced_context_builder") and self._enhanced_context_builder:
                ...
                binary_sensor_context = (
                    await self._enhanced_context_builder.build_binary_sensor_context()
                )
                ...
                automations_context = (
                    await self._enhanced_context_builder.build_existing_automations_context()
                )
    ...
    async def build_filtered_context(
        self, user_prompt: str | None = None, skip_truncation: bool = False
    ) -> str:
        """
    ...
    async def build_complete_system_prompt(self, skip_truncation: bool = False) -> str:
        ...
        """
        logger.debug(
            f"Building complete system prompt with context injection - skip_truncation={skip_truncation}"
        )
            ...
            async for session in get_session():
                ...
                if existing:
                    ...
                    cache_entry = ContextCache(
                        cache_key=cache_key, cache_value=cache_value, expires_at=expires_at
                    )
