...
class EntityInventoryService:
    ...
    async def get_summary(self, skip_truncation: bool = False) -> str:
        ...
        try:
            ...
            hue_room_devices: dict[
                str, dict[str, Any]
            ] = {}  # device_id → device_data (Hue Room/Zone groups)
            wled_master_entities: dict[str, str] = {}  # entity_id → master_entity_id (for segments)
            device_relationships: dict[
                str, list[str]
            ] = {}  # device_id/entity_id → related entities
            ...
            logger.info(
                f"🔍 Processing {len(device_ids_to_process)} devices with device mapping library..."
            )
            ...
            for entity in entities:
                ...
                device_metadata = (
                    device_metadata_map.get(entity_device_id, {}) if entity_device_id else {}
                )
                # Merge device metadata into entity data
                if device_metadata:
                    entity["manufacturer"] = entity.get("manufacturer") or device_metadata.get(
                        "manufacturer"
                    )
