...
class BasicValidationStrategy(ValidationStrategy):
    ...
    async def validate(self, automation_yaml: str) -> ValidationResult:
        ...
        try:
            ...
            if "description" not in automation_dict:
                warnings.append(
                    "Missing recommended field: 'description' (helps with automation management)"
                )
    ...
    async def _validate_entities(self, entities: list[str]) -> tuple[list[str], list[str]]:
        ...
        try:
            ...
            warnings.append(
                "Could not validate entity existence (Data API unavailable). Entity validation skipped."
            )
    ...
    def _detect_dimming_pattern_issues(self, automation_dict: dict[str, Any]) -> list[str]:
        ...
        is_dimming = (
            "dim" in description or "dim" in alias or "dimming" in description or "dimming" in alias
        )
            ...
            for cond in conditions:
                if isinstance(cond, dict) and cond.get("condition") == "and":
                    ...
                    if isinstance(and_conditions, list):
                        for_and_conditions = [
                            c for c in and_conditions if isinstance(c, dict) and "for" in c
                        ]
