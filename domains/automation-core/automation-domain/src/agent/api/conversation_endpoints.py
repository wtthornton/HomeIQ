...
# IMPORTANT: This route must come BEFORE /{conversation_id}/debug/prompt to avoid route conflicts
@router.get("/debug/{debug_id}/prompt")
async def get_prompt_breakdown_by_debug_id(
    debug_id: str,
    user_message: str | None = Query(
        None, description="Optional user message to include in breakdown"
    ),
    ...
    **Query Parameters:**
    ...
    try:
        ...
        conversation = await conversation_service.get_conversation_by_debug_id(debug_id)
        if not conversation:
            logger.warning(f"Conversation with debug_id {debug_id} not found in database")
            raise HTTPException(
        ...
        try:
            token_counts = await prompt_assembly_service.get_token_count(
        ...
        logger.exception(f"Error getting prompt breakdown for debug_id {debug_id}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Internal server error: {str(e)}",
        ) from e
    ...
    **Query Parameters:**
    ...
    try:
        ...
        if not conversation and len(conversation_id) == 36 and conversation_id.count("-") == 4:
            ...
            if conversation:
                logger.info(
                    f"Found conversation {conversation.conversation_id} by debug_id {conversation_id}"
                )
        ...
        if not debug_id:
            ...
            debug_id = str(uuid4())
            logger.warning(
                f"Conversation {conversation_id} missing debug_id, generated: {debug_id}"
            )
        ...
        if request.state is not None:
            try:
                ...
                await conversation_service.archive_conversation(
                    conversation_id
                ) if state == ConversationState.ARCHIVED else await (
                    conversation_service.activate_conversation(conversation_id)
                )
            except ValueError as e:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
