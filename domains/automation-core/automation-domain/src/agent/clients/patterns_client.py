...
class PatternsClient:
    ...
    def __init__(self, base_url: str | None = None, settings: Settings | None = None):
        ...
        self.api_key = (
            settings.ai_automation_api_key.get_secret_value()
            if settings.ai_automation_api_key
            else None
        )
    ...
    async def get_patterns(
        ...
        try:
            ...
            async with (
                ...
                session.get(
                    f"{self.base_url}/api/patterns/list", params=params, headers=headers
                ) as response,
            ):
                if response.status == 200:
                    ...
                    if device_ids:
                        ...
                        for pattern in patterns:
                            pattern_device_id = pattern.get("device_id") or pattern.get(
                                "pattern_metadata", {}
                            ).get("device_id")
