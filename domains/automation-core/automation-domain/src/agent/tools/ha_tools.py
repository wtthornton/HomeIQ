...
class HAToolHandler:
    ...
    async def _preview_with_hybrid_flow(
        ...
        try:
            ...
            request = AutomationPreviewRequest(
                ...
                alias=automation_dict.get(
                    "alias", human_summary.split("|")[0] if human_summary else "Automation"
                ),
                conversation_id=conversation_id,
            )
            ...
            if risk_notes:
                safety_warnings = [
                    note.get("message", "") for note in risk_notes if note.get("message")
                ]

            if safety_info.get("requires_confirmation"):
                safety_warnings.append(
                    "This automation requires confirmation due to safety classification"
                )
    ...
    async def preview_automation_from_prompt(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if (
            self.use_hybrid_flow
            and self.hybrid_flow_client
            and not arguments.get("automation_yaml")
        ):
            try:
                logger.info(
    ...
    def _build_preview_response(
        ...
        if (
            validation_result
            and hasattr(validation_result, "fixed_yaml")
            and validation_result.fixed_yaml
        ):
    ...
    def _extract_entities_from_yaml(self, automation_dict: dict[str, Any]) -> list[str]:
        ...
        entities.extend(
            self._extract_from_yaml(automation_dict, ["target", "entity_id"], sections=["action"])
        )
    ...
    def _validate_consistency(
        self, automation_dict: dict[str, Any], device_context: dict[str, Any]
    ) -> list[str]:
        """
    ...
    def enhancement_service(self) -> AutomationEnhancementService | None:
        ...
        if self._enhancement_service is None and self.chat_client:
            from ..config import Settings

            settings = Settings()
            self._enhancement_service = AutomationEnhancementService(
                chat_client=self.chat_client, settings=settings
            )
    ...
    async def suggest_automation_enhancements(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        conversation_id = arguments.get("conversation_id")
        creativity_level = arguments.get(
            "creativity_level", "balanced"
        )  # Optional, default to balanced
