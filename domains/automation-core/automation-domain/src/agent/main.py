...
- Chat turns answered by the AgentForge `assistant-chat` workflow (TAP-7275)
"""
...
chat_client: AgentChatClient | None = None
...
def _validate_llm_credentials() -> None:
    ...
    key = settings.agentforge_api_key
    if not key or not key.get_secret_value().strip():
        raise ValueError(
            "AGENTFORGE_API_KEY is required for the agent slice of automation-domain. "
...
async def _startup_services() -> None:
    ...
    global prompt_assembly_service, chat_client, group_health
    ...
    # TAP-7275: one AgentForge-backed client replaces the OpenAI client and the
    # provider router. There is no provider to route between any more -- the
    # model choice lives in each gene definition.
    cc = AgentChatClient(settings)
    logger.info("AgentForge chat client initialized (%s)", settings.agentforge_url)
    chat_client = cc
    ...
    if settings.enable_memory_extraction:
        try:
            ...
            if mem_ok:
                memory_client = mc
                memory_extractor = MemoryExtractor(mc, cc)
    ...
    logger.info("automation-domain agent slice started successfully")
...
# Composition
# ---------------------------------------------------------------------------
# This slice no longer builds its own app or lifespan. `src/main.py` registers
# `_startup_services` / `_shutdown_services` on the merged process lifespan and
# mounts the routers below. Keeping a second FastAPI app here would be a second
# source of truth for what the process actually serves.
