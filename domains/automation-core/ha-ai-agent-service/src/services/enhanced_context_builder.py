...
class EnhancedContextBuilder:
    ...
    async def build_area_entity_context(self, area_ids: list[str] | None = None) -> str:
        ...
        try:
            ...
            device_area_map = {d.get("id"): d.get("area_id") for d in devices if d.get("id") and d.get("area_id")}

            # Fetch areas for friendly names
            areas = await self.ha_client.get_area_registry()
            area_name_map = {a.get("area_id"): a.get("name", a.get("area_id", "")) for a in areas if a.get("area_id")}

            # Group entities by area and domain
            area_domain_entities: dict[str, dict[str, list[dict]]] = defaultdict(lambda: defaultdict(list))
            filtered_count = 0
            ...
            for area_id in sorted(area_domain_entities.keys()):
                ...
                sorted_domains = sorted(
                    domains.keys(), key=lambda d: (priority_domains.index(d) if d in priority_domains else 99, d)
                )
    ...
    async def build_binary_sensor_context(self) -> str:
        ...
        try:
            ...
            for entity in entities:
                ...
                if device_class not in ["motion", "presence", "occupancy", "door", "window"] and not any(
                    kw in entity_id.lower() for kw in ["motion", "presence", "occupancy", "fp2", "fp300"]
                ):
                    continue
            ...
            for area_id in sorted(sensor_by_area.keys()):
                ...
                for sensor in sensors:
                    parts.append(f"  - {sensor['entity_id']} [{sensor['device_class']}] (state: {sensor['state']})")
                parts.append("")
    ...
    async def build_existing_automations_context(self) -> str:
        ...
        try:
            ...
            entities = await self.data_api_client.fetch_entities(limit=10000)
            automations = [e for e in entities if e.get("domain") == "automation"]
            ...
            for auto in automations:
                ...
                if actual_state == "on":
                    ...
                    active_automations.append(
                        {"entity_id": entity_id, "friendly_name": friendly_name, "state": actual_state}
                    )
...
"""
    ...
    async def build_enhanced_context(self, user_prompt: str | None = None, area_ids: list[str] | None = None) -> str:
        """
