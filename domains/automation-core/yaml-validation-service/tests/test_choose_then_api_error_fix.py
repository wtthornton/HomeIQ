...
"""
    ...
    assert "sequence:" in normalized_yaml, "ERROR: 'sequence' key missing! Normalization failed."
    ...
    for i, choice in enumerate(choose_list):
        ...
        assert "sequence" in choice, (
            f"Choice {i} should have 'sequence' key (not 'then'). Choice content: {choice}"
        )
    ...
    assert len(then_fixes) > 0, (
        f"Expected fix log for 'then' → 'sequence' conversion. Fixes applied: {fixes_applied}"
    )
    ...
    print("   - Added 'sequence' key(s)")
    print("   - YAML structure is valid for Home Assistant API")
    print("   - This will NOT cause 'extra keys not allowed' error")
