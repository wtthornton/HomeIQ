...
class TestEventCounterTracking:
    ...
    async def test_event_counter_increment(self, correlator_instance):
        ...
        with patch.object(
            correlator_instance, "_get_power_at_time", new_callable=AsyncMock
        ) as mock_get_power:
    ...
    async def test_correlation_found_counter(self, correlator_instance):
        ...
        with patch.object(
            ...
            mock_get_power.side_effect = [
                2450.0,
                2510.0,  # +60W - above threshold
                2450.0,
                2455.0,  # +5W - below threshold
                1850.0,
                4350.0,  # +2500W - above threshold
                1000.0,
                1003.0,  # +3W - below threshold
                2150.0,
                2030.0,  # -120W - above threshold
            ]
