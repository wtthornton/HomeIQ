...
class TestConnectionFailures:
    ...
    async def test_authentication_failure(self):
        ...
        with patch(
            "src.influxdb_wrapper.InfluxDBWrapper.connect", side_effect=Exception("Unauthorized")
        ):
...
class TestQueryFailures:
    ...
    async def test_invalid_data_format(self, correlator_with_mock):
        ...
        mock_client.query = AsyncMock(return_value=[{"invalid": "data", "no_time": True}])
...
class TestDataValidation:
    ...
    async def test_invalid_datetime_format(self, correlator_with_mock):
        ...
        mock_client.query = AsyncMock(
            return_value=[{"time": "invalid-datetime", "entity_id": "switch.lamp", "_value": "on"}]
        )
...
class TestCacheFailures:
    ...
    async def test_cache_lookup_with_invalid_timestamp(self, correlator_with_mock):
        ...
        power = correlator._lookup_power_in_cache(datetime.now(UTC) + timedelta(hours=1), cache)
