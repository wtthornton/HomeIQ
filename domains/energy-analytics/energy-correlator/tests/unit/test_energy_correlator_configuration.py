...
class TestEnergyCorrelatorConfiguration:
    ...
    def test_custom_processing_interval(self):
        ...
        with patch.dict(
            os.environ, {"INFLUXDB_TOKEN": "test-token", "PROCESSING_INTERVAL": "300"}, clear=True
        ):
            service = EnergyCorrelatorService()
    ...
    def test_custom_lookback_minutes(self):
        ...
        with patch.dict(
            os.environ, {"INFLUXDB_TOKEN": "test-token", "LOOKBACK_MINUTES": "10"}, clear=True
        ):
            service = EnergyCorrelatorService()
    ...
    def test_custom_influxdb_url(self):
        ...
        with patch.dict(
            os.environ,
            {"INFLUXDB_TOKEN": "test-token", "INFLUXDB_URL": "http://custom-influxdb:9999"},
            clear=True,
        ):
            service = EnergyCorrelatorService()
...
class TestEnvironmentVariableParsing:
    ...
    def test_processing_interval_string_to_int(self):
        ...
        with patch.dict(
            os.environ, {"INFLUXDB_TOKEN": "test-token", "PROCESSING_INTERVAL": "120"}, clear=True
        ):
            service = EnergyCorrelatorService()
    ...
    def test_lookback_minutes_string_to_int(self):
        ...
        with patch.dict(
            os.environ, {"INFLUXDB_TOKEN": "test-token", "LOOKBACK_MINUTES": "15"}, clear=True
        ):
            service = EnergyCorrelatorService()
