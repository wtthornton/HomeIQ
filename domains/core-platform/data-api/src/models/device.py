...
class Device(Base):
    ...
    )  # Optional serial number (if available from integration)
    model_id = Column(
        String, nullable=True
    )  # Optional model ID (manufacturer identifier, may differ from model)

    # Source tracking
    config_entry_id = Column(String, index=True)  # Config entry ID (source tracking)
    via_device = Column(
        String, ForeignKey("devices.device_id"), index=True
    )  # Parent device (self-referential FK)

    # Phase 1.1: Device intelligence fields
    device_type = Column(
        String, index=True
    )  # Device classification: "fridge", "light", "sensor", "thermostat", etc.
    device_category = Column(
        String, index=True
    )  # Category: "appliance", "lighting", "security", "climate"
    ...
    parent_device = relationship(
        "Device", remote_side="Device.device_id", foreign_keys=[via_device], backref="child_devices"
    )
