...
class TeamPreferences(Base):
    ...
    user_id: Mapped[str] = mapped_column(
        String(50), primary_key=True, comment="User identifier (default: 'default')"
    )
    ...
    created_at: Mapped[datetime] = mapped_column(
        DateTime, server_default=func.now(), comment="Creation timestamp"
    )
    updated_at: Mapped[datetime] = mapped_column(
        DateTime, server_default=func.now(), onupdate=func.now(), comment="Last update timestamp"
    )
