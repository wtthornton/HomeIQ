...
class Entity(Base):
    ...
    name_by_user = Column(
        String
    )  # User-customized name (if set) - PRIORITY: Used first for display
    original_name = Column(String)  # Original name before user customization
    friendly_name = Column(
        String, index=True
    )  # Stored computed value: name_by_user or name or original_name
    ...
    supported_features = Column(
        Integer
    )  # Bitmask of supported features (from attributes.supported_features)
    capabilities = Column(JSONB)  # Parsed capabilities list (brightness, color, effect, etc.)
    available_services = Column(
        JSONB
    )  # List of available service calls for this entity (e.g., ["light.turn_on", "light.turn_off", "light.toggle"])
    ...
    device_class = Column(
        String, index=True
    )  # Device class (e.g., "motion", "door", "temperature")
    ...
    options = Column(
        JSONB
    )  # Entity-specific options/config (e.g., default brightness, preferred colors)
