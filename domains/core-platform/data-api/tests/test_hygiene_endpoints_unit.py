...
class TestPydanticModels:
    ...
    def test_hygiene_issue_response_minimal(self):
        issue = HygieneIssueResponse(issue_key="k", issue_type="t", severity="low", status="open")
...
class TestRequestDeviceIntelligence:
    ...
    async def test_404_raises_http_exception(self):
        mock_resp = _mock_httpx_response(404, json_data={"detail": "Issue not found"})
    ...
    async def test_500_raises_http_exception(self):
        mock_resp = _mock_httpx_response(500, json_data={"detail": "Internal error"})
    ...
    async def test_uses_configured_base_url(self):
        ...
        with (
            patch("src.hygiene_endpoints.httpx.AsyncClient") as mock_cls,
...
class TestListHygieneIssues:
    ...
    async def test_list_issues_no_filter(self):
        ...
        with patch(
            "src.hygiene_endpoints._request_device_intelligence", new_callable=AsyncMock
        ) as mock_req:
        ...
        mock_req.assert_called_once_with("GET", "/api/hygiene/issues", params={"limit": 100})
