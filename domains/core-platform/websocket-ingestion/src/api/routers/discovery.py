...
async def trigger_discovery(request: Request):
    ...
    try:
        ...
        if (
            connection_manager
            and connection_manager.client
            and hasattr(connection_manager.client, "websocket")
        ):
            ...
            logger.warning(
                "⚠️  Connection manager not available - device discovery will try HTTP API fallback"
            )

        try:
            logger.info("Calling discover_all()...")
            discovery_result = await service.connection_manager.discovery_service.discover_all(
                websocket=websocket, connection_manager=connection_manager, store=True
            )
            logger.info(
                f"Discovery completed: {len(discovery_result.get('devices', []))} devices, {len(discovery_result.get('entities', []))} entities"
            )
