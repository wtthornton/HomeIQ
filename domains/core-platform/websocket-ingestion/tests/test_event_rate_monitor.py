...
class TestEventRateMonitorInitialization:
    ...
    async def test_init_custom_window_size(self):
...
class TestEventRateMonitorEventRecording:
    ...
    async def test_record_events_by_type(self):
    ...
    async def test_record_events_by_entity(self):
    ...
    async def test_record_event_without_entity_id(self):
    ...
    async def test_record_event_with_unknown_type(self):
...
class TestEventRateMonitorRateCalculation:
    ...
    async def test_get_current_rate_single_minute(self):
    ...
    async def test_get_current_rate_five_minutes(self):
    ...
    async def test_get_current_rate_with_no_events(self):
    ...
    async def test_get_average_rate(self):
...
class TestEventRateMonitorStatistics:
    ...
    async def test_get_rate_statistics(self):
    ...
    async def test_get_rate_statistics_no_events(self):
    ...
    async def test_get_top_entities_with_limit(self):
...
class TestEventRateMonitorAlerts:
    ...
    async def test_get_rate_alerts_no_events(self):
    ...
    async def test_get_rate_alerts_high_rate(self):
    ...
    async def test_get_rate_alerts_no_events_recently(self):
        """Test alert generation for no recent events"""
        monitor = EventRateMonitor()

        # Record an event and manually set last_event_time to long ago
        await monitor.record_event({"event_type": "state_changed", "entity_id": "light.1"})
        monitor.last_event_time = datetime.now(UTC) - timedelta(minutes=10)
...
class TestEventRateMonitorTimestampCleaning:
    ...
    async def test_clean_old_timestamps(self):
        ...
        for ts in monitor.event_timestamps:
            time_diff = (datetime.now(UTC) - ts).total_seconds() / 60
...
class TestEventRateMonitorStatisticsReset:
    ...
    async def test_reset_statistics(self):
        ...
        # Reset statistics
        await monitor.reset_statistics()
...
class TestEventRateMonitorConcurrency:
    """Test concurrent access to the event rate monitor.

    record_event guards its state with `async with self.lock` (an asyncio
    Lock), so concurrency here means interleaved coroutines. The previous
    version drove this with threading.Thread and a sync inner function, which
    could not await the now-async record_event at all.
    """

    async def test_concurrent_event_recording(self):
        """Concurrent recorders must not lose or double-count events."""
        import asyncio

        monitor = EventRateMonitor()
        num_recorders = 10
        events_per_recorder = 10
        ...
        expected = num_recorders * events_per_recorder
        assert monitor.total_events == expected
        assert monitor.events_by_type["state_changed"] == expected
        assert monitor.events_by_entity["light.1"] == expected


class TestEventRateMonitorEdgeCases:
    """Test edge cases"""

    async def test_record_event_with_exception_in_data(self):
    ...
    async def test_large_event_volume(self):
...
class TestEventRateMonitorRateTrends:
    ...
    async def test_minute_rates_tracking(self):
    ...
    async def test_hour_rates_tracking(self):
