...
The Dockerfile uses `COPY domains/ml-engine/model-prep/requirements.txt .` and `COPY domains/ml-engine/model-prep/download_all_models.py .`, which assumes the build context is the repository root (`c:/cursor/HomeIQ/`). However, the README instructs building with the service directory as context:

```dockerfile
# Dockerfile line 26
COPY domains/ml-engine/model-prep/requirements.txt .

# Dockerfile line 45
COPY domains/ml-engine/model-prep/download_all_models.py .
```

**README (line 31-32) says:**
```bash
docker build -t homeiq-model-prep -f domains/ml-engine/model-prep/Dockerfile services/model-prep
```

This `docker build` command sets the context to `domains/ml-engine/model-prep/`, but the Dockerfile expects `domains/ml-engine/model-prep/` prefix in COPY paths -- which means the Dockerfile expects the **repo root** as context.

The README command will **fail** because inside `domains/ml-engine/model-prep/` context there is no `domains/ml-engine/model-prep/` subdirectory.
...
```bash
docker build -t homeiq-model-prep -f domains/ml-engine/model-prep/Dockerfile .
```
...
Option A is recommended since the Dockerfile already uses the `domains/ml-engine/model-prep/` prefix and docker-compose typically uses repo root as context.
