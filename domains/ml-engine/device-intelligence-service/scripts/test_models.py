...
async def test_production_model():
    ...
    try:
        ...
        logger.info(
            f"   Sample count: {engine.model_metadata.get('training_data_stats', {}).get('sample_count', 'unknown')}"
        )
        ...
        try:
            ...
            logger.info("   Model is functional")
        except Exception as pred_error:
            logger.warning(f"⚠️  Could not make prediction (model may still be valid): {pred_error}")
            logger.info("✅ Model loaded and ready (prediction test skipped)")
        ...
        try:
            ...
            if metadata.exists():
                with Path(metadata).open() as f:
                    ...
                    logger.info(
                        f"   Accuracy: {meta.get('model_performance', {}).get('accuracy', 0):.3f}"
                    )
