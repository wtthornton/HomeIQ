        ...
        if "randomforest" in results and results["randomforest"]["status"] == "success":
            ...
            for model_type, result in results.items():
                if result["status"] == "success" and model_type != "randomforest":
                    ...
                    logger.info(
                        f"   Accuracy: {acc_diff:+.3f} ({'+' if acc_diff > 0 else ''}{acc_diff / baseline_acc * 100:.1f}%)"
                    )
...
def main():
    ...
    exit_code = asyncio.run(compare_models(days_back=args.days_back, output_file=args.output))
