...
async def train_home_type_model(
    home_type: str, data_file: Path, models_dir: Path, days_back: int = 180
) -> dict[str, Any]:
    """
...
async def train_all_home_type_models(
    data_file: Path, output_dir: Path | None = None, days_back: int = 180
    ...
    for home_type in HOME_TYPES:
        performance = await train_home_type_model(
            home_type=home_type, data_file=data_file, models_dir=output_dir, days_back=days_back
        )
...
def compare_home_type_models(results: dict[str, dict[str, Any]]) -> None:
    ...
    logger.info(
        f"{'Home Type':<20} {'Samples':<10} {'Accuracy':<10} {'Precision':<10} {'Recall':<10} {'F1':<10}"
    )
    logger.info("-" * 80)

    for home_type, perf in sorted(
        results.items(), key=lambda x: x[1]["model_performance"].get("accuracy", 0), reverse=True
    ):
    ...
    # Calculate averages
    avg_accuracy = sum(r["model_performance"].get("accuracy", 0) for r in results.values()) / len(
        results
    )
    avg_precision = sum(r["model_performance"].get("precision", 0) for r in results.values()) / len(
        results
    )
    avg_recall = sum(r["model_performance"].get("recall", 0) for r in results.values()) / len(
        results
    )
    ...
    logger.info(
        f"Worst performer: {worst[0]} (Accuracy: {worst[1]['model_performance'].get('accuracy', 0):.3f})"
    )
...
async def main():
    ...
    try:
        results = await train_all_home_type_models(
            data_file=data_file, output_dir=Path(args.output_dir), days_back=args.days_back
        )
