...
async def get_failure_predictions(
    ...
    try:
        ...
        for prediction in predictions:
            ...
            if min_probability <= probability <= max_probability and (
                risk_level is None or prediction.get("risk_level") == risk_level
            ):
...
async def compare_models(
    ...
    try:
        ...
        backup_timestamp = (
            backup_path.name.split("backup_")[-1] if "backup_" in backup_path.name else None
        )
        ...
        if backup_metadata:
            ...
            for metric in ["accuracy", "precision", "recall", "f1_score"]:
                ...
                if current_val and backup_val:
                    ...
                    comparison["performance"]["changes"][metric] = {
                        ...
                        "percent_change": round((diff / backup_val) * 100, 2)
                        if backup_val > 0
                        else 0,
                        "improved": diff > 0,
                    }
            ...
            comparison["training_data"] = {"current": current_stats, "previous": backup_stats}
...
async def predict_device_failure(
    ...
    try:
        prediction = await analytics_engine.predict_device_failure(
            request.device_id, request.metrics
        )
        ...
        logger.error(
            "Error predicting failure for device %s: %s", request.device_id, e, exc_info=True
        )
        raise HTTPException(status_code=500, detail="Internal server error") from e
...
async def incremental_update(
    ...
    try:
        ...
        background_tasks.add_task(analytics_engine.incremental_update, request.new_data)
...
async def trigger_training_now(request: Request, mode: str | None = None):
    """
...
async def get_training_status(request: Request):
    ...
    try:
        # Get scheduler from app state
        if not hasattr(request.app.state, "training_scheduler"):
            return {"enabled": False, "message": "Training scheduler not available"}
...
async def get_model_version(engine: PredictiveAnalyticsEngine = Depends(get_analytics_engine)):
    """
    ...
    try:
        if not engine.is_trained:
            raise HTTPException(status_code=404, detail="Models not trained or loaded")
...
async def get_model_health(engine: PredictiveAnalyticsEngine = Depends(get_analytics_engine)):
    """
