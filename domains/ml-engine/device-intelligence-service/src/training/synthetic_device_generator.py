...
class SyntheticDeviceGenerator:
    ...
    def __init__(self, random_seed: int | None = None, home_type: str | None = None):
        ...
        logger.info(
            f"SyntheticDeviceGenerator initialized (template-based generation, home_type={home_type or 'default'})"
        )
    ...
    def generate_training_data(
        ...
        logger.info(
            f"Generating {count} synthetic device samples (template-based, {days} days, home_type={effective_home_type or 'default'}, reference_date={reference_date.date()})"
        )
        ...
        if effective_home_type and effective_home_type in self.HOME_TYPE_DEVICE_PATTERNS:
            device_types = self._adjust_device_types_for_home_type(
                device_types, effective_home_type
            )
    ...
    def _adjust_device_types_for_home_type(
        self, device_types: list[str], home_type: str
    ) -> list[str]:
        ...
        if "climate_ratio" in patterns:
            ...
            adjusted_distribution["climate"] = int(
                adjusted_distribution["climate"] * (1 + patterns["climate_ratio"])
            )

        if "security_ratio" in patterns:
            adjusted_distribution["security"] = int(
                adjusted_distribution["security"] * (1 + patterns["security_ratio"])
            )

        if "media_ratio" in patterns:
            adjusted_distribution["media"] = int(
                adjusted_distribution["media"] * (1 + patterns["media_ratio"])
            )

        if "vacuum_ratio" in patterns:
            adjusted_distribution["vacuum"] = int(
                adjusted_distribution["vacuum"] * (1 + patterns["vacuum_ratio"])
            )
    ...
    def _generate_device_sample(
        ...
        error_rate = self._generate_realistic_value(
            base_chars["error_rate"], pattern="stable", days=days, reference_date=reference_date
        )
        ...
        humidity = self._generate_realistic_value(
            base_chars["humidity"], pattern="seasonal", days=days, reference_date=reference_date
        )
        ...
        restart_count = self._generate_realistic_value(
            base_chars["restart_count"], pattern="stable", days=days, reference_date=reference_date
        )
        ...
        if failure_scenario:
            ...
            if failure_params["battery_depletion_rate"] > 0:
                battery_level = max(
                    0, battery_level - (failure_params["battery_depletion_rate"] * days * 100)
                )
