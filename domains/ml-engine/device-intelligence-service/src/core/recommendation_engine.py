...
class RecommendationEngine:
    ...
    async def generate_recommendations(
        ...
        try:
            ...
            recommendations.extend(
                await self._generate_energy_recommendations(
            ...
            recommendations.extend(
                await self._generate_configuration_recommendations(
                    device_id, health_score, device_metrics, historical_metrics
                )
            )
            recommendations.extend(
                await self._generate_usage_pattern_recommendations(
                    device_id, health_score, device_metrics, historical_metrics
                )
            )
            ...
            logger.info(
                "Generated %d recommendations for device %s", len(recommendations), device_id
            )
    ...
    async def _generate_configuration_recommendations(
        ...
        if (
            cpu_usage > self.thresholds["high_cpu_usage"]
            or memory_usage > self.thresholds["high_memory_usage"]
        ):
            recommendations.append(
                OptimizationRecommendation(
                    device_id=device_id,
