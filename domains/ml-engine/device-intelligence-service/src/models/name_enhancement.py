...
class NameSuggestion(Base):
    ...
    entity_id: Mapped[str | None] = mapped_column(
        String,
        ForeignKey("device_entities.entity_id", ondelete="CASCADE"),
        index=True,
        nullable=True,
    )
    ...
    suggestion_source: Mapped[str] = mapped_column(
        String
    )  # "pattern", "ai", "local_llm", "preference"
...
class NamePreference(Base):
    ...
    pattern_type: Mapped[str] = mapped_column(
        String, index=True
    )  # "naming_style", "modifier", "area_convention"
    pattern_data: Mapped[dict[str, Any] | None] = mapped_column(
        Text
    )  # JSON string (simple structure)
    ...
    # Indexes
    __table_args__ = (Index("idx_name_preferences_type", "pattern_type"),)
