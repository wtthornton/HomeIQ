...
class Device(Base):
    ...
    battery_low: Mapped[bool | None] = mapped_column(
        Boolean, index=True
    ...
    entry_type: Mapped[str | None] = mapped_column(
        String
    )  # Entry type (service, config_entry, etc.)
    ...
    health_metrics: Mapped[list["DeviceHealthMetric"]] = relationship(
        "DeviceHealthMetric", back_populates="device", cascade="all, delete-orphan"
    )
...
class DeviceRelationship(Base):
    ...
    source_device_id: Mapped[str] = mapped_column(
        String, ForeignKey("devices.id", ondelete="CASCADE"), index=True
    )
    ...
    source_device: Mapped["Device"] = relationship(
        "Device", foreign_keys=[source_device_id], back_populates="relationships"
    )
    target_device: Mapped["Device"] = relationship(
        "Device", foreign_keys=[target_device_id], back_populates="target_relationships"
    )
...
class DeviceHygieneIssue(Base):
    ...
    device_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("devices.id", ondelete="SET NULL"), index=True
    )
    entity_id: Mapped[str | None] = mapped_column(
        String, ForeignKey("device_entities.entity_id", ondelete="SET NULL"), index=True
    )
...
class TeamTrackerIntegration(Base):
    ...
    installation_status: Mapped[str] = mapped_column(
        String, default="not_installed"
    )  # not_installed, detected, configured
...
class TeamTrackerTeam(Base):
    ...
    configured_in_ha: Mapped[bool] = mapped_column(
        Boolean, default=False
    )  # Is this team configured in HA?
    last_detected: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True)
    )  # Last time entity was seen
