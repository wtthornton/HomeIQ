...
class SyntheticCorrelationEngine:
    ...
    def validate_weather_hvac_correlation(
        self, weather_data: list[dict[str, Any]], hvac_events: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
        ...
        for weather_point in weather_data:
            ...
            if temperature > self.AC_THRESHOLD:
                ...
                if not ac_on and hvac_events_at_time:
                    violations.append(f"High temp {temperature}°C but AC not on at {timestamp}")
                elif ac_on:
            ...
            # Rule 2: Low temperature → Heat on
                ...
                if not heat_on and hvac_events_at_time:
                    violations.append(f"Low temp {temperature}°C but heat not on at {timestamp}")
                elif heat_on:
                    correlations.append(1.0)
                else:
                    correlations.append(0.5)  # Partial correlation (no HVAC device)
    ...
    def validate_energy_correlation(
        ...
        for carbon_point in carbon_data:
            ...
            if (
                carbon_intensity < self.LOW_CARBON_THRESHOLD
                and price_per_kwh < baseline_price * self.LOW_PRICE_MULTIPLIER
            ):
    ...
    def validate_all_correlations(
        ...
        if devices:
            ...
            hvac_events = [e for e in device_events if e.get("entity_id") in hvac_entity_ids]
        ...
        if devices:
            energy_entity_ids = {
                ...
                for d in devices
                if (
                    d.get("device_type") in ("climate", "water_heater", "switch")
                    or "ev" in d.get("device_type", "").lower()
                    or "vehicle" in d.get("entity_id", "").lower()
                )
            }
            energy_events = [e for e in device_events if e.get("entity_id") in energy_entity_ids]
        ...
        if weather_data and hvac_events:
            results["weather_hvac"] = self.validate_weather_hvac_correlation(
                weather_data=weather_data, hvac_events=hvac_events
            )
        ...
        if carbon_data and pricing_data and energy_events:
            results["energy"] = self.validate_energy_correlation(
                carbon_data=carbon_data, pricing_data=pricing_data, energy_events=energy_events
            )
        ...
        if calendar_data:
            results["calendar_presence"] = self.validate_calendar_presence_correlation(
                calendar_data=calendar_data, device_events=device_events, devices=devices
            )
    ...
    def validate_calendar_presence_correlation(
        ...
        if devices:
            for device in devices:
                ...
                # Security devices
                if (
                    device_type in ("alarm_control_panel", "lock", "binary_sensor")
                ...
                if (
                    device_type in ("climate", "thermostat")
        ...
        for calendar_event in calendar_data:
            ...
            if presence_state == "away":
                ...
                if security_entity_ids and not security_on:
                    violations.append(f"Away at {timestamp} but security not on")
                    correlations.append(0.7)  # Partial correlation
                elif lights_on:
                    violations.append(f"Away at {timestamp} but lights are on")
            ...
            # Rule 3: Work → Reduced device activity
                ...
                active_devices = sum(
                    ...
                    for event in device_events_at_time
                    if (
                        event.get("entity_id") not in security_entity_ids
