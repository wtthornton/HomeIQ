...
class SyntheticDeviceGenerator:
    ...
    def generate_devices(
        self, home_data: dict[str, Any], areas: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """
        ...
        if device_categories:
            devices = self._generate_from_categories(
                device_categories, areas, home_type, size_category
            )
        else:
            # Generate devices based on home type, areas, and size
            devices = self._generate_from_template(home_type, areas, size_category)
    ...
    def _generate_from_template(
        self, _home_type: str, areas: list[dict[str, Any]], size_category: str
    ) -> list[dict[str, Any]]:
        """
        ...
        size_multipliers = {"small": 0.6, "medium": 1.0, "large": 1.5, "extra_large": 2.0}
        multiplier = size_multipliers.get(size_category, 1.0)

        # Base device counts per category
        base_counts = {"security": 5, "climate": 3, "lighting": 8, "appliances": 4, "monitoring": 3}
