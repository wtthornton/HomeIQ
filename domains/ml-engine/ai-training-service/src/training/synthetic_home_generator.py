...
class SyntheticHomeGenerator:
        ...
        if enable_openai_enhancement:
            if not openai_client:
                try:
                    ...
                    logger.warning("⚠️ OpenAI enhancement requires OpenAI client configuration")
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Failed to initialize OpenAI generator: {e}")
                    self.enable_openai_enhancement = False
        else:
    ...
    def generate_homes(
        ...
        if home_types:
            ...
            for _i, ht in enumerate(home_types[:remainder]):
