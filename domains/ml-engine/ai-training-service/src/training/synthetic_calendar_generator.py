...
class SyntheticCalendarGenerator:
    ...
    def _get_work_schedule(self, home: dict[str, Any]) -> str:
        """
    ...
    def generate_calendar(
        self, home: dict[str, Any], start_date: datetime, days: int
    ) -> list[dict[str, Any]]:
        """
    ...
    def _generate_work_schedule_events(
        self, work_schedule: str, current_date: datetime, days: int
    ) -> list[dict[str, Any]]:
        """
        Generate work schedule events (Story 34.6).
        ...
        for day_offset in range(days):
            ...
            if work_schedule == "shift_work" and start_hour > end_hour:
                ...
                work_start = date.replace(
                    hour=start_hour, minute=random.randint(0, 15), second=0, microsecond=0
                )
                work_end = date.replace(
                    hour=end_hour, minute=random.randint(0, 30), second=0, microsecond=0
                )
    ...
    def _generate_routine_events(self, current_date: datetime, days: int) -> list[dict[str, Any]]:
        """
        Generate routine events (Story 34.7).
        ...
        for day_offset in range(days):
            ...
            if day_of_week == 0:  # Monday - Grocery shopping
                grocery_time = date.replace(
                    hour=random.randint(14, 16), minute=random.randint(0, 30)
                )
                events.append(
                    {
    ...
    def calculate_presence_patterns(
        self, calendar_events: list[dict[str, Any]], start_date: datetime, days: int
    ) -> list[dict[str, Any]]:
        """
    ...
    def correlate_with_devices(
        ...
        # Create presence map
        {p["timestamp"]: p["presence_state"] for p in presence_states}
