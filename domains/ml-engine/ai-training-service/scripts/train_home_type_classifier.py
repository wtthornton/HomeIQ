...
try:
    ...
    logger.warning(
        f"FineTunedHomeTypeClassifier not available: {e}. Script may not work until dependency is migrated."
    )
...
async def main():
    ...
    if not DEPENDENCIES_AVAILABLE:
        ...
        logger.error(
            "This will be migrated in future stories or made available via shared modules."
        )
        return 1
    ...
    results = await classifier.train_from_synthetic_data(
        synthetic_homes=synthetic_homes, test_size=args.test_size
    )
