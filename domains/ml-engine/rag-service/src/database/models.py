...
class RAGKnowledge(Base):
    ...
    embedding = Column(
        JSON, nullable=False
    )  # 1024-dim BGE-M3-base embedding array (stored as JSON)
    knowledge_type = Column(
        String, nullable=False
    )  # 'query', 'pattern', 'blueprint', 'automation', etc.
    metadata_json = Column(
        "metadata", JSON, nullable=True
    )  # Flexible metadata (device_id, area_id, confidence, etc.) - mapped to 'metadata' column
    success_score = Column(
        Float, default=0.5, nullable=False
    )  # 0.0-1.0 (learned from user feedback)
