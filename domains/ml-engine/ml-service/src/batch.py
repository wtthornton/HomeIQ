...
async def process_cluster_op(
    ...
    if not clustering_manager:
        return BatchOperationResult(
            type="cluster", status="error", error="Clustering service not ready"
        )
    ...
    if algorithm == "kmeans":
        ...
        return BatchOperationResult(
            type="cluster", status="error", error=f"Unknown clustering algorithm: {algorithm}"
        )
    return BatchOperationResult(
        type="cluster", status="success", algorithm=algorithm, labels=labels, n_clusters=n_clusters
    )
...
async def process_anomaly_op(
    ...
    if not anomaly_manager:
        return BatchOperationResult(
            type="anomaly", status="error", error="Anomaly detection service not ready"
        )
...
async def process_single_operation(
    ...
    try:
        if operation.type == "cluster":
            return await process_cluster_op(
                operation.data, clustering_manager, run_cpu_bound, max_clusters
            )
        ...
        return BatchOperationResult(
            type=operation.type, status="error", error="Operation failed due to an internal error."
        )
