...
def mock_manager():
    ...
    manager.generate_embeddings = AsyncMock(return_value=np.array([[0.1, 0.2, 0.3]]))
    manager.rerank = AsyncMock(return_value=[{"description": "top", "score": 0.95}])
    manager.classify_pattern = AsyncMock(return_value={"category": "automation", "priority": "medium"})
...
def client(mock_manager) -> TestClient:
    ...
    TAP-7276: /health and /ready now also aggregate the merged process's ml
    managers, so they're stubbed ready here too -- this suite only exercises
    the openvino routes.
    """
    import src.main as main_module

    original_ov = main_module.openvino_manager
    original_cm = main_module.clustering_manager
    original_am = main_module.anomaly_manager
    ...
    try:
        ...
        main_module.openvino_manager = original_ov
        main_module.clustering_manager = original_cm
        main_module.anomaly_manager = original_am
...
class TestHealthEndpoint:
    ...
    def test_health_healthy(self, client: TestClient) -> None:
        ...
        assert data["dependencies"]["openvino-models"] == "ok"
    ...
    def test_not_ready_returns_503(self) -> None:
        ...
        # TAP-7276: /health now aggregates all three former services'
        # managers; pin all of them down so "unhealthy" (every check down)
        # is deterministic regardless of what ran before this test.
