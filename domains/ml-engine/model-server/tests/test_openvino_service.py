...
def wired_manager(tmp_path, monkeypatch):
    ...
    # TAP-7276: /health now also aggregates the merged process's ml managers;
    # this suite only exercises the openvino routes, so stub them ready.
    monkeypatch.setattr(service_main, "clustering_manager", MagicMock())
    monkeypatch.setattr(service_main, "anomaly_manager", MagicMock())
    return manager
...
async def test_health_endpoint_reports_ready():
    ...
    # TAP-7276: unified StandardHealthCheck reports the merged service identity.
    assert result["service"] == "model-server"
    assert result["status"] == "healthy"
    assert result["dependencies"]["openvino-models"] == "ok"
...
async def test_readiness_endpoint_no_manager_returns_503(monkeypatch):
    ...
    monkeypatch.setattr(service_main, "openvino_manager", None)
    # TAP-7276: /health now aggregates all three former services' managers;
    # pin the other two None as well so this test asserts "unhealthy" (every
    # check down), not "degraded" (mixed), regardless of what ran before it.
    monkeypatch.setattr(service_main, "clustering_manager", None)
    monkeypatch.setattr(service_main, "anomaly_manager", None)
