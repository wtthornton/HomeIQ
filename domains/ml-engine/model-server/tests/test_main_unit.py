...
class TestRateLimiting:
    ...
    def test_rate_limit_blocks_excess(self):
        ...
        with patch("src.ml.middleware.RATE_LIMIT_MAX_REQUESTS", 3):
    ...
    def test_rate_limit_per_ip(self):
        ...
        with patch("src.ml.middleware.RATE_LIMIT_MAX_REQUESTS", 1):
            assert _check_rate_limit("10.0.0.1") is True
...
class TestAPIEndpoints:
    ...
    async def test_health_endpoint(self):
        ...
        old_ov = main_module.openvino_manager
        # TAP-7276: /health now aggregates the shared OpenVINO model's
        # readiness too; stub it ready so this test stays scoped to ml.
        ...
        try:
            ...
            async with AsyncClient(transport=transport, base_url="http://test") as client:
                ...
                # unified StandardHealthCheck reports the merged service
                # identity, not the former ml-service; algorithm availability
                # moved to its own /algorithms/status endpoint.
                assert data["service"] == "model-server"
        finally:
            main_module.openvino_manager = old_ov
    ...
    async def test_ready_endpoint_initialized(self):
        ...
        try:
            ...
            async with AsyncClient(transport=transport, base_url="http://test") as client:
                response = await client.get("/ready")
                assert response.status_code == 200
                # TAP-7276: unified StandardHealthCheck's /ready payload has a
                # boolean "ready" key, not the former ml-service's "status".
    ...
    async def test_anomaly_detection_endpoint(self):
        ...
        mock_manager = MagicMock()
        mock_manager.detect_anomalies = MagicMock(return_value=([1, 1, -1, 1], [0.1, 0.2, -0.5, 0.15]))
        import src.main as main_module
...
class TestClusteringManager:
    ...
    def test_kmeans_basic(self):
        ...
        from src.ml.algorithms.clustering import ClusteringManager
...
class TestAnomalyDetectionManager:
    ...
    def test_anomaly_detection_basic(self):
        ...
        from src.ml.algorithms.anomaly_detection import AnomalyDetectionManager
