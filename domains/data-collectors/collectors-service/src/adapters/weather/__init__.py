"""Weather adapter (former weather-api service) — Open-Meteo integration.
...
Open-Meteo's non-commercial tier requires no API key, so this adapter holds no
credential. It is queried by coordinate rather than city name.

Former routes (unchanged): GET /current-weather, GET /cache/stats.
"""
...
# Mapped to the (condition, description) pair this adapter has always exposed,
...
class WeatherService:
    ...
    async def startup(self) -> None:
        ...
        logger.info("Initializing Weather adapter...")
    ...
    async def shutdown(self) -> None:
        """Shutdown adapter, stopping background task and closing connections."""
        logger.info("Shutting down Weather adapter...")
...
@router.get("/current-weather", response_model=WeatherResponse)
...
class WeatherAdapter(CollectorAdapter):
    """Adapter wrapping the former weather-api service."""
