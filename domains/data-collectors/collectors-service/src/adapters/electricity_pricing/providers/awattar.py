...
from .exceptions import ProviderAPIError, ProviderParseError
...
class AwattarProvider:
    ...
    async def fetch_pricing(self, session: aiohttp.ClientSession) -> dict[str, Any] | None:
        ...
        params = {"start": int(datetime.now(UTC).timestamp() * 1000)}
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                async with session.get(self.BASE_URL, params=params) as response:
                    if response.status == 200:
                        ...
                        raise ProviderAPIError(
                            response.status, f"Awattar API returned status {response.status}"
                        )
                ...
                if attempt < self.MAX_RETRIES - 1:
                    wait = 2**attempt
                    logger.warning(
                        f"Awattar fetch attempt {attempt + 1} failed: {e}, retrying in {wait}s"
                    )
        ...
        for entry in market_data:
            if not required_fields.issubset(entry.keys()):
                raise ProviderParseError(
                    f"Missing fields in Awattar response: {required_fields - entry.keys()}"
                )
            ...
            forecast_24h.append(
                {
                    ...
                    "timestamp": datetime.fromtimestamp(entry["start_timestamp"] / 1000, tz=UTC),
                }
            )
