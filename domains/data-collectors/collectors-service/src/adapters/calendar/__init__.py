"""Calendar adapter (former calendar-service) — HA occupancy prediction.
...
Former routes (unchanged): GET /api/v1/prediction, GET /api/v1/events.
"""
...
class CalendarService:
    ...
    async def startup(self) -> None:
        ...
        logger.info("Initializing Calendar adapter (Home Assistant Integration)...")
    ...
    async def shutdown(self) -> None:
        ...
        logger.info("Shutting down Calendar adapter...")
...
async def get_events() -> dict[str, Any]:
    ...
    if _service is None:
        return {"error": "Adapter not initialized", "events": []}
...
class CalendarAdapter(CollectorAdapter):
    """Adapter wrapping the former calendar-service."""
    ...
    async def startup(self) -> None:
        ...
        logger.info("Calendar adapter started with background prediction loop")

    async def shutdown(self) -> None:
        global _bg_task, _service
        if _bg_task:
            _bg_task.cancel()
        ...
        if _service:
            ...
            _service = None
        logger.info("Calendar adapter shut down")

    def register_health(self, health: StandardHealthCheck) -> None:
        health.register_check(f"{NAME}_ha-connection", _check_ha_connected)
        health.register_check(f"{NAME}_calendars", _check_calendars)
        health.register_check(f"{NAME}_recent-fetch", _check_recent_fetch)
