...
class TestLocationFetching:
    ...
    @pytest.mark.asyncio
    async def test_fetch_location_from_ha_success(
        self, service_instance, mock_aiohttp_session, sample_ha_config_response
    ):
...
class TestDataFetching:
    ...
    @pytest.mark.asyncio
    async def test_fetch_aqi_error_returns_cached(
        self, service_instance, mock_aiohttp_session, sample_aqi_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_aqi_empty_response(
        self, service_instance, mock_aiohttp_session, sample_aqi_data
    ):
...
class TestInfluxDBStorage:
    ...
    async def test_store_handles_error(
        self, service_instance, sample_aqi_data, mock_influxdb_client
    ):
...
class TestAPIEndpoints:
    ...
    def client(self, service_instance, monkeypatch):
        ...
        from src.adapters import air_quality

        monkeypatch.setattr(air_quality, "service", service_instance)
