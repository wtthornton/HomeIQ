...
@router.get("")
async def list_schedules(home_id: str = settings.home_id) -> dict[str, Any]:
    """
    ...
    if not SCHEDULER_AVAILABLE:
        raise HTTPException(status_code=503, detail="Scheduler not available (Huey not configured)")
...
async def enable_schedule(spec_id: str, home_id: str = settings.home_id) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if not schedule_trigger:
            raise HTTPException(
                status_code=400, detail=f"Spec {spec_id} does not have a schedule trigger"
            )

        cron_expr = schedule_trigger.get("cron")
        if not cron_expr:
            raise HTTPException(
                status_code=400, detail=f"Schedule trigger for {spec_id} missing cron expression"
            )

        # Register schedule
        job_id = scheduler.register_scheduled_automation(
            spec_id=spec_id, home_id=home_id, cron_expr=cron_expr
        )
...
async def disable_schedule(spec_id: str) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail=f"Schedule for spec {spec_id} not found")

        return {"success": True, "spec_id": spec_id, "status": "disabled"}
...
async def get_next_run(spec_id: str, home_id: str = settings.home_id) -> dict[str, Any]:
    """
