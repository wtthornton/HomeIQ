        ...
        if "data" in data and "synergies" in data["data"]:
            ...
            if synergies:
                ...
                for synergy in synergies:
                    ...
                    assert (
                        "action_entity" in metadata
                        or "trigger_entity" in metadata
                        or "relationship" in metadata
                    ...
                    assert "confidence" in synergy, (
                        "Synergy should have confidence score from raw data analysis"
                    )
                    assert "impact_score" in synergy, (
                        "Synergy should have impact score from raw data analysis"
                    )

                    # Confidence should be > 0 (indicates data was analyzed)
                    assert synergy["confidence"] > 0, (
                        "Synergy confidence should be > 0 (indicates raw data was analyzed)"
                    )
        ...
        if "data" in data and "synergies" in data["data"]:
            ...
            if synergies:
                ...
                synergies_with_context = [
                    s for s in synergies if s.get("context_breakdown") is not None
                ]
                ...
                for synergy in synergies:
                    ...
                    if synergy.get("context_breakdown"):
                        ...
                        has_weather = any(
                            key in context for key in ["weather", "weather_boost", "temperature"]
                        )
                        has_energy = any(
                            key in context
                            for key in ["energy", "energy_boost", "energy_cost", "peak_hours"]
                        )
                ...
                if synergies_with_context:
                    logger.info(
                        f"Found {len(synergies_with_context)} synergies with 3rd party context data"
                    )
                else:
                    logger.info(
                        "No synergies with 3rd party context data (enrichment_fetcher may not be configured)"
                    )
            ...
            if patterns and "data" in synergies_data and "synergies" in synergies_data["data"]:
                ...
                if synergies:
                    ...
                    if synergies_with_pattern_devices:
                        logger.info(
                            f"Found {len(synergies_with_pattern_devices)} synergies referencing devices with patterns"
                        )
                        ...
                        logger.info(
                            "No synergies found that reference devices with patterns (may be expected)"
                        )
        ...
        if "data" in data and "synergies" in data["data"]:
            ...
            if synergies:
                for synergy in synergies:
                    ...
                    assert "confidence" in synergy, (
                        "Synergy must have confidence (from raw data analysis)"
                    )
                    assert "impact_score" in synergy, (
                        "Synergy must have impact_score (from raw data analysis)"
                    )
                    ...
                    logger.info(
                        f"✅ Synergy {synergy.get('synergy_id', 'unknown')} has complete data structure"
                    )
