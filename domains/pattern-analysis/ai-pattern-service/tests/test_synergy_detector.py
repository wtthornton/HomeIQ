...
class TestDeviceSynergyDetector:
    ...
    @pytest.fixture
    def detector(self, mock_data_api_client, mock_ha_client, mock_influxdb_client):
    ...
    def sample_devices(self) -> list[dict[str, Any]]:
        ...
        return [
            ...
            {"device_id": "light_kitchen", "name": "Kitchen Light", "area_id": "kitchen"},
            {"device_id": "door_front", "name": "Front Door", "area_id": "entryway"},
        ]
    ...
    async def test_init_with_enrichment_fetcher(self, mock_data_api_client):
        ...
        detector = DeviceSynergyDetector(
            data_api_client=mock_data_api_client, enrichment_fetcher=mock_fetcher
        )
        ...
        pairs = [
            {
                ...
                "entity2": {"entity_id": "light.kitchen", "friendly_name": "Kitchen Light"},
                "area": "kitchen",
        ...
        pairs = [
            {
                ...
                "entity2": {"entity_id": "lock.front_door", "friendly_name": "Front Door Lock"},
                "area": "entryway",
                "domain1": "binary_sensor",
                "domain2": "lock",
            }
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_filter_existing_automations_with_automation_parser(
        self, detector, mock_ha_client
    ):
    ...
    async def test_rank_opportunities_advanced_handles_error(self, detector, sample_entities):
        ...
        mock_analyzer = AsyncMock()
        mock_analyzer.calculate_advanced_impact_score = AsyncMock(
    ...
    async def test_detect_3_device_chains(self, detector):
        ...
        chains = await detector._detect_3_device_chains(pairwise_synergies, [], [])

        # Method should run without error
        assert isinstance(chains, list)

        # Filter for valid chains (all 3 devices unique)
        valid_chains = [chain for chain in chains if len(set(chain["devices"])) == 3]
    ...
    async def test_detect_4_device_chains(self, detector):
        ...
        chains = await detector._detect_4_device_chains(
            three_chains, pairwise_synergies, [], entities
        )
    ...
    async def test_is_valid_cross_area_chain(self, detector):
        ...
        is_valid = detector._is_valid_cross_area_chain("device1", "device2", "device3", entities)
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_detect_synergies_filters_by_confidence(
        self, detector, sample_devices, sample_entities
    ):
    ...
    def test_are_sibling_entities_same_device(self, detector):
        ...
        assert detector._are_sibling_entities("light.hue_strip_1", "scene.garage_natural") is True
    ...
    def test_are_sibling_entities_different_device(self, detector):
        ...
        assert (
            detector._are_sibling_entities("binary_sensor.motion_kitchen", "light.kitchen") is False
        )
    ...
    def test_are_sibling_entities_missing_device_id(self, detector):
        ...
        assert detector._are_sibling_entities("light.hue_strip_1", "light.unknown") is False

        # Neither entity in index
        assert detector._are_sibling_entities("light.unknown_1", "light.unknown_2") is False
    ...
    async def test_find_device_pairs_filters_siblings(self, detector):
        ...
        pair_entity_ids = [(p["entity1"]["entity_id"], p["entity2"]["entity_id"]) for p in pairs]
