...
class TestAutomationGenerator:
    ...
    def generator(self, ha_url: str, ha_token: str) -> AutomationGenerator:
        ...
        return AutomationGenerator(ha_url=ha_url, ha_token=ha_token, ha_version="2025.1")
    ...
    async def test_synergy_to_homeiq_automation(
        self, generator: AutomationGenerator, sample_synergy: dict[str, Any]
    ):
    ...
    async def test_generate_automation_success(
        ...
        # Execute
        result = await generator.generate_automation_from_synergy(sample_synergy, mock_ha_client)
    ...
    async def test_generate_automation_validation_failure(
        ...
        with pytest.raises(ValueError, match="Automation validation failed"):
            await generator.generate_automation_from_synergy(sample_synergy, mock_ha_client)
    ...
    async def test_deploy_automation_success(
        self, generator: AutomationGenerator, mock_ha_client: AsyncMock
    ):
        ...
        # Execute
        automation_id = await generator._deploy_automation(mock_ha_client, yaml_content, synergy_id)
    ...
    async def test_deploy_automation_api_error(
        ...
        with pytest.raises(httpx.HTTPStatusError):
            await generator._deploy_automation(mock_ha_client, yaml_content, synergy_id)
