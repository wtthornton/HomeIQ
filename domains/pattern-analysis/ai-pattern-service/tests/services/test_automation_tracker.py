...
class TestAutomationTracker:
    ...
    def sample_execution_result(self) -> dict[str, Any]:
        ...
        return {"success": True, "error": None, "execution_time_ms": 150, "triggered_count": 5}
    ...
    async def test_track_automation_execution_success(
        ...
        await test_db.execute(
            ...
            """),
            {"synergy_id": synergy_id, "confidence": 0.7, "impact_score": 0.5},
        )
        await test_db.commit()

        # Execute
        await tracker.track_automation_execution(
            automation_id, synergy_id, sample_execution_result, db=test_db
        )
    ...
    async def test_track_automation_execution_failure(
        self, tracker: AutomationTracker, test_db: AsyncSession
    ):
        ...
        await test_db.execute(
            text("""
                ...
                VALUES (:synergy_id, :confidence, :impact_score)
            """),
            {"synergy_id": synergy_id, "confidence": 0.8, "impact_score": 0.7},
        )
        await test_db.commit()

        # Execute
        await tracker.track_automation_execution(
            automation_id, synergy_id, execution_result, db=test_db
        )
    ...
    @pytest.mark.unit
    @pytest.mark.requires_db
    async def test_get_execution_stats(self, tracker: AutomationTracker, test_db: AsyncSession):
    ...
    async def test_track_automation_execution_no_db(self):
        ...
        await tracker.track_automation_execution(
            "automation.test", "synergy-123", {"success": True, "triggered_count": 1}, db=None
        )
