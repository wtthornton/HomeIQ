...
class TestSequencePatternDetection:
    ...
    def test_simple_two_device_sequence(self, detector, base_time):
        ...
        seq_pattern = next((p for p in patterns if p["pattern_type"] == "sequence"), None)
    ...
    def test_three_device_sequence(self, detector, base_time):
        ...
        assert pattern["sequence"] == ["binary_sensor.motion", "light.hallway", "light.living_room"]
    ...
    def test_gap_tolerance_exceeded(self, detector, base_time):
        ...
        two_step = [
            p
            for p in patterns
            if len(p["sequence"]) == 2 and p["sequence"] == ["light.bedroom", "light.living_room"]
        ]
        ...
        events = pd.DataFrame(
            {
                ...
                "timestamp": [base_time + timedelta(seconds=i * 10) for i in range(4)],
                "state": ["game", "on", "72", "on"],
            }
        )
        ...
        events = pd.DataFrame(
            {
                ...
                * 3,
                "timestamp": [
                    base_time + timedelta(hours=h, seconds=s) for h in range(3) for s in [0, 30, 60]
                ],
                "state": ["on"] * 9,
            }
        )
