    ...
    def test_builds_baselines(self, detector, base_time):
        ...
        for day in range(7):
            for hour in [8, 12, 18]:
                ...
                events.append({"device_id": "light.kitchen", "timestamp": timestamp, "state": "on"})
    ...
    def test_detects_unusual_hour(self, detector, base_time):
        ...
        for day in range(5):
            for hour in [8, 9, 10, 18, 19, 20]:
                timestamp = base_time + timedelta(days=day, hours=hour - 8)
                events.append({"device_id": "light.bedroom", "timestamp": timestamp, "state": "on"})

        anomaly_time = base_time + timedelta(days=6, hours=-5)
        events.append({"device_id": "light.bedroom", "timestamp": anomaly_time, "state": "on"})

        df = pd.DataFrame(events)
        patterns = detector.detect_patterns(df)

        timing_anomalies = [p for p in patterns if p["anomaly_type"] == "timing"]
    ...
    def test_detects_high_frequency(self, detector, base_time):
        ...
        for day in range(5):
            for i in range(3):
                ...
                events.append({"device_id": "switch.garage", "timestamp": timestamp, "state": "on"})
        ...
        freq_anomalies = [p for p in patterns if p["anomaly_type"] == "frequency"]
    ...
    def test_detects_low_frequency(self, detector, base_time):
        ...
        for day in range(5):
            for i in range(10):
                timestamp = base_time + timedelta(days=day, hours=i)
                events.append(
                    {"device_id": "binary_sensor.motion", "timestamp": timestamp, "state": "on"}
                )

        timestamp = base_time + timedelta(days=6, hours=12)
        events.append({"device_id": "binary_sensor.motion", "timestamp": timestamp, "state": "on"})
        ...
        freq_anomalies = [
            p
            for p in patterns
            if p["anomaly_type"] == "frequency" and p["metadata"].get("direction") == "low"
        ]
...
class TestDurationAnomalyDetection:
    ...
    def test_detects_long_duration(self, detector, base_time):
        ...
        duration_anomalies = [p for p in patterns if p["anomaly_type"] == "duration"]
...
class TestAbsenceAnomalyDetection:
    ...
    def test_detects_device_absence(self, detector, base_time):
        ...
        events.append(
            {"device_id": "binary_sensor.motion_hall", "timestamp": late_event, "state": "on"}
        )

        df = pd.DataFrame(events)
        patterns = detector.detect_patterns(df)

        absence_anomalies = [p for p in patterns if p["anomaly_type"] == "absence"]
...
class TestSystemNoiseFiltering:
    ...
    def test_filters_system_sensors(self, detector_with_filter, base_time):
        ...
        for i in range(10):
            ...
            events.append(
                {"device_id": "sensor.home_assistant_cpu", "timestamp": timestamp, "state": "50"}
            )
            events.append({"device_id": "light.bedroom", "timestamp": timestamp, "state": "on"})
    ...
    def test_typical_hours_calculation(self, detector, base_time):
        ...
        for day in range(5):
            for hour in [8, 9, 18, 19]:
                timestamp = base_time + timedelta(days=day, hours=hour - 8)
                events.append({"device_id": "light.test", "timestamp": timestamp, "state": "on"})
