...
class TestSequenceIntegration:
    ...
    def generate_morning_routine_events(
        ...
        for day in range(num_days):
            ...
            events.extend(
                [
                    {
                        "device_id": "binary_sensor.bedroom_motion",
                        "timestamp": routine_start,
                        "state": "on",
                    },
                    {
                        "device_id": "light.bedroom",
                        "timestamp": routine_start + timedelta(seconds=15),
                        "state": "on",
                    },
                    {
                        "device_id": "light.bathroom",
                        "timestamp": routine_start + timedelta(minutes=2),
                        "state": "on",
                    },
                    {
                        "device_id": "switch.coffee_maker",
                        "timestamp": routine_start + timedelta(minutes=5),
                        "state": "on",
                    },
                ]
            )
                ...
                [
                    {
                        ...
                        "timestamp": routine_start + timedelta(minutes=1),
                        "state": "68",
                    },
                    {
                        "device_id": "light.kitchen",
                        "timestamp": routine_start + timedelta(minutes=10),
                        "state": "on",
                    },
                ]
            )
        ...
        for i in range(num_occurrences):
            ...
            events.extend(
                [
                    ...
                    {
                        "device_id": "binary_sensor.door_hallway",
                        "timestamp": arrival_time + timedelta(seconds=45),
                        "state": "on",
                    },
                    {
                        "device_id": "light.hallway",
                        "timestamp": arrival_time + timedelta(seconds=50),
                        "state": "on",
                    },
                    {
                        "device_id": "light.living_room",
                        "timestamp": arrival_time + timedelta(minutes=1, seconds=30),
                        "state": "on",
                    },
                ]
            )
    ...
    def test_morning_routine_detection(self, detector):
        ...
        two_step = [
            ...
            for p in patterns
            if len(p["sequence"]) == 2
            and "bedroom_motion" in p["sequence"][0]
            and "bedroom" in p["sequence"][1]
        ]
    ...
    def test_arrival_sequence_detection(self, detector):
        ...
        garage_patterns = [
            p
            for p in patterns
            if any("garage" in d for d in p["sequence"])
            and any("hallway" in d for d in p["sequence"])
        ]
    ...
    def test_combined_patterns(self, detector):
        ...
        morning_patterns = [p for p in patterns if any("bedroom" in d for d in p["sequence"])]
        assert len(morning_patterns) >= 1

        # Verify we got arrival patterns
        arrival_patterns = [
            p for p in patterns if any("garage" in d or "hallway" in d for d in p["sequence"])
        ]
