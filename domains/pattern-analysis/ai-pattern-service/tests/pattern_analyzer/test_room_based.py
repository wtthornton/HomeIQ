...
class TestAreaIdExtraction:
    ...
    def test_single_word_room(self):
        ...
        result = RoomBasedPatternDetector._extract_area_from_entity_id("light.bedroom_lamp")
        assert result == "bedroom"

    def test_multi_word_room(self):
        """Test extraction of multi-word room names."""
        result = RoomBasedPatternDetector._extract_area_from_entity_id("light.living_room_lamp")
    ...
    def test_simple_entity(self):
        ...
        result = RoomBasedPatternDetector._extract_area_from_entity_id("light.hallway")
...
class TestRoomPatternDetection:
        ...
        for day in range(3):
            ...
            events_data.extend(
                [
                    ...
                    {
                        "device_id": "switch.living_room_fan",
                        "timestamp": t + timedelta(seconds=30),
                        "state": "on",
                        "area_id": "living_room",
                    },
            ...
            events_data.extend(
                [
                    {
                        "device_id": "light.bedroom_light",
                        "timestamp": morning,
                        "state": "on",
                        "area_id": "bedroom",
                    },
                    {
                        "device_id": "cover.bedroom_blinds",
                        "timestamp": morning + timedelta(seconds=45),
                        "state": "open",
                        "area_id": "bedroom",
                    },
                ]
            ...
            events_data.extend(
                [
                    ...
                    },
                    {
                        ...
                        "timestamp": t + timedelta(seconds=50),
                        "state": "cool",
                        "area_id": "living_room",
                    },
                ]
    ...
    def test_unknown_area_filtered(self, detector, base_time):
        ...
        for day in range(3):
            ...
            events_data.extend(
                [
                    {"device_id": "light.x", "timestamp": t, "state": "on", "area_id": "unknown"},
                    {
                        "device_id": "switch.y",
...
class TestConfidenceCalculation:
    ...
    def test_high_occurrence_high_confidence(self):
        ...
        detector = RoomBasedPatternDetector()
        confidence = detector._calculate_confidence(occurrences=10, std_hour=0.3, n_devices=3)
        assert confidence > 0.8

    def test_low_occurrence_lower_confidence(self):
        """Test that few occurrences gives lower confidence."""
        detector = RoomBasedPatternDetector()
        confidence = detector._calculate_confidence(occurrences=2, std_hour=0.3, n_devices=2)
        assert confidence < 0.7

    def test_high_variance_lowers_confidence(self):
        """Test that high timing variance lowers confidence."""
        detector = RoomBasedPatternDetector()
        low_var = detector._calculate_confidence(occurrences=5, std_hour=0.3, n_devices=2)
        high_var = detector._calculate_confidence(occurrences=5, std_hour=5.0, n_devices=2)
        assert low_var > high_var

    def test_more_devices_slight_bonus(self):
        """Test that more devices gives a slight confidence bonus."""
        detector = RoomBasedPatternDetector()
        two_devices = detector._calculate_confidence(occurrences=5, std_hour=1.0, n_devices=2)
        four_devices = detector._calculate_confidence(occurrences=5, std_hour=1.0, n_devices=4)
