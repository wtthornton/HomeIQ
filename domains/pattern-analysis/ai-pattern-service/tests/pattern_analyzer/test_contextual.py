...
class TestContextualDetection:
    ...
    def test_detects_sunset_correlation(self, detector):
        ...
        sunset_patterns = [p for p in patterns if p["context_type"] == "sunset"]
    ...
    def test_sunrise_correlation(self, detector):
        ...
        sunrise_patterns = [p for p in patterns if p["context_type"] == "sunrise"]
...
class TestTemperatureCorrelation:
    def test_positive_correlation(self):
        ...
        temp_patterns = [p for p in patterns if p["context_type"] == "temperature"]
        ...
        events = pd.DataFrame(
            {
                ...
                "timestamp": pd.to_datetime([f"2025-06-{d:02d} 12:00" for d in range(1, 21)]),
                "state": ["on"] * 20,
            }
        )
        patterns = detector.detect_patterns(events)
...
class TestSafeCorrelation:
    ...
    def test_too_few_points(self):
        assert (
            ContextualPatternDetector._safe_correlation(np.array([1.0, 2.0]), np.array([3.0, 4.0]))
            == 0.0
        )
...
class TestConfidence:
    ...
    def test_temp_low_confidence(self):
        confidence = ContextualPatternDetector._calculate_temp_confidence(day_count=5, abs_corr=0.3)
        assert confidence < 0.3
...
class TestAutomationSuggestion:
    ...
    def test_empty_device(self, detector):
        assert detector.suggest_automation({"pattern_type": "contextual", "device_id": ""}) == {}
...
class TestBuildTrigger:
    def test_sunset_positive_offset(self):
        trigger = ContextualPatternDetector._build_trigger("sunset", {"avg_offset_minutes": 15})
    ...
    def test_sunrise_negative_offset(self):
        trigger = ContextualPatternDetector._build_trigger("sunrise", {"avg_offset_minutes": -10})
        assert trigger["platform"] == "sun"
        assert trigger["event"] == "sunrise"
        assert "-" in trigger["offset"]

    def test_temperature_trigger(self):
        trigger = ContextualPatternDetector._build_trigger("temperature", {})
