...
class TestSeasonalDetection:
    ...
    def test_detects_seasonal_shift(self, detector):
        ...
        events = self._make_seasonal_events(
            "climate.heater",
            {
                "winter": (10, 8),
                "summer": (1, 8),
    ...
    def test_multiple_devices(self, detector):
        events1 = self._make_seasonal_events(
            "climate.heater",
            {
                "winter": (10, 8),
                "summer": (1, 8),
        ...
        events2 = self._make_seasonal_events(
            "climate.ac",
            {
                "winter": (1, 14),
                "summer": (10, 14),
        ...
        events = pd.DataFrame(
            {
                ...
                "timestamp": pd.to_datetime([base + timedelta(days=i) for i in range(20)]),
                "state": ["on"] * 20,
            }
        )
        patterns = detector.detect_patterns(events)
...
class TestTimingShift:
    def test_no_shift(self):
        ...
        shift = SeasonalPatternDetector._compute_timing_shift(profiles, ["winter", "summer"])
        assert shift == 0.0
...
class TestConfidence:
    def test_high_data_high_confidence(self):
        ...
        confidence = SeasonalPatternDetector._calculate_confidence(profiles, list(profiles.keys()))
...
class TestAutomationSuggestion:
    ...
    def test_empty_device(self, detector):
        assert detector.suggest_automation({"pattern_type": "seasonal", "device_id": ""}) == {}
