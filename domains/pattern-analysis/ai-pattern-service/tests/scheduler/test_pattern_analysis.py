...
class TestPatternAnalysisSchedulerInitialization:
    ...
    def test_scheduler_initialization_custom_schedule(self):
        ...
        scheduler = PatternAnalysisScheduler(cron_schedule="0 3 * * *", enable_incremental=False)
...
class TestPatternDetection:
    ...
    async def test_detect_time_of_day_patterns_success(self):
        ...
        scheduler = PatternAnalysisScheduler()
        events_df = pd.DataFrame(
            {"entity_id": ["light.office"], "state": ["on"], "timestamp": [datetime.now(UTC)]}
        )
        job_result = {"errors": []}

        mock_detector = MagicMock()
        mock_detector.detect_patterns = MagicMock(
...
class TestSynergyDetection:
    ...
    async def test_detect_synergies_success(self):
        ...
        mock_detector.detect_synergies = AsyncMock(return_value=[{"synergy_type": "device_pair"}])

        with patch(
            "src.scheduler.pattern_analysis.DeviceSynergyDetector", return_value=mock_detector
        ):
    ...
    async def test_detect_synergies_failure(self):
        ...
        with patch(
            "src.scheduler.pattern_analysis.DeviceSynergyDetector",
            side_effect=Exception("Synergy failed"),
        ):
            synergies = await scheduler._detect_synergies(mock_client, events_df, job_result)
