...
class PatternCrossValidator:
    ...
    async def cross_validate(self, patterns: list[dict]) -> dict:
        ...
        for device_id, device_patterns in by_device.items():
            ...
            time_patterns = [p for p in device_patterns if p["pattern_type"] == "time_of_day"]

            # Find co-occurrence patterns
            co_patterns = [p for p in device_patterns if p["pattern_type"] == "co_occurrence"]
            ...
            if len(time_patterns) > 1:
                ...
                for i, time1 in enumerate(times):
                    for time2 in times[i + 1 :]:
                        diff_minutes = abs((time1[0] * 60 + time1[1]) - (time2[0] * 60 + time2[1]))
                        if diff_minutes <= 30:
        ...
        quality_score = (
            0.5 + (reinforcements * 0.1) - (contradictions * 0.2) - (redundancies * 0.05)
        )
