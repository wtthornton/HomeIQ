...
class TimeOfDayPatternDetector:
    ...
    def detect_patterns(self, events: pd.DataFrame) -> list[dict]:
        ...
        for device_id in unique_devices:
            ...
            required_occurrences = self.domain_occurrence_overrides.get(
                domain, self.min_occurrences
            )
            ...
            if len(device_events) < 5:
                logger.debug(
                    f"Skipping {device_id}: insufficient data ({len(device_events)} events)"
                )
                continue
            ...
            try:
                ...
                for cluster_id in range(n_clusters):
                    ...
                    if len(cluster_times) >= required_occurrences:
                        ...
                        std_minutes = (
                            float(cluster_times.std() * 60) if len(cluster_times) > 1 else 0.0
                        )
                        variance_penalty = min(
                            std_minutes / 120.0, 0.3
                        )  # Max 30% penalty for high variance
                        ...
                        confidence = min(
                            0.95, occurrence_ratio * (1 - variance_penalty) + threshold_boost
                        )
                        ...
                        if confidence >= required_confidence:
                            ...
                            pattern = {
                                ...
                                "confidence": float(
                                    confidence
                                ),  # Now calculated with variance penalty
                                "metadata": {
        ...
        logger.info(
            f"✅ Detected {len(patterns)} time-of-day patterns across {len(unique_devices)} devices"
        )
    ...
    def _store_daily_aggregates(self, patterns: list[dict], events: pd.DataFrame) -> None:
        ...
        try:
            ...
            for pattern in patterns:
                ...
                top_count = max(
                    1, len([h for h in sorted_hours if hourly_distribution[h] > 0]) // 4
                )
    ...
    def suggest_automation(self, pattern: dict) -> dict[str, any]:
        ...
        if pattern.get("pattern_type") != "time_of_day":
            logger.warning(
                f"Pattern type {pattern.get('pattern_type')} is not time_of_day, cannot suggest automation"
            )
            return {}
        ...
        service_name, expected_state, safety_level, safety_warnings = self._get_service_config(
            domain
        )
        ...
        device_name = (
            device_id.split(".")[-1].replace("_", " ").title() if "." in device_id else device_id
        )
        ...
        condition = None
        if expected_state:
            condition = [{"condition": "state", "entity_id": device_id, "state": expected_state}]
        ...
        suggestion = {
            ...
            "metadata": {
                ...
                "time_range": pattern.get("metadata", {}).get(
                    "time_range", f"{hour:02d}:{minute:02d}"
                ),
