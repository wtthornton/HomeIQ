...
class EnergySavingsCalculator:
    ...
    def __init__(
        ...
        device_power: dict[str, float] | None = None,
    ):
        """
    ...
    def calculate_energy_savings(self, synergy: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        if not devices:
            return {"energy_savings_score": 0.0, "rationale": "No devices in synergy"}
        ...
        if (
            ...
            or context_metadata.get("context_type") == "energy_scheduling"
        ):
            ...
            for domain in device_domains:
                if domain in high_power_domains:
                    ...
                    rationale_parts.append(
                        f"{domain} scheduling: ~${monthly_cost_savings:.2f}/month"
                    )

        # Pattern 2: Carbon intensity optimization
        if (
            synergy_type == "carbon_context"
            or context_metadata.get("context_type") == "carbon_scheduling"
        ):
            ...
            for domain in device_domains:
                if domain in high_power_domains:
                    ...
                    energy_score += 0.3
                    rationale_parts.append(
                        f"{domain} carbon optimization: ~${monthly_cost_savings:.2f}/month"
                    )
    ...
    def estimate_kwh_savings(self, synergy: dict[str, Any]) -> float:
        """
    ...
    def estimate_cost_savings(self, synergy: dict[str, Any]) -> float:
        """
