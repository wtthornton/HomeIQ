...
class DeviceSynergyDetector:
    ...
    def __init__(
        ...
        enrichment_fetcher: Any | None = None,
        ...
        if influxdb_client:
            try:
                ...
                logger.warning(
                    f"Failed to initialize DevicePairAnalyzer: {e}, continuing without advanced scoring"
                )
        ...
        for synergy in synergies:
            ...
            try:
                ...
                for eid in (trigger, action):
                    failures = await self.verification_store.query_failures(eid, lookback_hours=24)
                    if len(failures) >= 2:
    ...
    async def _rank_and_filter_synergies(
        ...
        if db:
            try:
                ...
                if patterns:
                    logger.info(
                        f"   → Validating {len(ranked_synergies)} synergies against {len(patterns)} patterns"
                    )
                    validated_count = 0
                    for synergy in ranked_synergies:
                        support_score, supporting_pattern_ids = self._calculate_pattern_support(
                            synergy, patterns
                        )
                    ...
                    logger.info(
                        f"   → Pattern validation complete: {validated_count}/{len(ranked_synergies)} synergies validated by patterns"
                    )
                else:
    ...
    async def _enrich_with_blueprints(
        ...
        try:
            ...
            logger.info(
                f"   → Blueprint enrichment: {enriched_count}/{len(synergies)} synergies matched blueprints"
            )
            return synergies
    ...
    def _calculate_pattern_support(
        self, synergy: dict[str, Any], patterns: list[Any]
        ...
        if len(device_time_patterns) >= 2:
            ...
            avg_confidence = (
                sum(pattern_confidences) / len(pattern_confidences) if pattern_confidences else 0.0
            )
            ...
            if patterns_list:  # Prevent division by zero
                avg_confidence = sum(p.get("confidence", 0.0) for p in patterns_list) / len(
                    patterns_list
                )
        ...
        device_patterns = [p for p in pattern_dicts if p.get("device_id") in device_ids]
        if device_patterns:
            avg_confidence = sum(p.get("confidence", 0.0) for p in device_patterns) / len(
                device_patterns
            )
    ...
    async def detect_synergies_from_patterns(
        self, patterns: list[dict[str, Any]], _db: AsyncSession | None = None
    ) -> list[dict[str, Any]]:
        """
        ...
        logger.info(
            f"   → Grouped into {len(time_groups_30min)} 30-min windows, {len(time_groups_hourly)} hourly windows"
        )
                ...
                if len(unique_devices) >= 2:
                    ...
                    for i, device1 in enumerate(unique_devices[:5]):  # Limit to 5 devices
                        for device2 in unique_devices[i + 1 : 5]:
                            ...
                            avg_confidence = sum(
                                p.get("confidence", 0.7) for p in patterns_at_time
                            ) / len(patterns_at_time)
                            ...
                            synergy = {
                                ...
                                "area": area,
                                "impact_score": min(
                                    0.9, avg_confidence + 0.1
                                ),  # Boost for 30-min match
                                ...
                                "supporting_pattern_ids": [
                                    p.get("id") for p in patterns_at_time if p.get("id")
                                ],
                                "synergy_depth": 2,
    ...
    def _lookup_area_from_entities(
        self, entity_ids: list[str], entities: list[dict[str, Any]] | None = None
    ) -> str | None:
        """
    ...
    def _pattern_to_synergy(
        self, pattern: dict[str, Any], entities: list[dict[str, Any]] | None = None
    ) -> dict[str, Any] | None:
        """
    ...
    async def detect_synergies(self, db: AsyncSession | None = None) -> list[dict[str, Any]]:
        """
        ...
        logger.info(
            f"   → Parameters: min_confidence={self.min_confidence}, same_area_required={self.same_area_required}"
        )
        ...
        try:
            ...
            if synergies:
                pairwise_synergies = await self._rank_and_filter_synergies(
                    synergies, entities, db=db
                )

                # Step 7-8: Detect chains
                chains_3 = await self._detect_3_device_chains(pairwise_synergies, devices, entities)
                chains_4 = await self._detect_4_device_chains(
                    chains_3, pairwise_synergies, devices, entities
                )
            else:
            ...
            context_synergies = await self._detect_context_aware_synergies(
                pairwise_synergies, entities
            )
            ...
            if self.spatial_intelligence and not self.same_area_required:
                try:
                    ...
                    for synergy in final_synergies:
                        ...
                        if area and len(devices) >= 2:
                            ...
                            if is_valid:
                                ...
                                logger.debug(
                                    f"Filtered cross-area synergy {synergy.get('synergy_id')} (spatial validation failed)"
                                )
                        else:
                            validated_synergies.append(synergy)
                    ...
                    logger.info(
                        f"   → Spatial validation: {len(final_synergies)} synergies validated"
                    )
                except Exception as e:
                ...
                try:
                    ...
                    for synergy in final_synergies:
                        try:
                            ...
                            if energy_result:
                                synergy["energy_savings_score"] = energy_result.get(
                                    "energy_savings_score", 0.0
                                )
                                synergy["estimated_kwh_savings"] = energy_result.get(
                                    "estimated_kwh_savings", 0.0
                                )
                                synergy["estimated_cost_savings"] = energy_result.get(
                                ...
                                synergy["context_metadata"]["energy"] = {
                                    ...
                                    "cost_savings": energy_result.get(
                                        "estimated_cost_savings", 0.0
                                    ),
                                }
                        except Exception as e:
                            logger.debug(
                                f"Failed to calculate energy savings for synergy {synergy.get('synergy_id')}: {e}"
                            )
                except Exception as e:
            ...
            if self.temporal_detector:
                try:
                    for synergy in final_synergies:
                        try:
                            enhanced = await self.temporal_detector.enhance_with_temporal_context(
                                synergy
                            )
                            if enhanced:
                            ...
                            logger.debug(
                                f"Failed to enhance synergy {synergy.get('synergy_id')} with temporal context: {e}"
                            )
                except Exception as e:
            ...
            if self.explainer:
                for synergy in final_synergies:
                    try:
                        ...
                        logger.warning(
                            f"Failed to generate explanation for synergy {synergy.get('synergy_id', 'unknown')}: {e}"
                        )
            ...
            self._log_synergy_results(
                ...
                final_synergies,
                duration,
                scene_synergies=scene_synergies,
                context_synergies=context_synergies,
            )
    ...
    async def _fetch_device_data(self) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        if not devices or not entities:
            ...
            logger.warning(
                f"   → Devices: {len(devices) if devices else 0}, Entities: {len(entities) if entities else 0}"
            )
            return ([], [])
    ...
    async def _detect_compatible_pairs_pipeline(
        self, devices: list[dict[str, Any]], entities: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """
        ...
        if device_pairs:
            logger.info(
                f"   → Sample pairs: {[(p.get('domain1', '?'), p.get('domain2', '?'), p.get('area', '?')) for p in device_pairs[:3]]}"
            )
        ...
        if compatible_pairs:
            logger.info(
                f"   → Sample compatible: {[p.get('relationship_type', '?') for p in compatible_pairs[:3]]}"
            )
    ...
    async def _get_devices(self) -> list[dict[str, Any]]:
        ...
        if (
            ...
            and self._device_cache_timestamp is not None
            and datetime.now(UTC) - self._device_cache_timestamp < self._cache_ttl
        ):
    ...
    def _are_sibling_entities(
        self, entity_id1: str, entity_id2: str, sibling_index: dict[str, str] | None = None
    ) -> bool:
        """
    ...
    async def _get_entities(self) -> list[dict[str, Any]]:
        ...
        if (
            ...
            and datetime.now(UTC) - self._entity_cache_timestamp < self._cache_ttl
        ):
        ...
        try:
            ...
            logger.info(
                f"✅ Loaded {len(self._entity_cache)} entities (cached for 6 hours, {len(self._sibling_index)} with device links)"
            )
            return self._entity_cache
        except Exception as e:
    ...
    def _find_device_pairs_by_area(
        self, _devices: list[dict[str, Any]], entities: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """
        ...
        if entities_without_area:
            logger.info(
                f"   → Found {len(entities_without_area)} entities without area, pairing compatible domains"
            )
                            ...
                            pairs.append(
                                {
                                    "entity1": entity1,
                                    "entity2": entity2,
                                    "area": entity1.get("area_id")
                                    or entity2.get("area_id")
                                    or None,
                                    "domain1": domain1,
                                    "domain2": domain2,
                                }
            ...
            logger.info(
                f"   → Created {processed_pairs} pairs from entities without area (filtered by compatible domains)"
            )

        if sibling_filtered_count > 0:
            logger.info(
                f"   ⏭️ Filtered {sibling_filtered_count} sibling entity pairs (same device_id)"
            )
    ...
    def _check_relationship_match(
        ...
        if (
            domain1 == trigger_domain
            and domain2 == action_domain
            and self._check_device_class_match(entity1, rel_config)
        ):
            return (entity1, entity2)

        # Check reverse direction: domain2 -> domain1
        if (
            domain2 == trigger_domain
            and domain1 == action_domain
            and self._check_device_class_match(entity2, rel_config)
        ):
            return (entity2, entity1)

        return None

    def _check_device_class_match(self, entity: dict[str, Any], rel_config: dict[str, Any]) -> bool:
        """
    ...
    def _get_automation_parser(self) -> Any | None:
        """
    ...
    def _log_filtered_pair(self, trigger_entity: str, action_entity: str, parser: Any) -> list[str]:
        """
    ...
    def _filter_pairs_with_parser(
        self, compatible_pairs: list[dict[str, Any]], parser: Any
    ...
    async def _filter_existing_automations(
        self, compatible_pairs: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                f"   → Parsed {count} automations, indexed {parser.get_entity_pair_count()} entity pairs"
            )
    ...
    def _calculate_fallback_score(self, synergy: dict[str, Any]) -> float:
        """
    ...
    def _ensure_synergy_scores(self, synergy: dict[str, Any]) -> None:
        """
    ...
    def _log_scoring_results(self, scored_synergies: list[dict[str, Any]]) -> None:
        """
        ...
        if len(scored_synergies) > 1:
            unique_scores = len({round(s["impact_score"], 4) for s in scored_synergies})
            score_range = max(s["impact_score"] for s in scored_synergies) - min(
                s["impact_score"] for s in scored_synergies
            )
            logger.info(
    ...
    async def _rank_opportunities_advanced(
        ...
        for synergy in synergies:
            ...
            scored_synergy = await self._calculate_synergy_score_with_fallback(
                synergy, area_synergies, entities, enrichment_fetcher
            )
            ...
            try:
                ...
                for synergy in scored_synergies:
                    try:
                        ...
                        logger.warning(
                            f"Failed to enhance synergy {synergy.get('synergy_id', 'unknown')} with context: {e}"
                        )
                    ...
                    try:
                        ...
                        if "rl_adjustment" in synergy:
                            ...
                            if isinstance(synergy["opportunity_metadata"], dict):
                                synergy["opportunity_metadata"]["rl_adjustment"] = synergy[
                                    "rl_adjustment"
                                ]
                    except Exception as e:
                        logger.warning(
                            f"Failed to optimize synergy {synergy.get('synergy_id', 'unknown')} with RL: {e}"
                        )
    ...
    async def _detect_3_device_chains(
        ...
        for synergy in pairs_to_use:
            ...
            for next_synergy in action_lookup[action_entity]:
                if len(chains) >= MAX_CHAINS:
                    logger.info(
                        f"   → Reached chain limit ({MAX_CHAINS}), stopping chain detection"
                    )
                    break
                ...
                if self._should_skip_chain(
                    trigger_entity, next_action, synergy, next_synergy, entities
                ):
                    continue
            ...
            if processed_count % 100 == 0:
                logger.debug(
                    f"   → Processed {processed_count}/{len(pairwise_synergies)} pairs for chains, found {len(chains)} chains"
                )
    ...
    async def _detect_4_device_chains(
        ...
        for three_chain in three_level_chains:
            ...
            for next_synergy in action_lookup[c]:
                if len(chains) >= MAX_CHAINS:
                    logger.info(
                        f"   → Reached 4-level chain limit ({MAX_CHAINS}), stopping detection"
                    )
                    break
                ...
                if self._should_skip_4_chain(
                    d, a, chain_devices, three_chain, next_synergy, entities
                ):
                    continue
    ...
    def _is_valid_cross_area_chain(
        self, _device1: str, _device2: str, _device3: str, _entities: list[dict[str, Any]]
    ) -> bool:
        """
    ...
    async def _detect_scene_based_synergies(
        ...
        for area_id, devices in area_devices.items():
            ...
            if len(devices) >= 3:
                area_scenes = [
                    s for s in scene_entities if area_id.lower() in s.get("entity_id", "").lower()
                ]
                ...
                if not area_scenes:
                    synergy = {
                        ...
                        "context_metadata": {
                            ...
                            "device_domains": list({d.split(".")[0] for d in devices}),
                        },
                    }
                    synergies.append(synergy)
            ...
            for domain, devices in domain_devices.items():
                ...
                if len(devices) >= 5:  # Require more devices for domain-based scenes
                    ...
                    domain_scenes = [
                        ...
                        if f"all_{domain}" in s.get("entity_id", "").lower()
                        or f"{domain}_all" in s.get("entity_id", "").lower()
                    ]
    ...
    async def _detect_context_aware_synergies(
        self, _pairwise_synergies: list[dict[str, Any]], entities: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """
        ...
        climate_devices = [
            e.get("entity_id") for e in entities if e.get("entity_id", "").startswith("climate.")
        ]
        cover_devices = [
            e.get("entity_id") for e in entities if e.get("entity_id", "").startswith("cover.")
        ]
        light_devices = [
            e.get("entity_id") for e in entities if e.get("entity_id", "").startswith("light.")
        ]
        ...
        if energy_sensors and high_power_devices:
            for device in high_power_devices[:5]:  # Limit to 5 devices
                ...
                energy = (
                    energy_sensors[0].get("entity_id") if energy_sensors else "sensor.energy_price"
                )
