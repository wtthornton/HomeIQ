...
class FBVLQualityScorer:
    ...
    def calculate_quality_with_feedback(
        self, pattern: dict[str, Any], base_quality: float
    ) -> dict[str, Any]:
        """
    ...
    def _get_validation_feedback(self, pattern: dict[str, Any]) -> dict[str, Any]:
        ...
        if similar_patterns:
            acceptance_count = sum(1 for p in similar_patterns if p.get("user_accepted", False))
            feedback["user_acceptance_rate"] = acceptance_count / len(similar_patterns)
            feedback["similar_pattern_count"] = len(similar_patterns)

        return feedback

    def _apply_feedback(self, base_quality: float, validation_feedback: dict[str, Any]) -> float:
        """
    ...
    def _find_similar_patterns(self, pattern: dict[str, Any]) -> list[dict[str, Any]]:
        """
    ...
    def add_validation_data(
        self, pattern: dict[str, Any], quality: float, user_accepted: bool = False
    ):
        """
        ...
        """
        validation_entry = {**pattern, "quality": quality, "user_accepted": user_accepted}
        self.validation_data.append(validation_entry)

    def get_feedback_statistics(self) -> dict[str, Any]:
        """Get feedback statistics"""
        if not self.feedback_history:
            return {"total_feedback": 0, "average_adjustment": 0.0, "ground_truth_matches": 0}
