...
class DeploymentRequest(BaseModel):
    ...
    input_values: dict[str, Any] = Field(
        default_factory=dict, description="Input values for the blueprint (entity IDs, etc.)"
    )
    use_auto_fill: bool = Field(
        default=True, description="Whether to use auto-fill for missing inputs"
    )
    enabled: bool = Field(default=True, description="Whether to enable the automation")
    mode: str = Field(
        default="single", description="Automation mode: single, restart, queued, parallel"
    )
...
class BlueprintImportResult(BaseModel):
    ...
    success: bool
    blueprint_path: str | None = Field(
        None, description="HA blueprint path (e.g., homeassistant/motion_light)"
    )
    error: str | None = None
