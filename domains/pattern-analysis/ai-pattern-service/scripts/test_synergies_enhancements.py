...
async def test_energy_savings_calculator():
    ...
    print(
        f"   ✅ Energy savings: {result['energy_savings_score']:.2f}, ${result.get('estimated_cost_savings', 0):.2f}/month"
    )
    return True
...
async def test_temporal_detector():
    ...
    print(
        f"   ✅ Temporal detector initialized (found {len(temporal_synergies)} temporal synergies)"
    )
    return True
...
async def test_capability_analyzer():
    ...
    print(
        f"   ✅ Capability analyzer initialized (found {len(capabilities)} capabilities for test device)"
    )
    return True
