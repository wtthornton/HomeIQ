...
This import will fail unless the working directory is exactly `domains/blueprints/automation-miner` or `PYTHONPATH` is set. Other test files (like `test_blueprint_parser.py`) use `sys.path` manipulation, but `test_api.py` does not.
