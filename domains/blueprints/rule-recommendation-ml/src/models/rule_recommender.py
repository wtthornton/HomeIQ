...
import logging
import pickle  # noqa: S403  # pickle used for trusted internal model serialization
...
class RuleRecommender:
    ...
    def recommend(
        ...
        recommendations = [
            ...
            for idx, score in zip(ids, scores, strict=True)
        ]
    ...
    def get_similar_rules(
        ...
        similar = [
            (self.idx_to_pattern.get(idx, f"unknown_{idx}"), float(score))
            for idx, score in zip(ids[1:], scores[1:], strict=True)
        ]
    ...
    def load(cls, path: Path | str) -> "RuleRecommender":
        ...
        with open(path, "rb") as f:
            model_data = pickle.load(f)  # noqa: S301  # trusted internal model file
