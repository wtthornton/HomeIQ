 ...
 * updated dashboard/nav/health-card selectors to match tab-based architecture.
 */
test.describe('Enhanced Integration and Performance Tests', () => {
  ...
  test.describe('Complete Data Flow Integration', () => {
    ...
    test('End-to-end data flow from Home Assistant simulation to dashboard', async ({ page }) => {
      // Step 1: Verify all services are healthy (enrichment-pipeline removed — Epic 31)
      const services = [
      ...
      try {
        ...
        if (wsResponse.status() === 200) {
          ...
          // Step 5: Verify event appears in data API
      ...
      // Verify health card is visible (system status)
    ...
    test('Service dependency chain validation', async ({ page }) => {
      ...
      // 3. Admin API aggregates all service health
      ...
      // 4. Verify dependency relationships in admin API response
    ...
    test('Weather enrichment integration end-to-end', async ({ page }) => {
      // Weather enrichment is now inline in websocket-ingestion (Epic 31)
      ...
      if (weatherResponse.status() === 200) {
        ...
        // Verify weather data appears in dashboard health cards
        await page.goto('http://localhost:3000');
        await page.waitForSelector('[data-testid="dashboard-root"]', { timeout: 15000 });

        const weatherCard = page.locator('[data-testid="health-card"]').filter({ hasText: 'Weather Enrichment' });
        await expect(weatherCard).toBeVisible();
        // Health card value is inside the card — query via text content
        await expect(weatherCard).toContainText('Enabled');
      }
    });
  ...
  test.describe('Data Integrity and Consistency', () => {
    ...
    test('Health status consistency across endpoints', async ({ page }) => {
      // Get health from all services (enrichment-pipeline removed)
      const services = [
