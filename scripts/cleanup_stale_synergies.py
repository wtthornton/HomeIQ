        ...
        for row in all_synergies:
            ...
            if not should_remove:
                ...
                if quality_score is None:
                    # Missing quality score - skip (needs quality scores calculated first)
                    continue
            ...
            if not should_remove and active_devices is not None:
                device_ids_val = row['device_ids']
                if device_ids_val:
                    try:
                        device_ids = json.loads(device_ids_val) if isinstance(device_ids_val, str) else device_ids_val
        ...
        if dry_run:
            ...
            if to_remove:
                ...
                for i in range(0, len(to_remove), batch_size):
                    ...
                    cursor.execute(
                        f"DELETE FROM patterns.synergy_opportunities WHERE synergy_id IN ({placeholders})",
                        batch
                    )
...
async def main():
    ...
    parser.add_argument('--pg-url', type=str, default=POSTGRES_URL, help='PostgreSQL connection URL')
