...
display_dashboard() {
    ...
    docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "homeiq-collectors|homeiq-device-intelligence|homeiq-automation-linter|homeiq-model-server|homeiq-health-dashboard" | head -6
