#!/usr/bin/env python3
"""
Monitor query performance for PostgreSQL and InfluxDB.
...
PG_SLOW_QUERY_MS = 1000  # 1 second
INFLUXDB_SLOW_QUERY_MS = 5000  # 5 seconds

def test_pg_queries(conn):
    """Test common PostgreSQL queries and measure performance"""
    ...
    test_queries = [
        {
            ...
            'query': "SELECT * FROM automation.suggestions WHERE status = 'draft' ORDER BY created_at DESC LIMIT 10",
            'description': 'Common query: Filter by status with date sorting'
        },
        {
            'name': 'Get patterns by type',
            'query': "SELECT * FROM patterns.patterns WHERE pattern_type = 'time_of_day' ORDER BY confidence DESC LIMIT 10",
            'description': 'Common query: Filter by pattern type with confidence sorting'
        },
        {
            'name': 'Get user queries',
            'query': "SELECT * FROM automation.ask_ai_queries WHERE user_id = 'anonymous' ORDER BY created_at DESC LIMIT 10",
        ...
        },
        {
            ...
            'query': """
                ...
                FROM automation.suggestions s
                LEFT JOIN patterns.patterns p ON s.pattern_id = p.id
    ...
    for test in test_queries:
        try:
            ...
            status = "✅" if elapsed_ms < PG_SLOW_QUERY_MS else "⚠️"
            ...
            results.append({
                ...
                'row_count': len(rows),
                'slow': elapsed_ms >= PG_SLOW_QUERY_MS
            })
...
def generate_report(pg_results, pg_slow, influxdb_results, influxdb_slow):
    ...
    # PostgreSQL summary
    if pg_results:
        pg_avg = sum(r.get('elapsed_ms', 0) for r in pg_results if 'elapsed_ms' in r) / len([r for r in pg_results if 'elapsed_ms' in r])
        print(f"PostgreSQL Performance:")
        print(f"  Queries tested: {len(pg_results)}")
        print(f"  Average execution time: {pg_avg:.2f}ms")
        print(f"  Slow queries (>={PG_SLOW_QUERY_MS}ms): {len(pg_slow)}")
        print()
    ...
    # Slow queries
    if pg_slow or influxdb_slow:
        print("⚠️  SLOW QUERIES DETECTED:")
        print("-" * 80)

        if pg_slow:
            print("\nPostgreSQL Slow Queries:")
            for query in pg_slow:
                print(f"  - {query['name']}: {query['elapsed_ms']:.2f}ms")
                print(f"    {query['description']}")
                print(f"    Recommendation: Check indexes, use EXPLAIN ANALYZE")
...
def main():
    ...
    print()
    print(f"Thresholds:")
    print(f"  PostgreSQL slow query: >={PG_SLOW_QUERY_MS}ms")
    ...
    try:
        ...
        pg_results, pg_slow = test_pg_queries(conn)
        conn.close()
    except Exception as e:
    ...
    # Generate report
    generate_report(pg_results, pg_slow, influxdb_results, influxdb_slow)

    return len(pg_slow) == 0 and len(influxdb_slow) == 0
