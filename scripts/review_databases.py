    ...
    async def review_influxdb(self):
        ...
        try:
            ...
            if health.status != "pass":
                ...
                print(f"[OK] InfluxDB connection: OK")
            ...
            try:
                ...
                if not result:
                    ...
                    print(f"[WARNING] No data found in the last hour")
                else:
                    ...
                    if sample_result:
                        for table in sample_result:
                            for record in table.records:
                                ...
                                print(f"  Tags: {dict((k, v) for k, v in record.values.items() if not k.startswith('_') and v is not None)}")
                                break
                    
            except Exception as e:
                self.issues.append(f"Error querying InfluxDB: {str(e)}")
                print(f"[ERROR] Error querying InfluxDB: {e}")
            
            client.close()
            
        except Exception as e:
            self.issues.append(f"Error connecting to InfluxDB: {str(e)}")
    ...
    async def review_sqlite(self):
        ...
        try:
            async with aiosqlite.connect(db_path) as db:
                ...
                for table in expected_tables:
                    ...
                    if table == 'entities':
                        ...
                        if orphaned > 0:
                            ...
                            print(f"  [OK] No orphaned entities")
                ...
                if journal_mode == 'wal':
                    print(f"  [OK] Journal mode: WAL (optimal)")
                else:
            ...
            self.issues.append(f"Error reviewing SQLite: {str(e)}")
