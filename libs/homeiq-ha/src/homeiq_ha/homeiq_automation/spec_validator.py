...
SCHEMA_PATHS = [
    ...
    / "HomeIQ_AutomationSpec_v1.schema.json",  # Primary: same directory as validator
    Path(__file__).parent.parent.parent
    / "implementation"
    / "HomeIQ_API_Driven_Automations_Docs"
    / "HomeIQ_AutomationSpec_v1.schema.json",  # Fallback: original location
    Path(
        "/app/implementation/HomeIQ_API_Driven_Automations_Docs/HomeIQ_AutomationSpec_v1.schema.json"
    ),  # Docker fallback (unlikely needed now)
]
        ...
        if schema_path.exists():
            try:
                ...
                raise json.JSONDecodeError(
                    f"Schema file contains invalid JSON: {e.msg}", e.doc, e.pos
                ) from e
            except OSError as e:
...
class SpecValidator:
    ...
    def _validate_custom_rules(self, spec: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if not actions:
            errors.append(
                {"field": "actions", "message": "At least one action is required", "value": actions}
            )
    ...
    def _validate_trigger_types(self, triggers: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for i, trigger in enumerate(triggers):
            ...
            elif trigger_type == "webhook" and "webhook_id" not in trigger:
                errors.append(
                    {
    ...
    def validate_file(self, file_path: Path) -> tuple[bool, list[dict[str, Any]]]:
        ...
        try:
            ...
            return False, [
                {"field": "file", "message": f"Invalid JSON format: {e}", "value": str(file_path)}
            ]
            ...
            return False, [
                {"field": "file", "message": f"Failed to read file: {e}", "value": str(file_path)}
            ]
