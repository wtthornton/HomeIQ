...
class HomeIQMetadata(BaseModel):
    ...
    created_by: str = Field(
        default="ai-automation-service", description="Service that created this automation"
    )
...
class PatternContext(BaseModel):
    ...
    pattern_type: str = Field(
        description="Pattern type (time_of_day, co_occurrence, anomaly, etc.)"
    )
    pattern_id: int = Field(description="Pattern ID")
    pattern_metadata: dict[str, Any] = Field(
        default_factory=dict, description="Pattern-specific metadata"
    )
...
class DeviceContext(BaseModel):
    ...
    device_types: list[str] = Field(
        default_factory=list, description="Device types (light, sensor, etc.)"
    )
...
class SafetyChecks(BaseModel):
    ...
    requires_confirmation: bool = Field(
        default=False, description="Requires user confirmation before deployment"
    )
    critical_devices: list[str] | None = Field(
        None, description="Critical device IDs (locks, security, etc.)"
    )
    time_constraints: TimeConstraints | None = Field(
        None, description="Time-based safety constraints"
    )
    safety_warnings: list[str] | None = Field(
        None, description="Safety warnings for this automation"
    )
...
class EnergyImpact(BaseModel):
    ...
    estimated_power_w: float | None = Field(
        None, ge=0.0, description="Estimated power consumption in watts"
    )
    estimated_daily_kwh: float | None = Field(
        None, ge=0.0, description="Estimated daily energy in kWh"
    )
...
class TriggerConfig(BaseModel):
    ...
    parameters: dict[str, Any] = Field(
        default_factory=dict, description="Trigger-specific parameters"
    )
...
class HomeIQTrigger(BaseModel):
    ...
    platform: str = Field(
        ..., description="Trigger platform (e.g., 'state', 'button', 'climate', 'time')"
    )
...
class HomeIQCondition(BaseModel):
    ...
    condition: str = Field(
        ..., description="Condition type (e.g., 'state', 'numeric_state', 'time')"
    )
...
class HomeIQAction(BaseModel):
    ...
    target: Target | None = Field(
        None, description="Enhanced targeting (areas, floors, labels, devices, entities)"
    )
...
class HomeIQAutomation(BaseModel):
    ...
    pattern_context: PatternContext | None = Field(
        None, description="Pattern that generated this automation"
    )
    ...
    mode: AutomationMode = Field(
        default=AutomationMode.SINGLE, description="Automation execution mode"
    )
