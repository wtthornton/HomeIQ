...
class VersionAwareRenderer:
    ...
    def _adapt_spec_for_version(self, spec: AutomationSpec) -> AutomationSpec:
        ...
        if self.capabilities.get("initial_state_required", True) and (
            not hasattr(spec, "initial_state") or spec.initial_state is None
        ):
    ...
    def _apply_version_transformations(self, yaml_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        # Handle singular vs plural keys — convert trigger: to triggers:
        if not self.capabilities.get("singular_trigger_action", True) and "trigger" in yaml_dict:
            yaml_dict["triggers"] = yaml_dict.pop("trigger")

        # Convert action: to actions:
        if not self.capabilities.get("singular_action", True) and "action" in yaml_dict:
