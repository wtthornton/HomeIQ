...
class ValidationPipeline:
    ...
    def _validate_schema(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        if "action" in data and isinstance(data["action"], list):
            for i, action in enumerate(data["action"]):
                ...
                # The advanced action types are the second half of the same test:
                # an action is valid if it names any one of them.
                elif (
    ...
    def _validate_entity_states(
        self, data: dict[str, Any], _entity_ids: list[str], _entity_states: dict[str, str]
    ) -> dict[str, list[str]]:
        """
            ...
            for trigger in data["trigger"]:
                if isinstance(trigger, dict) and trigger.get("platform") == "state":
                    ...
                    if entity_id and to_state and "." in entity_id:
                        ...
                        # Check if state is valid for domain. Only the string
                        # form is checked: a list-valued `to` cannot reach here,
                        # since isinstance(to_state, str) already excludes it.
    ...
    def _validate_safety(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        critical_services = {
            ...
            "alarm_control_panel.alarm_arm_away": {
                "risk_level": "high",
                "description": "Arm alarm (away)",
            },
            "alarm_control_panel.alarm_arm_home": {
                "risk_level": "high",
                "description": "Arm alarm (home)",
            },
        ...
        if critical_services_used:
            ...
            medium_risk_services = [
                s for s in critical_services_used if s["risk_level"] == "medium"
            ]
    ...
    def _detect_risky_patterns(
        self, data: dict[str, Any], _entity_ids: list[str], services: list[str]
    ) -> list[str]:
        """
    ...
    def _is_valid_entity_id(self, entity_id: str) -> bool:
        ...
        # For now, accept all valid domain.entity patterns — the entity name must
        # still be alphanumeric with underscores.
        return bool(entity_name and entity_name.replace("_", "").isalnum())
        ...
        if isinstance(data, dict):
            ...
            if "service" in data:
                ...
                if (
                    ...
                    and "." in service
                    and self._is_valid_service_name(service)
                ):
    ...
    def _is_valid_service_name(self, service: str) -> bool:
        ...
        # Service name must be alphanumeric with underscores
        return not (not service_name or not service_name.replace("_", "").isalnum())
