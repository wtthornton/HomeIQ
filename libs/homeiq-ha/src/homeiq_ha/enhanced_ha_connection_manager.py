...
class HAConnectionConfig:
    ...
    circuit_breaker_config: dict[str, Any] = field(
        default_factory=lambda: {"fail_max": 5, "reset_timeout": 60, "success_threshold": 3}
    )
...
class ConnectionResult:
    ...
    success: bool
    connection_config: HAConnectionConfig | None = None
    error: str | None = None
...
class CircuitBreaker:
    ...
    def __init__(
        self, name: str, fail_max: int = 5, reset_timeout: int = 60, success_threshold: int = 3
    ):
        self.name = name
    ...
    def record_success(self):
        ...
        if self.state == CircuitBreakerState.HALF_OPEN and self.successes >= self.success_threshold:
            self.state = CircuitBreakerState.CLOSED
            self.state_changes += 1
            self.failures = 0
            logger.info(
                f"Circuit breaker {self.name} transitioning to CLOSED after {self.successes} successes"
            )
    ...
    def record_failure(self, _error: Exception):
        ...
        if self.state == CircuitBreakerState.CLOSED:
            if self.failures >= self.fail_max:
                ...
                self.state_changes += 1
                logger.warning(
                    f"Circuit breaker {self.name} transitioning to OPEN after {self.failures} failures"
                )
        elif self.state == CircuitBreakerState.HALF_OPEN:
            self.state = CircuitBreakerState.OPEN
            self.state_changes += 1
...
class EnhancedHAConnectionManager:
    ...
    def _load_connection_configs(self):
        ...
        if ha_http_url and ha_token:
            ...
            if ha_ws_url:
                try:
                    ...
                    logger.error(
                        f"❌ Failed to normalize WebSocket URL: {e}. Using HTTP URL instead."
                    )
        ...
        if nabu_casa_url and nabu_casa_token:
            try:
                ...
                logger.error(
                    f"❌ Failed to configure Nabu Casa fallback: {e}. URL: {nabu_casa_url}"
                )
    ...
    async def get_connection_with_circuit_breaker(self) -> HAConnectionConfig | None:
        """
        ...
        for connection in self.connections:
            ...
            try:
                ...
                if success:
                    ...
                    logger.info(
                        f"✅ Connected to {connection.name} ({connection.url}) in {response_time:.2f}s"
                    )
                    return connection
                else:
    ...
    async def _test_connection(self, config: HAConnectionConfig) -> bool:
        ...
        try:
            ...
            ssl_verify = (
                os.getenv("NABU_CASA_SSL_VERIFY", "true").lower() == "true"
    ...
    def get_connection_status(self) -> dict[str, Any]:
        ...
        for connection in self.connections:
            conn_status = {
                ...
                "circuit_breaker": self.circuit_breakers.get(connection.name).get_stats()
                if connection.name in self.circuit_breakers
                else None,
            }
        ...
        for connection in self.connections:
            try:
                ...
                conn_health = {
                    ...
                    "healthy": is_healthy,
                    "url": connection.url,
                    "circuit_breaker_state": self.circuit_breakers.get(connection.name).state.value
                    if connection.name in self.circuit_breakers
                    else "unknown",
                }
...
async def get_connection() -> HAConnectionConfig | None:
