...
class ConnectionResult:
    ...
    success: bool
    config: HAConnectionConfig | None
    error: str | None = None
...
class HAConnectionManager:
    ...
    def __init__(self):
        ...
        self.current_connection: HAConnectionConfig | None = None
    ...
    def _create_ssl_context(self) -> ssl.SSLContext | None:
        ...
        try:
            ...
            if os.getenv("DOCKER_CONTAINER", "").lower() in ("true", "1", "yes"):
                logger.info(
                    "🐳 Docker container detected - configuring SSL context for containerized environment"
                )
                ...
                logger.warning(
                    "⚠️ SSL certificate verification disabled for Docker container environment"
                )
    ...
    def _load_connection_configs(self):
        ...
        if not self.connections:
            ...
            logger.warning(
                "   Set HA_HTTP_URL/HA_WS_URL + HA_TOKEN or NABU_CASA_URL + NABU_CASA_TOKEN"
            )
        else:
        ...
        try:
            ...
            async with websockets.connect(
                ...
                if auth_data.get("type") == "auth_required":
                    ...
                    auth_message = {"type": "auth", "access_token": config.token}
                    ...
                    logger.info(
                        f"📋 Parsed auth result data from {config.name}: {auth_result_data}"
                    )

                    if auth_result_data.get("type") == "auth_ok":
                        response_time = asyncio.get_event_loop().time() - start_time
                        logger.info(
                            f"✅ Connection test successful: {config.name} ({response_time:.2f}s)"
                        )
                        return ConnectionResult(
                            success=True, config=config, response_time=response_time
                        )
                    else:
                        ...
                        logger.warning(
                            f"   Auth result type: {auth_result_data.get('type', 'No type field')}"
                        )
                        return ConnectionResult(success=False, config=config, error=error_msg)
                else:
    ...
    async def get_best_connection(self) -> HAConnectionConfig | None:
        """
    ...
    async def _test_connection_with_retry(self, config: HAConnectionConfig) -> ConnectionResult:
        ...
        for attempt in range(max_retries):
            logger.info(
                f"🔄 Attempting connection to {config.name} (attempt {attempt + 1}/{max_retries})"
            )
            ...
            if attempt < max_retries - 1:  # Don't wait after the last attempt
                logger.warning(
                    f"⚠️ Connection attempt {attempt + 1} failed, retrying in {retry_delay}s..."
                )
    ...
    async def get_connection(self) -> HAConnectionConfig | None:
        """
    ...
    def _update_connection_stats(self, name: str, success: bool, response_time: float | None):
            ...
            if auth_data.get("type") == "auth_required":
                ...
                if auth_result_data.get("type") != "auth_ok":
                    ...
                    raise ConnectionError(
                        f"Authentication failed: {auth_result_data.get('message', 'Unknown error')}"
                    )
            else:
                await websocket.close()
                raise ConnectionError(
                    f"Unexpected auth response: {auth_data.get('type', 'Unknown')}"
                )
    ...
    async def get_http_client(self) -> aiohttp.ClientSession | None:
        ...
        return aiohttp.ClientSession(base_url=http_url, headers=headers, timeout=timeout)
...
async def get_ha_connection() -> HAConnectionConfig | None:
...
async def get_ha_http_client() -> aiohttp.ClientSession | None:
