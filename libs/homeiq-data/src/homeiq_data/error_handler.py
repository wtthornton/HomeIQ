...
def handle_error(
    error: Exception, request: Request | None = None, context: dict[str, Any] | None = None
) -> HTTPException:
    """
    ...
    if isinstance(error, HomeIQError):
        logger.error(
            ...
            extra={**context, **error.context, "error_code": error.error_code},
        )
        ...
        error_response = {
            "error": {"code": error.error_code, "message": error.message, "context": error.context}
        }
        ...
        # Generic error
        error_msg = str(error)
        logger.error(f"Unhandled error: {error_msg}", extra=context, exc_info=True)
