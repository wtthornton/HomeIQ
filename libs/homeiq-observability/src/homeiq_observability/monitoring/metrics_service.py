...
class MetricsCollector:
    ...
    def _initialize_system_metrics(self):
        ...
        self.register_metric(
            "system_cpu_usage_percent", MetricType.GAUGE, "System CPU usage percentage", "percent"
        )
        self.register_metric("system_cpu_count", MetricType.GAUGE, "Number of CPU cores", "count")

        # Memory metrics
        self.register_metric(
            "system_memory_usage_bytes", MetricType.GAUGE, "System memory usage in bytes", "bytes"
        )
        self.register_metric(
        ...
        # Disk metrics
        self.register_metric(
            "system_disk_usage_bytes", MetricType.GAUGE, "System disk usage in bytes", "bytes"
        )
        self.register_metric(
            "system_disk_usage_percent", MetricType.GAUGE, "System disk usage percentage", "percent"
        )
        self.register_metric(
            "system_disk_free_bytes", MetricType.GAUGE, "System free disk space in bytes", "bytes"
        )

        # Network metrics
        self.register_metric(
            "system_network_bytes_sent", MetricType.COUNTER, "System network bytes sent", "bytes"
        )
        self.register_metric(
        ...
        # Process metrics
        self.register_metric(
            "system_process_count", MetricType.GAUGE, "Number of running processes", "count"
        )
    ...
    def record_value(self, name: str, value: float, labels: dict[str, str] | None = None):
        ...
        with self.metrics_lock:
            ...
            metric_value = MetricValue(
                timestamp=datetime.now(UTC).isoformat(), value=value, labels=labels
            )
...
class PerformanceTracker:
    ...
    def end_operation(self, timer_id: str, labels: dict[str, str] | None = None):
        ...
        if timer_id in self.operation_timers:
            ...
            self.metrics_collector.record_timer(
                metric_name, duration, {"operation": operation_name, **(labels or {})}
            )

    def record_event_processed(
        self, event_type: str, processing_time_ms: float, entity_id: str | None = None
    ):
        ...
        self.metrics_collector.record_timer(
            "event_processing_duration_seconds", processing_time_ms / 1000, labels
        )
    ...
    def record_error(self, error_type: str, service: str):
        ...
        self.metrics_collector.increment_counter(
            "errors_total", 1, {"error_type": error_type, "service": service}
        )
...
class MetricsService:
    ...
    def _initialize_application_metrics(self):
        ...
        # API metrics
        self.collector.register_metric(
            "api_requests_total", MetricType.COUNTER, "Total number of API requests", "count"
        )
        self.collector.register_metric(
            "api_request_duration_seconds", MetricType.HISTOGRAM, "API request duration", "seconds"
        )

        # Error metrics
        self.collector.register_metric(
            "errors_total", MetricType.COUNTER, "Total number of errors", "count"
        )
    ...
    def get_metrics(self, metric_names: list[str] | None = None) -> list[Metric]:
        """Get metrics."""
        if metric_names:
            return [
                self.collector.get_metric(name)
