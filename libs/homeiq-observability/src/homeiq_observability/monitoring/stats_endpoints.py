...
class StatsEndpoints:
    ...
    def _add_routes(self):
        ...
        async def get_statistics(
            ...
            service: str | None = Query(None, description="Specific service to get stats for"),
        ):
            ...
            try:
                ...
                if self.use_influxdb and self.influxdb_client.is_connected:
                    try:
                        ...
                        logger.warning(
                            f"InfluxDB query failed: {influx_error}, falling back to service calls"
                        )
        ...
        async def get_metrics(
            metric_name: str | None = Query(None, description="Specific metric name"),
        ...
        async def get_alerts():
            ...
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get alerts"
                ) from e
    ...
    async def _get_stats_from_influxdb(
        self, period: str, service: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        if service:
            service_metrics = {
                service: await self.influxdb_client.get_service_metrics(service, period)
            }
        else:
        ...
        return {
            "metrics": {**event_stats, **error_stats, "services": service_metrics},
            "trends": trends.get("trends", []),
            "alerts": alerts,
        }

    def _calculate_alerts(
        self, service_metrics: dict[str, Any], error_stats: dict[str, Any]
    ) -> list[dict[str, Any]]:
        """
        Calculate alerts from metrics
    ...
    async def _get_all_stats(self, period: str) -> dict[str, Any]:
        ...
        all_stats = {"metrics": {}, "trends": {}, "alerts": []}
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                try:
                    ...
                    raise Exception(f"Timeout after {timeout}s") from exc
        except Exception as e:
    ...
    async def _transform_websocket_health_to_stats(
        self, health_data: dict[str, Any], _period: str
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "trends": {
                    ...
                    "error_rate": [
                        {"timestamp": health_data.get("timestamp"), "value": metrics["error_rate"]}
                    ],
                },
                "alerts": [],
            }
    ...
    async def _get_metrics(
        self, metric_name: str | None, service: str | None, limit: int
    ) -> list[MetricData]:
        """Get specific metrics"""
        metrics = []
    ...
    async def _get_performance_stats(self) -> dict[str, Any]:
        """Get performance statistics"""
        performance_stats = {"overall": {}, "services": {}, "recommendations": []}

        for service_name, service_url in self.service_urls.items():
            try:
                async with (
                    aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session,
                    session.get(f"{service_url}/stats/performance") as response,
                ):
                    if response.status == 200:
                        data = await response.json()
                        performance_stats["services"][service_name] = data
                    else:
                        performance_stats["services"][service_name] = {
        ...
        performance_stats["overall"] = self._calculate_overall_performance(
            performance_stats["services"]
        )

        # Generate recommendations
        performance_stats["recommendations"] = self._generate_recommendations(
            performance_stats["services"]
        )
    ...
    async def _get_alerts(self) -> list[dict[str, Any]]:
        ...
        for service_name, service_url in self.service_urls.items():
            try:
                async with (
                    aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session,
                    session.get(f"{service_url}/alerts") as response,
                ):
                    if response.status == 200:
                        data = await response.json()
                        for alert in data:
        ...
        alerts.sort(
            key=lambda x: (severity_order.get(x.get("level", "info"), 3), x.get("timestamp", ""))
        )
    ...
    def _calculate_overall_performance(self, services_stats: dict[str, Any]) -> dict[str, Any]:
        ...
        for _service_name, stats in services_stats.items():
            if "error" in stats:
                continue
        ...
        if valid_services > 0:
            ...
            if overall["total_requests"] > 0:
                overall["success_rate"] = (
                    (overall["total_requests"] - overall["total_errors"])
        ...
        try:
            async with (
                ...
                if resp.status == 200:
                    ...
                    logger.warning(
                        f"Failed to get event rate from websocket-ingestion: {resp.status}"
                    )
                    return 0.0
        except Exception as e:
        ...
        for service_info in api_services:
            ...
            if service_name in self.service_urls:
                coro = self._get_api_metrics_with_timeout(
                    service_name, self.service_urls[service_name], service_info["timeout"]
                )
            else:
                ...
                async with session.get(health_url, timeout=5) as resp:
                    if resp.status == 200:
                        ...
                        if "events_per_hour" in data:
                            ...
                            and "event_rate_per_minute" in data["subscription"]
                        ):
    ...
    async def _get_api_metrics_with_timeout(
        self, service_name: str, service_url: str, timeout: int
    ) -> dict[str, Any]:
    ...
    def _create_fallback_metric(
        self, service_name: str, status: str, error_message: str = None
    ) -> dict[str, Any]:
