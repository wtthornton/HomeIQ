        ...
        async def get_health():
            ...
            try:
                ...
                healthy_services = sum(
                    1 for status in services_status.values() if status.get("status") == "healthy"
                )
                ...
                return {
                    ...
                    "ingestion_service": {
                        ...
                        "websocket_connection": {
                            ...
                            "last_connection_time": websocket_health.get(
                        ...
                        "event_processing": {
                            "status": "healthy"
                            if subscription_data.get("is_subscribed", False)
                            else "degraded",
                            "events_per_minute": subscription_data.get(
                        ...
                        "weather_enrichment": {
                            "enabled": True,
                            "cache_hits": websocket_health.get("weather_enrichment", {}).get(
                                "cache_hits", 0
                            ),
                            "api_calls": websocket_health.get("weather_enrichment", {})
                            .get("weather_client_stats", {})
                            .get("total_requests", 0),
                            "last_error": None,
                        },
                        "influxdb_storage": {
                            "is_connected": services_status.get("influxdb", {}).get("status")
                            == "healthy",
