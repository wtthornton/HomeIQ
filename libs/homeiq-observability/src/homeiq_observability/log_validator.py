...
class LogValidator:
    ...
    RECOMMENDED_FIELDS = {"correlation_id", "context", "performance"}
    ...
    def _validate_context_structure(self, log_data: dict[str, Any]):
        ...
        if "context" in log_data:
            ...
            recommended_context_fields = {"filename", "lineno", "function", "module", "pathname"}
            ...
            if "duration_ms" in performance:
                ...
                if not isinstance(duration, (int, float)) or duration < 0:
                    self.validation_errors.append(
                        "Invalid duration_ms value in performance metrics"
                    )
    ...
    def _check_consistency(self, log_data: dict[str, Any]):
        ...
        if log_data.get("level") == "ERROR" and "exception" not in log_data:
            self.validation_suggestions.append(
                "Error level logs should include exception information"
            )
        ...
        if "performance" in log_data and "operation" not in log_data["performance"]:
            self.validation_warnings.append(
                "Performance metrics present but no operation specified"
            )
    ...
    def validate_service_logs(self, service_name: str, log_entries: list[str]) -> ValidationResult:
        ...
        return ValidationResult(
            is_valid=is_valid, errors=all_errors, warnings=all_warnings, suggestions=all_suggestions
        )

    def _validate_service_consistency(
        self, service_name: str, log_entries: list[str], warnings: list[str], suggestions: list[str]
    ):
        ...
        if len(correlation_ids) > 1:
            suggestions.append(
                f"Multiple correlation IDs found in service logs: {len(correlation_ids)} unique IDs"
            )
