...
def setup_logging(
    ...
    if output in ["file", "both"]:
        log_file_path = Path(os.getenv("LOG_FILE_PATH", "/var/log/homeiq/"))
        log_file_path.mkdir(parents=True, exist_ok=True)

        file_handler = logging.handlers.RotatingFileHandler(
            filename=log_file_path / f"{service_name}.log",
...
def log_with_context(logger: logging.Logger, level: str, message: str, **context):
    ...
    """
    getattr(logger, level.lower())(message, extra={"context": context})


def log_performance(
    logger: logging.Logger, operation: str, duration_ms: float, status: str = "success", **metrics
):
    """
    Log performance metrics
