...
class MetricsAPI:
    ...
    async def query_metrics(self, query: MetricsQuery) -> MetricsResponse:
        ...
        try:
            ...
            raise Exception(f"Error querying metrics: {str(e)}") from e
    ...
    async def get_system_metrics(self, hours: int = 1) -> dict[str, Any]:
        ...
        query = MetricsQuery(
            measurement="system_resources", start_time=datetime.now(UTC) - timedelta(hours=hours)
        )
        ...
        for record in response.data:
            ...
            metrics["cpu"].append({"timestamp": timestamp, "percent": fields.get("cpu_percent", 0)})
    ...
    async def get_metrics_summary(self) -> dict[str, Any]:
        ...
        perf_query = MetricsQuery(
            measurement="performance", start_time=datetime.now(UTC) - timedelta(hours=24)
        )
        perf_response = await self.query_metrics(perf_query)

        # Get system metrics summary
        sys_query = MetricsQuery(
            measurement="system_resources", start_time=datetime.now(UTC) - timedelta(hours=1)
        )
        sys_response = await self.query_metrics(sys_query)

        # Get counter metrics summary
        counter_query = MetricsQuery(
            measurement="counters", start_time=datetime.now(UTC) - timedelta(hours=24)
        )
    ...
    async def check_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            recent_query = MetricsQuery(
                measurement="performance", start_time=datetime.now(UTC) - timedelta(minutes=5)
            )
            ...
            sys_query = MetricsQuery(
                measurement="system_resources", start_time=datetime.now(UTC) - timedelta(minutes=5)
            )
            ...
            health_status["checks"]["recent_metrics"] = {"status": "unhealthy", "error": str(e)}
...
async def get_service_health_metrics(metrics_api: MetricsAPI, service: str) -> dict[str, Any]:
    ...
    try:
        ...
        avg_response_time = (
            sum(op["avg_duration"] * op["count"] for op in perf_metrics["operations"].values())
            / total_ops
            if total_ops > 0
            else 0
        )
        ...
        return {"service": service, "health_score": 0, "error": str(e)}
...
async def get_system_health_metrics(metrics_api: MetricsAPI) -> dict[str, Any]:
    ...
    try:
        ...
        avg_memory = (
            sum(point["percent"] for point in memory_data) / len(memory_data) if memory_data else 0
        )
