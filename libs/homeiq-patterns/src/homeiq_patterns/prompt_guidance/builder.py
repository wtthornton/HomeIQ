...
class PromptBuilder:
    ...
    def build_automation_generation_prompt(
        additional_context: str = "", entity_context: dict[str, Any] | None = None
    ) -> str:
        """
        ...
        if entity_context:
            ...
            for domain, entities in entity_context.items():
                if entities:
                    entity_ids = [
                        e.get("entity_id", "") for e in entities[:30] if e.get("entity_id")
                    ]
                    if entity_ids:
    ...
    def build_suggestion_generation_prompt(
        device_inventory: dict[str, Any] | None = None, home_context: str = ""
    ) -> str:
        """
        ...
        """
        prompt = SUGGESTION_GENERATION_SYSTEM_PROMPT.format(CORE_IDENTITY=CORE_IDENTITY)

        if device_inventory:
            device_section = "\n\n## Available Devices\n\n"
            device_section += (
                "You may ONLY suggest automations for devices that exist in this list:\n\n"
            )
    ...
    def build_yaml_generation_prompt(
        ha_version: str = "2025.10", additional_instructions: str = ""
    ) -> str:
        """
