...
from src.miner.discourse_client import DiscourseClient
                    ...
                    try:
                        ...
                        if existing:
                            ...
                            if existing.vote_count != metadata.vote_count:
                                ...
                                skipped_count += 1
                        # Add new
                        elif not await repo.is_duplicate(metadata):
                            await repo.save_automation(metadata)
                            added_count += 1
                        else:
                            skipped_count += 1
                        ...
                        logger.exception(f"[{correlation_id}] Failed to process post {post['id']}: {e}")
                        continue
