...
async def lifespan(app: FastAPI):
    ...
    # Start background cleanup task for in-memory stores (memory leak prevention)
    try:
        start_background_cleanup()
        logger.info("✅ Background cleanup task started for in-memory stores")
    except Exception as e:
        logger.error(f"❌ Background cleanup task startup failed: {e}")
        # Continue without background cleanup - cleanup will still happen on-demand
    ...
    try:
        ...
        logger.error(f"❌ Background cleanup task shutdown failed: {e}")
...
app.include_router(alias_router)  # Epic 39.13: Alias Management endpoints (extracted from ask_ai_router)
app.include_router(analytics_router)  # Epic 39.13: Analytics endpoints (extracted from ask_ai_router)
