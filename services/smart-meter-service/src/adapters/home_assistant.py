...
class HomeAssistantAdapter(MeterAdapter):
    ...
    async def fetch_consumption(
        ...
        try:
            ...
            logger.exception(f"Error fetching from Home Assistant: {e}")
            raise
    ...
    async def _get_sensor_state(
        ...
        try:
            ...
            logger.exception(f"Network error fetching {entity_id}: {e}")
            return None
        except Exception as e:
            logger.exception(f"Unexpected error fetching {entity_id}: {e}")
            return None
    ...
    async def _get_circuit_data(
        ...
        try:
            ...
            logger.exception(f"Error fetching circuit data: {e}")
            return []
