...
def handle_route_errors(operation_name: str, default_status_code: int = 500) -> Callable[[F], F]:
    """
...
def handle_service_errors(
    service_name: str, operation: str, default_status_code: int = 500
) -> Callable[[F], F]:
    """
    ...
    return handle_route_errors(
        operation_name=f"{service_name} {operation}", default_status_code=default_status_code
    )
