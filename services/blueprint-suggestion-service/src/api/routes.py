...
# Cached schema check result, set at startup via init_schema_cache()
_schema_ok: bool = True


def get_suggestion_service() -> SuggestionService:
    """Dependency for getting suggestion service."""
...
async def init_schema_cache(db: AsyncSession) -> None:
    """Cache schema check result at startup to avoid per-request PRAGMA calls."""
    global _schema_ok
    _schema_ok = await check_schema_version(db)
    if not _schema_ok:
        logger.warning("Schema mismatch detected at startup - some columns may be missing")
...
async def get_suggestions(
    ...
    try:
        ...
        # Batch-enrich suggestions with missing blueprint names
        from ..clients.blueprint_client import BlueprintClient

        missing = [s for s in suggestions if not s.blueprint_name]
        if missing:
        ...
        response_suggestions = [_suggestion_to_response(s) for s in suggestions]
