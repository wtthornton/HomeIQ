...
async def get_game_status(team: str):
    ...
    try:
        ...
        logger.exception(f"Error getting game status for {team}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get game status: {e!s}",
        )
...
async def get_game_context(team: str):
    ...
    try:
        ...
        logger.exception(f"Error getting game context for {team}: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get game context: {e!s}",
        )
...
async def register_webhook(registration: WebhookRegistration):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to register webhook: {e!s}",
        )
...
async def list_webhooks():
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to list webhooks: {e!s}",
        )
...
async def delete_webhook(webhook_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to delete webhook: {e!s}",
        )
...
async def deliver_webhook(webhook: WebhookRegistration, event_type: str, payload: dict[str, Any]):
    ...
    try:
        ...
        logger.exception(f"Error delivering webhook: {e}")
        return False
...
async def webhook_event_detector():
    ...
    while True:
        try:
            ...
            logger.exception(f"Error in webhook event detector: {e}")
