...
from src.clients.pattern_aggregate_client import PatternAggregateClient
from src.config import settings
from src.database.crud import get_synergy_opportunities, record_analysis_run, store_patterns, store_suggestion
from src.database.models import get_db_session

# Epic AI-2 imports (Device Intelligence)
from src.device_intelligence import FeatureAnalyzer, FeatureSuggestionGenerator, update_device_capabilities_batch
...
from src.pattern_detection.anomaly_detector import AnomalyDetector
from src.pattern_detection.contextual_detector import ContextualDetector
from src.pattern_detection.day_type_detector import DayTypeDetector
from src.pattern_detection.duration_detector import DurationDetector
from src.pattern_detection.room_based_detector import RoomBasedDetector
from src.pattern_detection.seasonal_detector import SeasonalDetector

# New ML-enhanced pattern detectors
from src.pattern_detection.sequence_detector import SequenceDetector
from src.pattern_detection.session_detector import SessionDetector
...
class DailyAnalysisScheduler:
    ...
    async def run_daily_analysis(self):
        ...
        try:
            ...
            if getattr(settings, "enable_pdl_workflows", False):
                try:
                    from src.pdl.runtime import PDLExecutionError, PDLInterpreter
                    ...
                    logger.exception("❌ PDL nightly batch guardrail violation: %s", pdl_exc)
                    return
            ...
            try:
                ...
                logger.exception(f"⚠️ Device capability update failed: {e}")
            ...
            if getattr(settings, "enable_langchain_pattern_chain", False):
                try:
                    from src.langchain_integration.pattern_chain import build_pattern_detection_chain
            ...
            try:
                ...
                from src.synergy_detection import DeviceSynergyDetector
                ...
                try:
                    from src.contextual_patterns import WeatherOpportunityDetector
                ...
                try:
                    from src.contextual_patterns import EnergyOpportunityDetector
                ...
                try:
                    from src.contextual_patterns import EventOpportunityDetector
                ...
                try:
                    ...
                    from src.synergy_detection.ml_synergy_miner import DynamicSynergyMiner
                ...
                if synergies:
                    async with get_db_session() as db:
                        ...
                        from src.database.models import SynergyOpportunity
                ...
                logger.exception(f"⚠️ Synergy detection failed: {e}")
            ...
            try:
                ...
                logger.exception(f"⚠️ Feature analysis failed: {e}")
            ...
            async def process_pattern_suggestion(pattern, cached_contexts):
                try:
                    ...
                    logger.exception(f"     Failed to process pattern: {e}")
                    return None
            ...
            if opportunities:
                try:
                    ...
                    logger.exception(f"     ❌ Feature suggestion generation failed: {e}")
            else:
            ...
            async with get_db_session() as db:
                ...
                try:
                    ...
                    await db.rollback()
                    logger.exception(f"   ❌ Failed to commit suggestions: {e}")
                    suggestions_stored = 0
            ...
            try:
                ...
                logger.exception(f"Failed to persist analysis run summary: {e}")
    ...
    def get_next_run_time(self) -> datetime | None:
        ...
        try:
            ...
            logger.exception(f"Failed to get next run time: {e}")
        return None
    ...
    async def _maybe_run_self_improvement(self, job_result: dict[str, Any]) -> None:
        ...
        try:
            from src.langchain_integration.self_improvement import generate_prompt_tuning_report
