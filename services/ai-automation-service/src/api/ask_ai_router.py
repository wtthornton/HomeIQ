...
from src.database.models import AskAIQuery as AskAIQueryModel
from src.database.models import ClarificationSessionDB
from src.entity_extraction import EnhancedEntityExtractor, MultiModelEntityExtractor, extract_entities_from_query
from src.guardrails.hf_guardrails import get_guardrail_checker
from src.llm.openai_client import OpenAIClient
from src.model_services.orchestrator import ModelOrchestrator
from src.model_services.soft_prompt_adapter import SoftPromptAdapter, get_soft_prompt_adapter
...
from src.services.clarification.models import AmbiguitySeverity
from src.services.clarification.outcome_tracker import ClarificationOutcomeTracker
from src.services.component_detector import ComponentDetector
from src.services.entity_attribute_service import EntityAttributeService
from src.services.rag import RAGClient
from src.services.safety_validator import SafetyValidator
from src.services.service_container import ServiceContainer
from src.services.yaml_self_correction import YAMLSelfCorrectionService
from src.utils.capability_utils import format_capability_for_display, normalize_capability
...
async def _update_model_comparison_metrics_on_approval(
    ...
    try:
        ...
        from src.database.models import ModelComparisonMetrics
...
def _build_entity_validation_context_with_comprehensive_data(entities: list[dict[str, Any]], enriched_data: dict[str, dict[str, Any]] | None = None) -> str:
    ...
    """
    from src.services.comprehensive_entity_enrichment import format_comprehensive_enrichment_for_prompt
...
async def get_reverse_engineering_analytics(
    ...
    """
    try:
        from src.services.reverse_engineering_metrics import get_reverse_engineering_analytics
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve analytics: {e!s}",
        ) from e
...
async def search_entities(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to search entities: {e!s}",
        ) from e
...
if settings.ha_url and settings.ha_token:
    try:
        ...
        logger.exception(f"❌ Failed to initialize HA client: {e}")
...
async def verify_entities_exist_in_ha(
    ...
    if use_ensemble:
        try:
            from src.services.ensemble_entity_validator import EnsembleEntityValidator
...
async def map_devices_to_entities(
    ...
    if enriched_data:
        ...
        first_entity_id = next(iter(enriched_data.keys())) if enriched_data else None
        if first_entity_id:
...
async def pre_validate_suggestion_for_yaml(
    ...
    all_mentions = {}
    for _field, text in text_fields.items():
...
async def generate_automation_yaml(
    ...
    if not openai_client:
        msg = "OpenAI client not initialized - cannot generate YAML"
    ...
    if validated_entities:
        ...
        for term, entity_id in validated_entities.items():
            ...
            [term, term.lower(), term.upper(), term.title()]
...
"""
        ...
        example_entity = next(iter(validated_entities.values())) if validated_entities else "{EXAMPLE_ENTITY_ID}"
...
"""
        ...
        if entity_context_json:
            ...
            if db_session:
                try:
                    from src.services.service_validator import ServiceValidator
...
"""
        ...
        msg = "No validated entities available - cannot generate YAML"
        raise ValueError(msg)
    ...
    if validated_entities:
        ...
        next((eid for eid in validated_entities.values() if "motion" in eid.lower() and eid.startswith("binary_sensor.")), example_sensor)
        next((eid for eid in validated_entities.values() if "wled" in eid.lower()), example_light)
        example_entity_1 = example_light or example_entity
        next((eid for eid in list(validated_entities.values())[1:2] if eid.startswith("light.")), example_light) or example_entity_1
    else:
...
"""
    ...
    try:
        ...
        from src.database.crud import get_system_settings
        ...
        if enable_parallel:
            ...
            from src.services.parallel_model_tester import ParallelModelTester
            ...
            try:
                ...
                logger.exception(f"❌ Generated invalid YAML syntax: {e}")
                msg = f"Generated YAML syntax is invalid: {e}"
                raise ValueError(msg) from e

            # Validate YAML structure and fix service names
            from src.services.yaml_structure_validator import YAMLStructureValidator
...
- Maximum 20 words"""
    ...
    try:
        ...
        logger.exception(f"Failed to simplify via AI: {e}, using fallback")
...
async def extract_entities_with_ha(query: str) -> list[dict[str, Any]]:
    ...
    if settings.entity_extraction_method == "multi_model" and _multi_model_extractor:
        try:
            ...
            logger.exception(f"Multi-model extraction failed, falling back to enhanced: {e}")
    ...
    if _enhanced_extractor:
        try:
            ...
            logger.exception(f"Enhanced extraction failed, falling back to basic: {e}")
...
async def resolve_entities_to_specific_devices(
    ...
    if resolved_devices:
        ...
        updated_entities = []
        {e.get("name", "").lower() for e in device_entities}
...
async def generate_suggestions_from_query(
    ...
    if not openai_client:
        msg = "OpenAI client not available - cannot generate suggestions"
        raise ValueError(msg)
    ...
    try:
                    ...
                    for qa in clarification_context["questions_and_answers"]:
                        ...
                        if selected:
                            for entity_ref in selected:
                                ...
                                if "." in entity_ref and (entity_ref.startswith(("light.", "switch.", "binary_sensor.", "sensor."))):
                ...
                if resolved_entity_ids:
                    ...
                    if enable_enrichment:
                        try:
                            ...
                            from src.services.enrichment_context_fetcher import (
                    ...
                    if cached_data:
                        ...
                        from src.services.comprehensive_entity_enrichment import enrich_entities_comprehensively
                    ...
                    if getattr(settings, "enable_token_counting", True):
                        from src.utils.entity_context_compressor import compress_entity_context
        ...
        if getattr(settings, "enable_token_counting", True):
            from src.utils.token_budget import check_token_budget, enforce_token_budget
        ...
        if getattr(settings, "enable_langchain_prompt_builder", False):
            try:
                from src.langchain_integration.ask_ai_chain import build_prompt_with_langchain
        ...
        try:
            ...
            for i, suggestion in enumerate(parsed):
                ...
                if enriched_data and devices_involved:
                    ...
                    if validated_entities:
                        ...
                        if entity_ids_found:
                            ...
                            logger.error("❌ No entity IDs found in devices_involved")
                ...
                if not validated_entities or len(validated_entities) == 0:
                    ...
                    logger.error(f"❌ devices_involved: {devices_involved}")
                    logger.error("❌ This indicates a bug in entity mapping logic - mapping should have populated validated_entities")
                ...
                logger.info(
                    ...
                    f"(count: {len(devices_involved)}, unique: {len({d.lower() for d in devices_involved})})",
                )
        ...
        if guardrail_checker:
            ...
            flagged_count = 0
            for suggestion, result in zip(suggestions, guardrail_results, strict=False):
...
async def process_natural_language_query(
    ...
    from src.utils.area_detection import extract_area_from_request
        ...
        if clarification_detector:
            try:
                ...
                if confidence_calculator.should_ask_clarification(confidence, ambiguities, threshold=adaptive_threshold):
                    ...
                    if questions:
                        try:
                            from src.database.crud import find_similar_past_answers

                            # Get RAG client if available for semantic matching
                            rag_client = None
                            if db:
                                try:
                                    from src.services.rag.rag_client import RAGClient
                                    openvino_url = os.getenv("OPENVINO_SERVICE_URL", "http://openvino-service:8019")
                                    rag_client = RAGClient(
                                        openvino_service_url=openvino_url,
                                        db_session=db,
    ...
    for qa in qa_list:
        ...
        if selected:
            for entity_ref in selected:
                ...
                if entity_ref.startswith(("light.", "switch.")) or "." in entity_ref:
                    # Likely an entity_id
    ...
    try:
        ...
        if (session.current_confidence >= session.confidence_threshold or
            ...
            try:
                ...
                if not enriched_query or not enriched_query.strip():
                    ...
                    msg = "Enriched query cannot be empty"
                    raise ValueError(msg)
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Failed to rebuild enriched query: {e!s}",
                ) from e
            ...
            try:
                ...
                logger.exception("❌ Entity extraction timed out after 30 seconds")
                raise HTTPException(
                    status_code=504,
                    detail={
                        "error": "timeout",
            ...
            try:
                ...
                logger.exception("❌ Entity re-enrichment timed out after 45 seconds")
                raise HTTPException(
                    status_code=504,
                    detail={
                        "error": "timeout",
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        continue
                    logger.exception(f"❌ All {max_retries} attempts timed out")
                    raise HTTPException(
                        status_code=504,
                        detail={
                            "error": "timeout",
                    ...
                    if ("Empty content from OpenAI" in error_str or
                        ...
                        logger.exception(f"❌ OpenAI returned empty content on attempt {attempt + 1}/{max_retries} (likely rate limit or token overflow): {error_str}")
                        if attempt < max_retries - 1:
                    ...
                    if "entity mapping failures" in error_str.lower() or "skipped" in error_str.lower():
                        ...
                        raise HTTPException(
                            ...
                            detail={
                                ...
                                "message": f"Failed to generate suggestions: {e!s}. This usually means the device names don't match available entities. Please check your device names and try again.",
                    ...
                    if os.getenv("ENVIRONMENT") == "production":
                        ...
                        error_message = f"Failed to generate suggestions: {e!s}"
                    raise HTTPException(
                        status_code=500,
                        detail={
                    ...
                    if os.getenv("ENVIRONMENT") == "production":
                        ...
                        error_message = f"Failed to generate suggestions after {max_retries} attempts: {e!s}"
                    raise HTTPException(
                        status_code=500,
                        detail={
                            "error": "internal_error",
        ...
        if len(remaining_ambiguities) == 0:
            ...
            try:
                ...
                logger.exception("❌ Entity extraction timed out after 5 seconds (all-ambiguities-resolved path)")
                raise HTTPException(
                    status_code=504,
                    detail={
                        "error": "timeout",
            ...
            try:
                ...
                logger.exception("❌ Entity re-enrichment timed out after 45 seconds (all-ambiguities-resolved path)")
                raise HTTPException(
                    status_code=504,
                    detail={
                        "error": "timeout",
            ...
            try:
                ...
                logger.exception("❌ Suggestion generation timed out after 60 seconds (all-ambiguities-resolved path)")
                raise HTTPException(
                    status_code=504,
                    detail={
                        "error": "timeout",
                ...
                if ("Empty content from OpenAI" in error_str or
                    "Empty response from OpenAI" in error_str):
                    logger.exception(f"❌ OpenAI returned empty content (all-ambiguities-resolved path): {error_str}")
                    raise HTTPException(
                        status_code=503,
                        detail={
                            "error": "api_error",
        ...
        raise HTTPException(
            ...
            detail={
                "error": "internal_error",
                "message": f"Failed to process clarification: {e!s}",
            },
        ) from e
...
async def get_query_suggestions(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to retrieve suggestions: {e!s}",
        ) from e
...
def _detects_timing_requirement(query: str) -> bool:
    ...
    import re
    return any(re.search(keyword, query_lower) for keyword in timing_keywords)
...
def _generate_test_quality_report(
    ...
    if validated_entities:
        ...
        for _device_name, entity_id in validated_entities.items():
...
{{
  ...
  "confidence": 0.0-1.0
        ...
        try:
            ...
            logger.exception(f"Failed to analyze test execution: {e}")
            return {
                "success": True,  # Default to success if analysis fails
                "issues": [f"Analysis unavailable: {e!s}"],
                "recommendations": [],
                "confidence": 0.5,
            }
    ...
    try:
        ...
        if correction_service:
            try:
                ...
                try:
                    from src.services.reverse_engineering_metrics import store_reverse_engineering_metrics
        ...
        logger.exception(f"HTTPException in test endpoint: {e.detail}")
        raise
    except Exception as e:
...
{{
  ...
  "intent_validation": "explanation of how restored components match user intent"
    ...
    try:
        ...
        logger.exception(f"Failed to restore components: {e}")
        # Preserve all original suggestion data including validated_entities
        return {
            "suggestion": original_suggestion.copy(),  # Make copy to preserve validated_entities
            "restored_components": stripped_components,
            "restoration_log": [f"Restoration attempted but failed: {e!s}"],
...
async def approve_suggestion_from_query(
    ...
    try:
        ...
        try:
            ...
            error_msg = str(e)
            logger.exception(f"❌ YAML generation failed: {error_msg}")
        ...
        if ha_client:
            try:
                ...
                if parsed_yaml:
                    from src.services.entity_id_validator import EntityIDValidator
                    ...
                    for entity_id in unique_entity_ids:
                        try:
                            ...
                            logger.exception(f"❌ Connection error during entity validation: {connection_error}")
                            break
                        except Exception as e:
                            ...
                            if parsed_yaml:
                                ...
                                for entity_id in unique_entity_ids_after_fix:
                                    try:
                                        ...
                                        logger.exception(f"❌ Connection error during re-validation: {connection_error_after_fix}")
                                        break
                                    except Exception as e:
                                        # Other errors - treat as entity not found
                ...
                return {
                    ...
                    "error_details": {
                        "type": "validation_error",
                        "message": f"Entity validation failed: {e!s}",
                    },
                }
        ...
        try:
            ...
            error_message = str(e)
            logger.exception(f"❌ [DEPLOY] Exception during HA deployment: {type(e).__name__}: {error_message}")
        ...
        logger.exception(f"❌ [APPROVAL] Unexpected exception: {type(e).__name__}: {e!s}")
...
async def list_aliases(
    ...
    try:
        ...
        alias_service = AliasService(db)
        return await alias_service.get_all_aliases(user_id)
...
async def get_model_comparison_summary(
    ...
    try:
        ...
        def analyze_task_type(task_metrics, task_name):
            ...
            sum(m.model1_latency_ms for m in task_metrics) / len(task_metrics)
            sum(m.model2_latency_ms for m in task_metrics) / len(task_metrics)
...
async def reverse_engineer_yaml(request: dict[str, Any]):
    ...
    try:
        ...
        if not yaml_content or not original_prompt:
            msg = "yaml and original_prompt are required"
            raise ValueError(msg)
