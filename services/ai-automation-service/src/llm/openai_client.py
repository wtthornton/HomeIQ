...
from src.utils.token_counter import count_message_tokens, get_token_breakdown
...
class OpenAIClient:
    ...
    def _infer_category(self, pattern: dict) -> str:
        ...
        if any(keyword in device_id for keyword in ["light", "switch"]):
            return "convenience"
        if any(keyword in device_id for keyword in ["climate", "thermostat", "temperature", "hvac"]):
            return "comfort"
        if any(keyword in device_id for keyword in ["alarm", "lock", "door", "camera", "motion"]):
            return "security"
        if any(keyword in device_id for keyword in ["energy", "power", "electricity"]):
            return "energy"
        return "convenience"
...
"""
    ...
    async def generate_with_unified_prompt(
        ...
        try:
            ...
            logger.exception(f"❌ Unified prompt generation error: {e}")
            import traceback
...
Choose the category that BEST fits the primary purpose of this automation.""",
            ...
            logger.exception(f"❌ Failed to infer category: {e}")
