...
class OpenAIClient:
    ...
    def __init__(self, api_key: str, model: str = "gpt-4o", enable_token_counting: bool = True):
        ...
        self.endpoint_stats: dict[
            str, dict[str, Any]
        ] = {}  # endpoint_name -> {tokens, cost, calls}
    ...
    def _parse_automation_response(self, llm_response: str, pattern: dict) -> AutomationSuggestion:
        ...
        description = (
            self._extract_description(llm_response) or "Automation based on detected pattern"
        )
    ...
    def _extract_yaml(self, text: str) -> str | None:
        ...
        for line in lines:
            ...
            if in_yaml:
                ...
                if any(
                    marker in line.upper() for marker in ["RATIONALE:", "CATEGORY:", "PRIORITY:"]
                ):
                    break
    ...
    def _extract_rationale(self, text: str) -> str | None:
        ...
        match = re.search(
            r"RATIONALE:\s*(.+?)(?:CATEGORY:|PRIORITY:|$)", text, re.IGNORECASE | re.DOTALL
        )
        return match.group(1).strip() if match else None
    ...
    def _infer_category(self, pattern: dict) -> str:
        ...
        if any(keyword in device_id for keyword in ["light", "switch"]):
            ...
            keyword in device_id for keyword in ["climate", "thermostat", "temperature", "hvac"]
        ):
            return "comfort"
...
"""
            ...
            device1_name = (
                device1.split(".")[-1].replace("_", " ").title() if "." in device1 else device1
            )
            device2_name = (
                device2.split(".")[-1].replace("_", " ").title() if "." in device2 else device2
            )
...
"""
    ...
    def get_usage_stats(self) -> dict:
        ...
        cost = CostTracker.calculate_cost(
            self.total_input_tokens, self.total_output_tokens, model=self.model
        )
        ...
        for endpoint_key, stats in self.endpoint_stats.items():
            ...
            endpoint_breakdown[endpoint]["models"][model] = {
                ...
                "avg_cost_per_call": round(stats["cost_usd"] / stats["calls"], 4)
                if stats["calls"] > 0
                else 0.0,
            }
    ...
    async def generate_with_unified_prompt(
        ...
        gpt51_use_case: str
        | None = None,  # GPT-5.1 use case: "deterministic", "creative", "structured", "extraction"
    ) -> dict:
        """
        ...
        """
        try:
            messages = [{"role": "system", "content": prompt_dict["system_prompt"]}]
            ...
            if settings and getattr(settings, "enable_prompt_caching", True):
                ...
                if system_prompt:
                    ...
                    logger.debug(
                        f"Prompt caching requested (cache key: {cache_key[:8]}...) - not implemented in current SDK version"
                    )
            ...
            if is_gpt51_model(self.model):
                ...
                if gpt51_use_case is None:
                    ...
                    if output_format == "json" or output_format == "yaml":
                        gpt51_use_case: Literal["deterministic", "creative", "structured", "extraction"] = "structured" if temperature < 0.5 else "creative"
                    else:
            ...
            if not content:
                ...
                logger.error(
                    f"❌ Empty content from OpenAI API. Finish reason: {finish_reason}, Model: {self.model}"
                )
            ...
            pattern_source: dict[str, Any] = prompt_dict.get("pattern_source", {})
            ...
            if isinstance(error_body, str):
                ...
                try:
                    ...
                    error_body = {}
            error_type = (
                error_body.get("error", {}).get("type", "") if isinstance(error_body, dict) else ""
            )
            error_code = (
                error_body.get("error", {}).get("code", "") if isinstance(error_body, dict) else ""
            )
            ...
            if error_type == "insufficient_quota" or error_code == "insufficient_quota":
                ...
                logger.warning(
                    f"⚠️ OpenAI API rate limit hit, will retry with exponential backoff: {e}"
                )
                raise
...
Choose the category that BEST fits the primary purpose of this automation.""",
            ...
            if category not in ["energy", "comfort", "security", "convenience"]:
                logger.warning(
                    f"Invalid category '{category}' from LLM, defaulting to 'convenience'"
                )
                category = "convenience"
            ...
            return {"category": "convenience", "priority": "medium"}
        finally:
