...
class DeviceSynergyDetector:
    ...
    async def detect_synergies(self) -> list[dict]:
        ...
        try:
            ...
            # Note: PDL workflows are optional - gracefully handle if not available
            if getattr(settings, "enable_pdl_workflows", False):
                try:
                    # PDL runtime will be available in later stories
    ...
    async def _filter_existing_automations(
        ...
        try:
            ...
            # Note: AutomationParser will be available in later stories
            # For now, gracefully handle if not available
            try:
