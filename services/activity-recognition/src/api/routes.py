...
from ..models.activity_classifier import predict_with_onnx
...
class SensorReading(BaseModel):
    ...
    motion: float = Field(0.0, ge=0.0, le=1.0, description="Motion sensor value (0 or 1)")
    door: float = Field(0.0, ge=0.0, le=1.0, description="Door sensor value (0 or 1)")
    temperature: float = Field(20.0, ge=-50.0, le=80.0, description="Temperature in Celsius")
    humidity: float = Field(50.0, ge=0.0, le=100.0, description="Humidity percentage")
    power: float = Field(0.0, ge=0.0, le=100000.0, description="Power consumption in Watts")
...
def predict_activity(sensor_array: np.ndarray) -> tuple[int, np.ndarray]:
    ...
    # Run inference using shared predict_with_onnx
    predictions, probs = predict_with_onnx(session, input_array)

    return int(predictions[0]), probs[0]
