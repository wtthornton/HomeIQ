...
- Observability tier gated behind an `observability` profile: **48 → 42 production services**, ratchet updated deliberately. Tracing export now opt-in.
...
## Next (P0)
- Build the pre-provisioned Home Assistant image: pin the HA version, vendor Powercalc, Team Tracker and the Aqara FP1E quirk into `/config/custom_components/`, ship no HACS at runtime, and add a checked-in component version manifest CI verifies. It is testable today against the existing `home-assistant-test` fixture and unblocks the next three stories.
...
## Verify
- `git log --oneline -1` — expect `2ecb4076`
